...
//_________________
McRun::McRun()
   : TNamed("run", "Run Header"), fGenerator(""), fComment(""), fDecayer(""), fAProj(0), fZProj(0), fPProj(0),
     fATarg(0), fZTarg(0), fPTarg(0), fBMin(0), fBMax(0), fBWeight(0), fPhiMin(0), fPhiMax(0), fXSection(0), fNEvents(0)
{
   // Default constructor. Should not be used
   std::cout << "Warning: Default constructor of McRun should not be used!" << std::endl;
}

//_________________
McRun::McRun(const char *generator, const char *comment, const Int_t &aProj, const Int_t &zProj, const Double_t &pProj,
             const Int_t &aTarg, const Int_t &zTarg, const Double_t &pTarg, const Double_t &bMin, const Double_t &bMax,
             const Int_t &bWeight, const Double_t &phiMin, const Double_t &phiMax, const Double_t &sigma,
             const Int_t &nEvents)
   : TNamed("run", "Run Header")
{
   ...
   fComment   = comment;
   fAProj =
      ((TMath::Abs(aProj) >= std::numeric_limits<short>::max()) ? std::numeric_limits<short>::max() : (Short_t)aProj);
   fZProj =
      ((TMath::Abs(zProj) >= std::numeric_limits<short>::max()) ? std::numeric_limits<short>::max() : (Short_t)zProj);
   fPProj = (Float_t)pProj;
   fATarg =
      ((TMath::Abs(aTarg) >= std::numeric_limits<short>::max()) ? std::numeric_limits<short>::max() : (Short_t)aTarg);
   fZTarg =
      ((TMath::Abs(zTarg) >= std::numeric_limits<short>::max()) ? std::numeric_limits<short>::max() : (Short_t)zTarg);
...
Double_t McRun::projectileEnergy() const
{
   ...
   if (fAProj > 0) { // nucleus
      ...
      std::cout << "Warning:: McRun: Projectile mass " << fAProj << " not valid! " << std::endl;
   }
   return eProj;
}
...
Double_t McRun::targetEnergy() const
{
   ...
   if (fATarg > 0) { // nucleus
      ...
      std::cout << "Warning:: McRun: Target mass " << fATarg << " not valid! " << std::endl;
   }
   return eTarg;
}
...
Double_t McRun::nnSqrtS() const
{
   // Get the cm energy
   Double_t eSum =
      (TMath::Sqrt(fPTarg * fPTarg + mProtMass * mProtMass) + TMath::Sqrt(fPProj * fPProj + mNeutMass * mNeutMass));
