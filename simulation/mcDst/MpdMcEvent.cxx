...
McEvent::McEvent()
   : TObject(), fEventNr(0), fB(0), fPhi(0), fNes(0), fStepNr(0), fStepT(0), fComment(""), fNpart(-1), fNcoll(-1)
{
   // Default constructor
   /* empty */
}
...
//_________________
void McEvent::setParameters(const Int_t &eventNr, const Double_t &b, const Double_t &phi, const Int_t &nes,
                            const Int_t &stepNr, const Double_t &stepT, const char *comment)
{
   ...
   fPhi     = (Float_t)phi;
   fNes =
      ((nes > std::numeric_limits<unsigned short>::max()) ? std::numeric_limits<unsigned short>::max() : (UShort_t)nes);
   fStepNr  = ((stepNr > std::numeric_limits<unsigned short>::max()) ? std::numeric_limits<unsigned short>::max()
                                                                     : (UShort_t)stepNr);
