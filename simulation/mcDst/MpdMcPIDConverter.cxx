...
Int_t McPIDConverter::pdgCode(const Int_t &pid, const EConvention &pidType)
{
   ...
   if (mIt == fConversionTables.end()) {
      ...
      if (mIt == fConversionTables.end()) {
         Error("GetPDGCode", "PID conversion table not available for convention=%u", pidType);
         return 0;
      }
   }
   ...
   if (pidIt == conversionTable.end()) {
      Warning("GetPDGCode", "PDG code not found for convention=%u, pid=%d", pidType, pid);
      return 0;
   }
...
void McPIDConverter::loadConversionTable(const EConvention &pidType)
{
   ...
   switch (pidType) {
   case (ePluto): dataFileName += "pluto_pdg.dat"; break;
   case (eUrQMD): dataFileName += "urqmd_pdg.dat"; break;
   case (eWerner):
      // VENUS, NEXUS, EPOS
      dataFileName += "werner_pdg.dat";
      break;
   default:
      Error("LoadConversionTable", "I do not know where to find conversion table for convention=%u", pidType);
      return;
   }
   ...
   if (!fin.good()) {
      Error("LoadConversionTable", "Failed to open conversion-table file %s", fullFileName);
      return;
   }
