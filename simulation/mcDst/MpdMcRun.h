...
public:
   ...
   McRun(const char *generator, const char *comment, const Int_t &aProj, const Int_t &zProj, const Double_t &pProj,
   ...
   void setNEvents(const Int_t &nEvents)
   {
      if (nEvents < 0) {
         ...
         fNEvents =
            ((nEvents > std::numeric_limits<int>::max()) ? std::numeric_limits<unsigned int>::max() : (UInt_t)nEvents);
      }
   }
   /// Set amount of event that was requested
