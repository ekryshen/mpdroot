...
public:
   ...
   McParticle(const Int_t &index, const Int_t &pdg, const Int_t &status, const Int_t &parent, const Int_t &parentDecay,
              const Int_t &mate, const Int_t &decay, Int_t child[2], const Double_t &px, const Double_t &py,
              const Double_t &pz, const Double_t &e, const Double_t &x, const Double_t &y, const Double_t &z,
              const Double_t &t);
   /// Another constructor with parameters
   McParticle(const Int_t &index, const Int_t &pdg, const Int_t &status, const Int_t &parent, const Int_t &parentDecay,
              const Int_t &mate, const Int_t &decay, Int_t child[2], const TLorentzVector &mom,
   ...
   /// Return p (GeV/c)
   Double_t ptot() const { return TMath::Sqrt(px() * px() + py() * py() + pz() * pz()); }
   ...
   {
      return (TDatabasePDG::Instance()->GetParticle(fPdg)) ? TDatabasePDG::Instance()->GetParticle(fPdg)->Mass()
                                                           : -999.;
   }
   /// Return mass according to the PDG code (GeV/c^2)
   ...
   {
      return (TDatabasePDG::Instance()->GetParticle(fPdg)) ? TDatabasePDG::Instance()->GetParticle(fPdg)->Charge() : 0;
   }
   ...
   Double_t pdgEnergy() const { return TMath::Sqrt(ptot() * ptot() + pdgMass() * pdgMass()); }
   /// Return energy (GeV)
   ...
   TLorentzVector momentum() const { return TLorentzVector(fPx, fPy, fPz, fE); }
   /// Return four-momentum (px,py,pz,E)
   ...
   TLorentzVector position() const { return TLorentzVector(fX, fY, fZ, fT); }
   /// Return four-coordinate (x,y,z,t)
   TLorentzVector GetPosition() const { return position(); }
   /// Set four-coordinate to the pos vector
   void position(TLorentzVector &pos) const { pos.SetXYZT(fX, fY, fZ, fT); }
   /// Set four-coordinate to the pos vector
   ...
   {
      fIndex = ((index > std::numeric_limits<unsigned short>::max()) ? std::numeric_limits<unsigned short>::max()
                                                                     : (UShort_t)index);
   }
   /// Set particle index
   ...
   {
      fParent = ((parent > std::numeric_limits<unsigned short>::max()) ? std::numeric_limits<unsigned short>::max()
                                                                       : (UShort_t)parent);
   }
   /// Set parent index
   ...
   {
      fParentDecay =
         ((parentDecay > std::numeric_limits<unsigned short>::max()) ? std::numeric_limits<unsigned short>::max()
                                                                     : (UShort_t)parentDecay);
   }
   /// Set parent decay index
   void SetParentDecay(const Int_t &parentDecay) { setParentDecay(parentDecay); }
   /// Set index of the last collision partner
   void setMate(const Int_t &mate)
   {
      fMate = ((mate > std::numeric_limits<unsigned short>::max()) ? std::numeric_limits<unsigned short>::max()
                                                                   : (UShort_t)mate);
   }
   /// Set index of the last collision partner
   ...
   {
      fDecay =
         ((TMath::Abs(decay) > std::numeric_limits<short>::max()) ? std::numeric_limits<short>::max() : (Short_t)decay);
   }
   /// Set decay index (-1 if not decayed)
   ...
   {
      fChild[0] = ((child > std::numeric_limits<unsigned short>::max()) ? std::numeric_limits<unsigned short>::max()
                                                                        : (UShort_t)child);
   }
   /// Set index of the first child
   ...
   {
      fChild[1] = ((child > std::numeric_limits<unsigned short>::max()) ? std::numeric_limits<unsigned short>::max()
                                                                        : (UShort_t)child);
   }
   /// Set index of the second child
   ...
   void setMomentum(const Double_t &px, const Double_t &py, const Double_t &pz, const Double_t &e)
   {
      fPx = (Float_t)px;
      fPy = (Float_t)py;
      fPz = (Float_t)pz;
   ...
   void setPosition(const Double_t &x, const Double_t &y, const Double_t &z, const Double_t &t)
   {
      fX = (Float_t)x;
      fY = (Float_t)y;
      fZ = (Float_t)z;
