...
void McDstReader::setStatus(const Char_t *branchNameRegex, Int_t enable)
{
   ...
   for (Int_t iArr = 0; iArr < McArrays::NAllMcArrays; iArr++) {
      ...
      std::cout << "[WARNING] McDstReader::SetStatus " << enable << " to " << McArrays::mcArrayNames[iArr] << std::endl;
...
void McDstReader::setBranchAddresses(TChain *chain)
{
   ...
   for (Int_t i = 0; i < McArrays::NAllMcArrays; ++i) {
      ...
      if (!tb) {
         std::cout << "[WARNING] setBranchAddress: Branch name " << bname << " does not exist!" << std::endl;
         continue;
      }
      ts = bname;
      ts += "*";
...
void McDstReader::createArrays()
{
   // Create arrays
   for (Int_t iArr = 0; iArr < McArrays::NAllMcArrays; iArr++) {
      mMcArrays[iArr] = new TClonesArray(McArrays::mcArrayTypes[iArr], McArrays::mcArraySizes[iArr]);
   }
...
Bool_t McDstReader::loadEntry(Long64_t iEntry __attribute__((unused)))
{
   ...
   while (bytes <= 0) {
      ...
      std::cout << "[WARNING] Encountered invalid entry or I/O error while reading entry " << mEventCounter
                << " from \"" << mChain->GetName() << "\" input tree\n";
      bytes = mChain->GetEntry(mEventCounter++);
      nCycles++;
      std::cout << "[WARNING] Not input has been found for: " << nCycles << " times" << std::endl;
      if (nCycles >= 10) {
         std::cout << "[ERROR] Terminating MpdMcDstReader::loadEntry(Long64_t) after " << nCycles << " times!"
                   << std::endl;
         mStatusRead = false;
         break;
      }
