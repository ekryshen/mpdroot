...
MpdStack::MpdStack(Int_t size) // {
   ...
   : FairGenericStack(), fStack(), fParticles(new TClonesArray("TParticle", size)),
     fTracks(new TClonesArray("MpdMCTrack", size)), fStoreMap(), fStoreIter(), fIndexMap(), fIndexIter(), fPointsMap(),
     fCurrentTrack(-1), fNPrimaries(0), fNParticles(0), fNTracks(0), fIndex(0), fDecayFlag(0), fStoreSecondaries(kTRUE),
     fMinPoints(1),
     // fEnergyCut(0.),
     fEnergyCut(-0.001), // AZ
     fStoreMothers(kTRUE), fMinMotherMass(0.4), fMaxMotherMass(6.1),
...
void MpdStack::PushTrack(Int_t toBeDone, Int_t parentId, Int_t pdgCode, Double_t px, Double_t py, Double_t pz,
                         ...
                         Double_t polz, TMCProcess proc, Int_t &ntr, Double_t weight, Int_t is)
{
   ...
   // AZ
   if (fDecayFlag &&
       (!particle->GetPDG()->Stable() || (particle->GetPdgCode() == 221) || (particle->GetPdgCode() == 111)) &&
       particle->GetPDG()->DecayList()) {
      ...
      for (Int_t j = 0; j < npart; ++j) { // skip mother particle
         ...
         PushTrack(toDo, trackId, part->GetPdgCode(), part->Px(), part->Py(), part->Pz(), part->Energy(), part->Vx(),
                   part->Vy(), part->Vz(), time, polx, poly, polz, proc, ntr, weight, is);
      }
...
void MpdStack::PushTrack(Int_t toBeDone, Int_t parentId, Int_t pdgCode, Double_t px, Double_t py, Double_t pz,
                         Double_t e, Double_t vx, Double_t vy, Double_t vz, Double_t time, Double_t polx, Double_t poly,
                         Double_t polz, TMCProcess proc, Int_t &ntr, Double_t weight, Int_t is, Int_t secondMotherID)
{
   ...
   TParticle *particle =
      ...
      new (partArray[fNParticles++]) TParticle(pdgCode, trackId, secondMotherID, nPoints, daughter1Id, daughter2Id, px,
                                               py, pz, e, vx, vy, vz, time);
   particle->SetPolarisation(polx, poly, polz);
   particle->SetWeight(weight);
   particle->SetUniqueID(proc);
...
void MpdStack::FillTrackArray()
{
   ...
   for (Int_t iPart = 0; iPart < fNParticles; iPart++) {
      ...
      if (fStoreIter == fStoreMap.end()) {
         cout << "-E- MpdStack: Particle " << iPart << " not found in storage map!" << endl;
         Fatal("MpdStack::FillTrackArray", "Particle not found in storage map.");
      }
      Bool_t store = (*fStoreIter).second;

      if (store) {
         MpdMCTrack *track = new ((*fTracks)[fNTracks]) MpdMCTrack(GetParticle(iPart));
...
void MpdStack::UpdateTrackIndex(TRefArray *detList)
{
   ...
   for (Int_t i = 0; i < fNTracks; i++) {
      ...
      if (fIndexIter == fIndexMap.end()) {
         cout << "-E- MpdStack: Particle index " << iMotherOld << " not found in dex map! " << endl;
         Fatal("MpdStack::UpdateTrackIndex", "Particle index not found in map");
      }
   ...
   while ((det = (FairDetector *)detIter->Next())) {
      ...
      while ((hitArray = det->GetCollection(iColl++))) {
         ...
         for (Int_t iPoint = 0; iPoint < nPoints; iPoint++) {
            ...
            if (fIndexIter == fIndexMap.end()) {
               cout << "-E- MpdStack: Particle index " << iTrack << " not found in index map! " << endl;
               Fatal("MpdStack::UpdateTrackIndex", "Particle index not found in map");
            }
...
{
   cout << "-I- MpdStack: Number of primaries        = " << fNPrimaries << endl;
   cout << "              Total number of particles  = " << fNParticles << endl;
   cout << "              Number of tracks in output = " << fNTracks << endl;
   if (iVerbose) {
      for (Int_t iTrack = 0; iTrack < fNTracks; iTrack++) ((MpdMCTrack *)fTracks->At(iTrack))->Print(iTrack);
   }
}
// -------------------------------------------------------------------------
...
TParticle *MpdStack::GetParticle(Int_t trackID) const
{
   if (trackID < 0 || trackID >= fNParticles) {
      cout << "-E- MpdStack: Particle index " << trackID << " out of range." << endl;
...
void MpdStack::SelectTracks()
{
   ...
   if (lEDEBUGcounter <= 20) std::cout << "EDEBUG-- MpdStack::SelectTracks(): entered " << fNParticles << endl;
   lEDEBUGcounter++;
#endif
...
#ifdef EDEBUG
      if (lEDEBUGcounter <= 20) thisPart->Print();
      lEDEBUGcounter++;
#endif
#undef EDEBUG
      ...
      for (Int_t iDet = kSTS; iDet <= kMCORD; iDet++) {
         ...
         if (fPointsMap.find(a) != fPointsMap.end()) {
            ...
            if (iDet == kZDC)
               nPointsZDC += fPointsMap[a]; // NG save points in ZDC
            else
               nPointsNoZDC += fPointsMap[a]; // points not in ZDC
         }
      }
      ...
      if (iMother < 0)
         ...
         if (nPoints == 0) {
            ...
            if (fStoreMothers) {
               ...
               if (mass < 0.1 && thisPart->P() < 0.01)
                  store = kFALSE; // P < cut on e+- and gamma
               else if (mother->GetPDG()->Mass() > fMinMotherMass &&
                        mother->GetPDG()->Mass() < fMaxMotherMass && // 0.4 and 6.1
                        TMath::Abs(mother->GetPDG()->Mass() - 0.940) > 0.004 && mother->R() < fRadiusCut &&
                        abs(mother->Vz()) < fVzCut)
      ...
      for (Int_t i = 0; i < fNParticles; i++) {
         if (copyMap[i]) { // if particle is to be stored
            ...
            while (iMother >= 0) { // and it's mother is not primary
               ...
               if (abs(mother->Vz()) > fVzCut || mother->R() > fRadiusCut)
                  break;                   // NG and doesn't originate within Rcut/Zcut
               fStoreMap[iMother] = kTRUE; // store the mother
               ret                = moths.equal_range(iMother);

               for (it = ret.first; it != ret.second; ++it) {
                  TParticle *daught = GetParticle(it->second);
                  if (abs(daught->Vz()) > fVzCut || daught->R() > fRadiusCut)
                     continue; // NG and doesn't originate within Rcut/Zcut
