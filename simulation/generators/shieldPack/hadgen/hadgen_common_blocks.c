...
int hadgen_check_params()
{
   ...
   if ((inreac.TINT > 100) || (inreac.JPART < 1) || (inreac.JPART > 4) || (inreac.NUCLID <= 8)) debug.LCASC = 0;
   if ((inreac.JPART <= 0) || (inreac.JPART == 5) || ((inreac.JPART >= 12) && (inreac.JPART <= 20)) ||
       (inreac.JPART > 25))
      return 1;                                                    // error in initial data (particle type)
   if ((inreac.TINT < 0) || (inreac.TINT >= 10000001.0)) return 1; // error in energy value
   if ((inreac.NUCLID < 1) || (inreac.NUCLID > 107)) return 1;     // error: bad nuclid index
   if ((inreac.JPART >= 21) && (inreac.NUCLID == 1)) return 1;     // error
   ...
   if ((inreac.JPART >= 21) && (inreac.JPART <= 25)) {
      if ((hiproj.APROJ < 2.0) || (hiproj.APROJ > 260.0) || (hiproj.ZPROJ < 1.0) || (hiproj.ZPROJ > 100.0)) return 1;
...
{ // TODO WHAT?
   ...
   if (_float_compare(a6, 2820.) && _float_compare(a5, 2.)) return PARTICLE_He3;
   if (_float_compare(a6, 2820.) && _float_compare(a5, 1.)) return PARTICLE_Tritium;
   if (_float_compare(a6, 1880.) && _float_compare(a5, 1.)) return PARTICLE_Deuterium;
   if (_float_compare(a6, 12.) && _float_compare(a5, 12.)) return PARTICLE_Gamma;
   if (_float_compare(a6, 495.) && _float_compare(a5, 11.)) return PARTICLE_K0Short; // TODO Test and correct if doesn't
   if (_float_compare(a6, 495.) && _float_compare(a5, .0)) return PARTICLE_K0Long;
   if (_float_compare(a6, 495.) && _float_compare(a5, 1.)) return PARTICLE_Kplus;
   if (_float_compare(a6, 495.) && _float_compare(a5, -1.)) return PARTICLE_Kminus;
   if (_float_compare(a6, 940.) && _float_compare(a5, -1.)) return PARTICLE_AntiProton;
   if (_float_compare(a6, 940.) && _float_compare(a5, 6.)) return PARTICLE_AntiNeutron;
   if (_float_compare(a6, 140.) && _float_compare(a5, 0.)) return PARTICLE_PI0;
   if (_float_compare(a6, 140.) && _float_compare(a5, 1.)) return PARTICLE_PIplus;
   if (_float_compare(a6, 140.) && _float_compare(a5, -1.)) return PARTICLE_PIminus;
   if (_float_compare(a6, 940.) && _float_compare(a5, 1.)) return PARTICLE_Proton;
   if (_float_compare(a6, 940.) && _float_compare(a5, 0.)) return PARTICLE_Neutron;
...
float _hadgen_get_particle_weight(int type)
{
   ...
   case PARTICLE_AntiNeutron: return 939.57;
   case PARTICLE_Proton:
   case PARTICLE_AntiProton: return 938.27;
   case PARTICLE_PIminus:
   case PARTICLE_PIplus: return 139.57;
   case PARTICLE_PI0: return 134.98;
   case PARTICLE_Kminus:
   case PARTICLE_Kplus: return 493.67;
   case PARTICLE_K0Long:
   case PARTICLE_K0Short: return 497.65;
   case PARTICLE_Deuterium: return 1876.12;
   case PARTICLE_Tritium: return 2809.43;
   case PARTICLE_He3: return 2808.39;
   case PARTICLE_Alpha: return 3727.38;
   case PARTICLE_Electron:
   case PARTICLE_Positron: return 0.510998928;
   case PARTICLE_MUminus:
   case PARTICLE_MUplus: return 105, 6583715;
   case PARTICLE_ElNeutrino:
   case PARTICLE_ElAntiNeutrino: return 2.2 * 1E-6; // Upper bound is 2.2 eV
   case PARTICLE_MuNeutrino:
   case PARTICLE_MuAntiNeutrino: return 170 * 1E-3; // Upper bound is 170 keV
   case PARTICLE_Gamma:
   case PARTICLE_NONE:
   default: return .0;
   }
...
float _hadgen_get_particle_charge(int type)
{
   ...
   case PARTICLE_MUplus: return 1.;
   case PARTICLE_AntiProton:
   case PARTICLE_PIminus:
   case PARTICLE_Kminus:
   case PARTICLE_Electron:
   case PARTICLE_MUminus: return -1.;
   case PARTICLE_He3:
   case PARTICLE_Alpha: return 2.;
   case PARTICLE_Neutron:
   case PARTICLE_PI0:
...
// MEAN FREE LIFETIME
{
   switch (type) {
   case PARTICLE_Neutron:
   case PARTICLE_AntiNeutron: return 885.7;
   case PARTICLE_AntiProton:
   case PARTICLE_Proton: return 1e29; // Stable
   case PARTICLE_PIminus:
   case PARTICLE_PIplus: return 2.6e-8;
   case PARTICLE_PI0: return 8.4e-17;
   case PARTICLE_Kminus:
   case PARTICLE_Kplus: return 1.24e-8;
   case PARTICLE_K0Long: return 5.116e-8;   // A time for KL (long-living kaon)
   case PARTICLE_K0Short: return 8.953e-11; // A time for KS (short-living kaon)
   case PARTICLE_Gamma: return 1e30;        // Stable
   case PARTICLE_Deuterium: return 1e30;    // Stable
   case PARTICLE_Tritium: return 3.8852352e+8;
   case PARTICLE_He3: return 1e30;   // Stable?
   case PARTICLE_Alpha: return 1e30; // Stable
   case PARTICLE_Electron:
   case PARTICLE_Positron: return 1e26;
   case PARTICLE_MUminus:
   case PARTICLE_MUplus: return 2.19703e-6;
   case PARTICLE_ElNeutrino:
   case PARTICLE_ElAntiNeutrino: return 1e30; // Stable?
   case PARTICLE_MuNeutrino:
   case PARTICLE_MuAntiNeutrino: return 1e30; // Stable?
   case PARTICLE_NONE:
   default: return 0;
   }
}
...
void hadgen_iter_stp(struct HadgenParticle_t *p)
{
   if (iter_stp_counter < 5000) {
      if (sechar.SPT[iter_stp_counter][5] != 0.0) {
         ...
         float CT = sechar.SPT[iter_stp_counter][0], ST = sqrt(1.0 - CT * CT), SF = sechar.SPT[iter_stp_counter][1],
...
void hadgen_iter_snu(struct HadgenResidualNuclei_t *p)
{
   if (iter_snu_counter < 101) {
      if (sechar.SNU[iter_snu_counter][0] != 0) {
         ...
         p->Energy =
            sqrt(_float_SQR(p->Px) + _float_SQR(p->Py) + _float_SQR(p->Pz) + _float_SQR(0.940 * p->A)) - 0.940 * p->A;
