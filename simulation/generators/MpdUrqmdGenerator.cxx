...
MpdUrqmdGenerator::MpdUrqmdGenerator()
   : FairGenerator(), fInputFile(NULL), fParticleTable(), fFileName(NULL), fPhiMin(0.), fPhiMax(0.),
...
MpdUrqmdGenerator::MpdUrqmdGenerator(const char *fileName)
   : FairGenerator(), fInputFile(NULL), fParticleTable(), fFileName(fileName), fPhiMin(0.), fPhiMax(0.),
     fEventPlaneSet(kFALSE)
{
   //  fFileName = fileName;
...
Bool_t MpdUrqmdGenerator::ReadEvent(FairPrimaryGenerator *primGen)
{
   ...
   sscanf(read, "projectile:  (mass, char) %d %d target:  (mass, char) %d %d", &aProj, &zProj, &aTarg,
          &zTarg);                // line 2
   ...
   for (Int_t iline = 0; iline < ((urqmdVersion == 30400) ? 11 : 8); iline++) gzgets(fInputFile, read, 200);
   ...
   cout << "-I MpdUrqmdGenerator: Event " << evnr << ",  b = " << b << " fm,  multiplicity " << ntracks
        << ", ekin: " << ekin << endl;
   ...
   for (int itrack = 0; itrack < ntracks; itrack++) {
      ...
      if (fParticleTable.find(pid) == fParticleTable.end()) {
         cout << "-W MpdUrqmdGenerator: PID " << ityp << " charge " << ichg << " not found in table (" << pid << ")"
              << endl;
         continue;
      }
      ...
      // if (gCoordinateSystem == sysLaboratory) pz = gammaCM * (pz + betaCM * e); // 2022-01-04 Dead piece of code
