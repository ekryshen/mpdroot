...
{
   switch (whence) {
   case 0: return gzseek(file, pos, SEEK_CUR);
   case 1: return gzseek(file, pos, SEEK_SET);
   default: cerr << __FILE__ << __func__ << " wrong gzseek."; return -1;
   }
}
...
bool MpdGetNumEvents::GetQGSMEventHeader(char *ss, MpdLibZ *libz, Int_t &fQGSM_format_ID)
{
      ...
      while (!(tss.Contains("particles, b,bx,by"))) {
         if (tss.Contains("QGSM format ID")) {
            ...
            if (fQGSM_format_ID == 2) {
               ...
               for (int k = 0; k < 5; k++) libz->gets(ss, 250);
   ...
   case 0:
   case 1: libz->gets(tmp, 250); break;
   case 2:
   case 3: {
      libz->gets(ss, 250);
      libz->gets(ss, 250);
...
Int_t MpdGetNumEvents::GetNumQGSMEvents(const char *fileName)
{
   ...
   while (GetQGSMEventHeader(ss, libz, fQGSM_format_ID)) {
      ...
      for (Int_t itrack = 0; itrack < nTracks; itrack++) libz->gets(ss, 250);
   }
...
Int_t MpdGetNumEvents::GetNumURQMDEvents(const char *fileName)
{
   ...
   while (!libz->eof()) {
      ...
      for (int iline = 0; iline < ((urqmdVersion == 30400) ? 16 : 13); iline++) libz->gets(read, 200);

      libz->gets(read, 200);
      sscanf(read, "%d", &ntracks);
      libz->gets(read, 200);

      for (int itrack = 0; itrack < ntracks; itrack++) libz->gets(read, 200);
