...
URun::URun()
   : TNamed("run", "Run Header"), fGenerator(""), fComment(""), fDecayer(""), fAProj(0), fZProj(0), fPProj(0.),
     fATarg(0), fZTarg(0), fPTarg(0.), fBMin(0.), fBMax(0.), fBWeight(0), fPhiMin(0.), fPhiMax(0.), fSigma(0.),
     fNEvents(0)
{
}
//--------------------------------------------------------------------
...
URun::URun(const char *generator, const char *comment, Int_t aProj, Int_t zProj, Double_t pProj, Int_t aTarg,
   ...
   : TNamed("run", "Run Header"), fGenerator(generator), fComment(comment), fDecayer(""), fAProj(aProj), fZProj(zProj),
     fPProj(pProj), fATarg(aTarg), fZTarg(zTarg), fPTarg(pTarg), fBMin(bMin), fBMax(bMax), fBWeight(bWeight),
     fPhiMin(phiMin), fPhiMax(phiMax), fSigma(sigma), fNEvents(nEvents)
{
}
//--------------------------------------------------------------------
...
//--------------------------------------------------------------------
void URun::Print(Option_t *opt) const
{
