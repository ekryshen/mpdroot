...
MpdUnigenGenerator::MpdUnigenGenerator()
   : FairGenerator(), fEventNumber(0), fInFile(nullptr), fInTree(nullptr), fRun(nullptr), fEvent(nullptr), fParticle(nullptr),
...
Bool_t MpdUnigenGenerator::ReadEvent(FairPrimaryGenerator *primGen)
{
   ...
   if (fEventPlaneSet) {
      ...
      phi += dphi;
      cout << "-I MpdUnigenGenerator: PhiEP = " << phi*TMath::RadToDeg() << " (deg.)" << endl;
   }
   ...
   for (Int_t iTrack = 0; iTrack < nTracks; iTrack++) {
      ...
      if (fParticle->GetPdg() < 1e9) {
         ...
         if (GetIonLambdas(fParticle->GetPdg())) {
            ...
            std::cout << "-W- MpdUnigenGenerator: Replacing hypernucleus (PDG " << fParticle->GetPdg() << ") by PDG "
                      << ionPdg << std::endl;
         }
         ...
         if (GetIonCharge(ionPdg)) {
            ...
            std::cout << "-W- MpdUnigenGenerator: Neutral fragment (PDG " << ionPdg << ") is split into " << mass
                      << " neutrons." << std::endl;
         } // Neutral ion ?
      }    // ion ?
   }
      ...
      for (Int_t iParticle = 0; iParticle < fEvent->GetNpa(); iParticle++) {
         ...
         if (pdgCode > 1e9) { // ion
            ...
            if (fIonMap.find(ionName) == fIonMap.end()) { // new ion
               ...
               std::cout << "-I- MpdUnigenGenerator: Added new ion with PDG " << pdgCode << " (Charge " << chrg
                         << ", Mass " << mass << ",  nLambda " << nLambda << ") from event " << iEvent << " at index "
                         << iParticle << std::endl;
