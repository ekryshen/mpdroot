...
// -----   Default constructor   ------------------------------------------
MpdLAQGSMGenerator::MpdLAQGSMGenerator() : fInputFile(NULL), fGZInputFile(NULL) {}
// ------------------------------------------------------------------------
...
MpdLAQGSMGenerator::MpdLAQGSMGenerator(const char *fileName, const Bool_t use_collider_system, Int_t QGSM_format_ID,
   ...
   : FairGenerator(), fIonMap(), fInputFile(NULL), fGZInputFile(NULL)
{
   ...
   if ((!fInputFile) && (!fGZInputFile)) Fatal("MpdLAQGSMGenerator", "Cannot open input file.");
   ...
   cout << "-I- MpdLAQGSMGenerator: " << nIons << " ions registered." << endl;

   CloseInput();
   cout << "-I- MpdLAQGSMGenerator: Reopening input file " << fileName << endl;
   if (fGZ_input)
      fGZInputFile = gzopen(fFileName, "rb");
   else
      fInputFile = fopen(fFileName, "r");
...
void MpdLAQGSMGenerator::Init(const char *light_particles_filename)
{
   ...
   while (!(feof(fInputFile_light))) {
      ...
      i++;
      */
      i           = fscanf(fInputFile_light, "%d", &i_val);
      lt->pdg     = i_val;
      i           = fscanf(fInputFile_light, "%d", &i_val);
      lt->Z       = i_val;
      i           = fscanf(fInputFile_light, "%d", &i_val);
      ...
      i           = fscanf(fInputFile_light, "%d", &i_val);
      lt->A       = i_val;
      i           = fscanf(fInputFile_light, "%f", &a_val);
      lt->mass    = a_val;
      i           = fscanf(fInputFile_light, "%s", ss);
...
Bool_t MpdLAQGSMGenerator::general_fgets(char *ss, Int_t nn, FILE *p)
{
   ...
   if (fGZ_input) {
      ...
      if (!fgets(ss, nn, fInputFile)) cerr << __FILE__ << __func__ << " something went wrong in reading input.";
...
Bool_t MpdLAQGSMGenerator::GetEventHeader(char *ss)
{
   ...
   if (tss.Contains("QGSM")) { // 0
      ...
      while (!(tss.Contains("particles, b,bx,by"))) {
         if (tss.Contains("QGSM format ID")) {
            ...
            if (fQGSM_format_ID == 2) { // correction of incorrect format_ID in some data files
               ...
               if (strlen(ss) > 90) fQGSM_format_ID = 3;
         ...
         tss = ss;
         lines++;
         if ((fQGSM_format_ID >= 2) && (lines >= 4)) return kTRUE;
         if (lines > 25) {
            wrong_file = kTRUE;
            break;
         }
      ...
      if (fQGSM_format_ID >= 2) return kTRUE;
   ...
   case 0:
   case 1: general_fgets(tmp, 250, fInputFile); break;
   case 2:
   case 3:
      general_fgets(ss, 250, fInputFile);
      general_fgets(ss, 250, fInputFile);
...
Bool_t MpdLAQGSMGenerator::SkipEvents(Int_t count)
{
   ...
   for (Int_t ii = 0; ii < count; ii++) {
      ...
      if (!GetEventHeader(ss)) return kFALSE;
...
Bool_t MpdLAQGSMGenerator::ReadEvent(FairPrimaryGenerator *primGen)
{
   ...
   if (!fQGSM_format_ID)
      sscanf(&(ss[bpos]), "%g %g %g %g %g %g %g %g %g", &b, &bx, &by, &projA, &projZ, &projEex, &targA, &targZ,
             &targEex);
   else
      sscanf(&(ss[bpos]), "%g %g %g", &b, &bx, &by);

   cout << "-I- MpdLAQGSMGenerator::ReadEvent: Event " << eventId << ",  b = " << b << " fm, multiplicity " << nTracks
        << endl;
   ...
   for (Int_t itrack = 0; itrack < nTracks; itrack++) {
      ...
      if (fQGSM_format_ID < 3)
         ...
         if (fQGSM_format_ID == 4) {
            // cout << ss << endl;
            sscanf(ss, " %d %d %d %d %d %g %g %g %g %g %g", &iCharge, &iLeptonic, &iStrange, &iBarionic, &PDG, &px, &py,
                   &pz, &pz1, &pza1, &mass);
            // printf("!!!!!!!!!!!!!!!!!!!!!reading: %d %d %d %d %d %g %g %g %g %g %g\n",
            // iCharge,iLeptonic,iStrange,iBarionic, PDG, px, py, pz, pz1, pza1, mass); primGen->AddTrack(PDG, px, py,
            // pz, 0., 0., 0.);
      ...
      if (fQGSM_format_ID != 4) {
         ...
         if (FindParticle(iCharge, iStrange, iLeptonic, iBarionic, mass, PDG, ionName)) {
            ...
            if (!fPDG->GetParticle(PDG)) {
               cout << "-W- MpdLAQGSMGenerator::ReadEvent: Cannot find " << PDG << " " << ionName << " in database!"
                    << endl;
               continue;
            }
...
{
   if (fInputFile) {
      if (!fGZ_input) fclose(fInputFile);
      fInputFile = NULL;
   }
   if (fGZInputFile) {
      if (fGZ_input) gzclose(fGZInputFile);
      fGZInputFile = NULL;
   }
}
// ------------------------------------------------------------------------
...
Int_t MpdLAQGSMGenerator::RegisterIons(Int_t Max_Event_Number)
{
   ...
   // Define track variables to be read from file
   Int_t   iCharge = 0, iLeptonic = 0, iStrange = 0, iBarionic = 0, iCode = 0;
   Float_t px = 0., py = 0., pz = 0., pz1 = 0, pza1 = 0,
           //    Ekin = 0,
           //    Ekin1 = 0,
      mass = 0, excEnergy = 0;
   char ss[252], buf_ionName[30];

   Int_t A = 0, Z = 0, Q = 0;

   fIonMap.clear();

   if (!GetEventHeader(ss)) return 0;
   ...
   while ((!general_feof(fInputFile)) && ((!Max_Event_Number) || (eventId < Max_Event_Number))) {
      ...
      for (Int_t iTrack = 0; iTrack < nTracks; iTrack++) {
         ...
         if (iBarionic > 1) { // ion
            ...
            // if(iCharge>20)
            //	cout << "??????????????????1 " << iCharge << " " << iStrange << " " << iLeptonic << " " << iBarionic<< "
            //" <<  mass << endl;
            if (FindParticle(iCharge, iStrange, iLeptonic, iBarionic, mass, PDG, buf_ionName)) {

               // if(iCharge>20)
               //	cout << "??????????????????2 " << iCharge << " " << iStrange << " " << iLeptonic << " " << iBarionic<<
               //" " <<  mass << endl;
...
// ------------------------------------------------------------------------
Bool_t MpdLAQGSMGenerator::FindParticle(Int_t Z, Int_t strange, Int_t lepton, Int_t A, Float_t mass, Int_t &PDG,
                                        char name[11])
{
   Int_t n_mass, result = 0;

   if (mass < 6) { // use table

      Int_t l_min = -1, // line number first
...
Bool_t MpdLAQGSMGenerator::CreateNucleus(Int_t Z, Float_t mass, Int_t pdgCode, char pdgName[11])
{
   ...
   if (Z)
      p = fPDG->AddParticle(pdgName, pdgName, mass, kFALSE, width, Z * 3, "nucleus", pdgCode, 0, kPTHadron);
   else
      p = fPDG->AddParticle(pdgName, pdgName, mass, kFALSE, 6.58211889e-25 / 2.6e-10, 0,
