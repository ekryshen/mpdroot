...
MpdPHSDGenerator::MpdPHSDGenerator(const char *filename = "phsd.dat.gz")
{
   ...
   if (!fgzFile) {
      printf("-E- MpdPHSDGenerator: can not open file: %s\n", filename);
      exit(1);
   }
...
Bool_t MpdPHSDGenerator::ReadEvent(FairPrimaryGenerator *primGen)
{
   ...
   for (Int_t i = 1; i <= fntr; i++) {
      ...
      if (res != 4) {
         printf("-E- MpdPHSDGenerator: selftest error in track, scan %d of 4\n", res);
         exit(1);
      }
      if (ipdg == 0) {
         printf("-W- MpdPHSDGenerator: particle with pdg=0\n");
         continue;
      }
...
Bool_t MpdPHSDGenerator::ReadHeader()
{
   ...
   if (res != 2) {
      printf("-E- MpdPHSDGenerator: selftest error in header, scan %d of 2\n", res);
      exit(1);
   }
   gzgets(fgzFile, fbuffer, 256); // 2nd line
   if (gzeof(fgzFile)) {
      printf("-E- MpdPHSDGenerator: unexpected end of file\n");
      exit(1);
   }
   return kTRUE;
};
