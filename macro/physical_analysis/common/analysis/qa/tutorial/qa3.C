...
void qa3(TString inFile = "$VMCWORKDIR/macro/mpd/mpddst.MiniDst.root", TString outFile = "qa.root") {
  ...
  NicaEventVirtualCut virtualCut;
  virtualCut.SetCollectionID(1);

  trackTask->AddCut(central);
  trackTask->AddCut(semicentral);
  trackTask->AddCut(virtualCut);
  ...
  trackTask->AddCut(pion, "{0}");  // you can specify collection ID by {}
  trackTask->AddCut(proton, "{1}");
...
NicaQAPlot GetTrackQA() {
  ...
  plot.AddTH2("KinematicsMC", ETrack::kEta, ETrack::kPt, 100, -2, 2, 100, 0, 4);
  return plot;
}
