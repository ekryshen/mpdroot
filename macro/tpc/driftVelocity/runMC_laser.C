...
void runMC_laser(EGenerators generator = EGenerators::BOX, EVMCType vmc = EVMCType::GEANT3, Int_t nStartSeed = 0,
           TString inFile = "auau.04gev.0_3fm.10k.f14.gz", TString outFile = "evetest_laser_tpc.root", Int_t nStartEvent = 0,
           Int_t nEvents = 20, Bool_t flag_store_FairRadLenPoint = kFALSE, Int_t FieldSwitcher = 0)
{
   TStopwatch timer;
   timer.Start();
   gDebug = 0;
   ...
   // laser (muon) grid generation
   const Double_t Fieldcage_shift_deg = 8.;
   const Double_t R =
      123 -
      3. / TMath::Cos(15. *
                      TMath::DegToRad()); // Membrane_outer_holder_R_edge -
                                          // Fieldcage_Pin_R[1] /
   ...
   for (Int_t j = -4; j < 4; ++j) {
      for (Int_t i = 0; i < 4; ++i) {
         for (Int_t a = -1; a <= 1; ++a) {
            ...
            Double_t phi = 360. - 90. * (i + 1) - Fieldcage_shift_deg + spread_ang * a;
            Double_t theta = 90.;
            FairBoxGenerator *laserGen = new FairBoxGenerator(13, 1);
            laserGen->SetPRange(55.5, 55.5);   // GeV/c, setPRange vs setPtRange
            laserGen->SetPhiRange(phi, phi); // Azimuth angle range [degree]
            laserGen->SetThetaRange(theta, theta); // Polar angle in lab system range [degree]
            laserGen->SetXYZ(x, y, j < 0 ? j * pos_offset : (j + 1) * pos_offset);
            primGen->AddGenerator(laserGen);
         }
      }
   }
   ...
   {
      gRandom->SetSeed(0);
      FairBoxGenerator *boxGen = new FairBoxGenerator(13, 400); // 13 = muon; 1 = multipl.
   ...
   case EGenerators::NONE:
   {
      std::cout << "Warning. No generator type was setted.\n";
      break;
   }
   default:
   {
