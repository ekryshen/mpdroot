...
void EmcGeom::BuildSensVolume()
{
   ...
   for (int i = 1; i < (((base - box_s_d) / tan(q)) / box_sum); i++) {
      if (ad < (base - asc - apb)) {
         ...
         emc1_cl_sc->Fill_TRAP((box_s_d + x + y) / 2, ((box_s_d + y + x + p) - (box_s_d + x + y) / 2), box_sc_h / 2.,
                               base / 2., 1);
         emc1_cl_sc->SetSegment(1);
         emc1_cl_sc->SetMedia("FscScint");
         emc1_cl_sc->SetPosition((box_s_d + x + y) / 2 - box_s_d / 2, -emc1_box1_h / 2. + box_pb_h + x1 + box_sc_h / 2.,
                                 z);
