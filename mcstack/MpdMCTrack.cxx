...
MpdMCTrack::MpdMCTrack(Int_t pdgCode,
                       Int_t motherId,
                       Double_t px,
                       Double_t py,
                       Double_t pz,
...
MpdMCTrack::MpdMCTrack(const MpdMCTrack& track)  // {
  ...
  for (Int_t i = 0; i < 3; ++i)
    fPolar[i] = track.fPolar[i];
}
// -------------------------------------------------------------------------


// -----   Constructor from TParticle   ------------------------------------
MpdMCTrack::MpdMCTrack(TParticle* part) : TObject() {
...
void MpdMCTrack::Print(Int_t trackId) const {
  cout << "Track " << trackId << ", mother : " << fMotherId << ", Type " << fPdgCode << ", momentum (" << fPx << ", " << fPy
       << ", " << fPz << ") GeV" << endl;
  cout << "       STS " << GetNPoints(kSTS) << ", TPC " << GetNPoints(kTPC) << ", TOF " << GetNPoints(kTOF) << ", ETOF "
       << GetNPoints(kETOF) << ", FFD " << GetNPoints(kFFD) << ", ECT " << GetNPoints(kECT) << ", ECAL " << GetNPoints(kECAL)
       << ", NDET " << GetNPoints(kNDET) << ", CPC " << GetNPoints(kCPC) << ", BBC " << GetNPoints(kBBC) << ", ZDC "
       << GetNPoints(kZDC) << ", FSA " << GetNPoints(kFSA) << ", BMD " << GetNPoints(kBMD) << ", MCORD " << GetNPoints(kMCORD)
       << endl;
}
// -------------------------------------------------------------------------
