...
void			MpdBayesPriors::_setDefaultPriors() // set default value = 1. for all bins
{
	...
	for(size_t pid = 0; pid < fNpdf; pid++)
	{
		for(size_t bin = 1, nBins = fPriorOrigin->GetXaxis()->GetNbins(); bin <= nBins; bin++)	fPriors[pid]->SetBinContent(bin, 1.);
	}
}
//------------------------------------------------------------------------------------------------------------------------
...
{ 
	h1->SetDirectory(nullptr); fList.Add(h1);
}
//------------------------------------------------------------------------------------------------------------------------
...
void		PidProbabilitiesMatrix::Update(double* prob_H_S, double Pt, Int_t pdgcode)
{
	...
	{	
		...
		{
			double maxProb = 0.; size_t maxPid = -1;
			for(size_t pid = 0; pid < fNpdf; pid++)
			{
...
void		PidProbabilitiesMatrix::FinalizeMatrices()
{
	...
	{		
		...
		// to species i that are wrongly identified as j (Ni identified as j ), divided by the total number of identified j particles (Ameas j)
		// Contamination_ji = N_i identified as j  / A_meas_j

		for(size_t pid2 = 0; pid2 < fNpdf; pid2++)
		{
			if(pid == pid2)continue;
			h1 = CreatePriorClone(TString::Format("TofPidMatrix_Contamination_%s_by_%s", 
						MpdBayesPriors::GetSpeciesName(pid), MpdBayesPriors::GetSpeciesName(pid2)));
...
{ 
	...
	// Set default name
	if(fPidThres < 0.) 	fName = "max. probability method.";
	else			fName.Form("fixed threshold(%f) method.", fPidThres); 
}
//------------------------------------------------------------------------------------------------------------------------
...
:  FairTask(taskName, verbose), fUseMCData(useMCdata), fDoPDFs(false), fPDFflnm(flnm), fQAflnm(QAflnm)
{
	...
	{
		double thresh = (0 == i) ? -1. :  i * 0.1;  // default threshoulds: -1., 0.1, 0.2, ... ,0.9 (Use MpdTofBayesPid::SetProbMatrixThresh() to change.)
	...
	/////////////// update ALREADY	PRIOR	fNextPrior->Yields2Priors(fYield);
		...
		{		
			for(size_t k = 0; k < fNmatrix; k++)
			{
				...
				TString flnm;flnm.Form("PidMatrices_%s_%zd.root", fEffMatrix[k]->GetFlnm(), k);
...
//------------------------------------------------------------------------------------------------------------------------
// increase square rank until the all prob. sums becomes not zero or will reach the limit; return - notZeroExist and rank
...
bool 			MpdTofBayesPid::_getProbs(Int_t rank, double mass2, double P, double dedx, double *data) 
{
	...
	Int_t Xfirst  = (Xbin-rank >= 1) ? Xbin-rank : 1,  Xlast  = (Xbin+rank <= XNbins) ? Xbin+rank : XNbins; // clamp(Xbin+-rank, 1, XNbins);
	Int_t Yfirst  = (Ybin-rank >= 1) ? Ybin-rank : 1,  Ylast  = (Ybin+rank <= YNbins) ? Ybin+rank : YNbins; // clamp(Ybin+-rank, 1, YNbins);	
	Int_t Zfirst  = (Zbin-rank >= 1) ? Zbin-rank : 1,  Zlast  = (Zbin+rank <= ZNbins) ? Zbin+rank : ZNbins; // clamp(Zbin+-rank, 1, ZNbins);
...
{
		if(2212 == pdg || -2212 == pdg)	return 0;
	else 	if(211 == pdg || -211 == pdg) 	return 1;
	else 	if(321 == pdg || -321 == pdg)	return 2;
	else 	if(11 == pdg || -11 == pdg) 	return 3;
...
static const TString pdfPrefix("MpdTOFpid_Pdf_");
	...
	if(fDoPDFs) // create new PDFs
	{
		...
		for(size_t pid = 0; pid < fNpdf; pid++)
		{
			TString name = pdfPrefix; name += MpdBayesPriors::GetSpeciesName(pid);
			cout<<"\n "<<name;
			
//			hPdf[pid] = new TH3D(name.Data(), ";mass^{2}, GeV^{2};P, GeV/c;dE/dx", 500, -0.5, 3., 500, 0., 10., 100, 0., 10.);
#ifdef _PDFbyBeta
			hPdf[pid] = new TH3D(name.Data(), ";mass^{2}, GeV^{2};#beta;dE/dx", 500, -1.5, 6., 500, 0., 1.1, 100, 0., 10.); 
#else
			hPdf[pid] = new TH3D(name.Data(), ";mass^{2}, GeV^{2};P, GeV/c;dE/dx", 500, -1.5, 6., 500, 0., 10., 100, 0., 10.);
#endif
	...
	else // load PDFs from file
	{
		...
		{
			TString name = pdfPrefix; name += MpdBayesPriors::GetSpeciesName(n);
