   ...
   : FairTask(name, verbose), fUseMCData(useMCdata), fFlnm(flnm)
{
   ...
   if (fDoTest) {
      ...
      Add(hTeveN =
             new TH2D("MpdTofT0_hTeveN", ";matching mupltiplicity; Tevent, ns", 1000, -0.5, 999.5, 1000, -5., 5.));
      Add(hTcalcN = new TH2D("MpdTofT0_hTcalcN", ";matching mupltiplicity; calc. time, s", 1000, -0.5, 999.5, 1000, 0.,
                             1000.));
      Add(hTofmultB = new TH2D("MpdTofT0_hTofmultB", ";impact parameter, fm; matching mupltiplicity", 1000, 0., 20.,
                               1000, -0.5, 999.5));
      ...
      Add(effT0_100 =
             new TEfficiency("MpdTofT0_effT0_100", "T0 < 100 ps;impact parameter, fm;efficiency", 20, 0., 20.));
   }
}
//------------------------------------------------------------------------------------------------------------------------
MpdTofT0::~MpdTofT0()
...
InitStatus MpdTofT0::Init()
{
   ...
   FairRootManager::Instance()->Register("TOFTevent", "Tof", fResult,
                                         kFALSE); // if kTRUE, branch will be saved to the tree file
...
void MpdTofT0::Exec(Option_t *option)
{
   ...
   if (fUseMCData) {
      ...
      //		if(10. < MCdata.Zvert || MCdata.Zvert < -10.) return; 						// pass vertex Z inside[-10,10] cm
      //<<<---------CUT!!!
   }
   ...
   for (auto it = mmMatchData.begin(), itEnd = mmMatchData.end(); it != itEnd;
        it++, nMatchings++) // cycle by of matchings, decrement by Pt
   {
