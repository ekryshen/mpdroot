...
UniDbParameter *UniDbParameter::CreateParameter(TString parameter_name, int parameter_type, bool is_array)
{
   ...
   TString        sql  = TString::Format("insert into parameter_(parameter_name, parameter_type, is_array) "
                                                 "values ($1, $2, $3)");
...
UniDbParameter *UniDbParameter::GetParameter(int parameter_id)
{
   ...
   TString        sql  = TString::Format("select parameter_id, parameter_name, parameter_type, is_array "
                                                 "from parameter_ "
...
int UniDbParameter::DeleteParameter(int parameter_id)
{
   ...
   TString        sql  = TString::Format("delete from parameter_ "
...
int UniDbParameter::SetParameterName(TString parameter_name)
{
   ...
   TString        sql  = TString::Format("update parameter_ "
...
void UniDbParameter::Print()
{
   ...
   cout << ". parameter_id: " << i_parameter_id << ". parameter_name: " << str_parameter_name
        << ". parameter_type: " << i_parameter_type << ". is_array: " << b_is_array << endl;

   return;
}
/* END OF GENERATED CLASS PART (SHOULD NOT BE CHANGED MANUALLY) */

bool UniDbParameter::CheckAndGetParameterID(TSQLServer *uni_db, TString parameter_name,
                                            enumValueType enum_parameter_type, int &parameter_id)
{
   // get parameter object from 'parameter_' table
   TString        sql  = TString::Format("select parameter_id, parameter_name, parameter_type "
                                                 "from parameter_ "
                                                 "where lower(parameter_name) = lower('%s')",
                                         parameter_name.Data());
   TSQLStatement *stmt = uni_db->Statement(sql);
   ...
   if (parameter_type != enum_parameter_type) {
      cout << "ERROR: '" << parameter_name << "' parameter has not the same type (type = " << parameter_type << ", but "
           << enum_parameter_type << " used)" << endl;
      return false;
   }
