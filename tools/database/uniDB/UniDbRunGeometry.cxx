...
UniDbRunGeometry::UniDbRunGeometry(UniConnection *connUniDb, int geometry_id, unsigned char *root_geometry,
...
UniDbRunGeometry::~UniDbRunGeometry()
{
   ...
   if (blob_root_geometry) delete[] blob_root_geometry;
}
...
UniDbRunGeometry *UniDbRunGeometry::CreateRunGeometry(unsigned char *root_geometry, Long_t size_root_geometry)
{
   ...
   if (gSystem->AccessPathName(patch_check)) {
      cout << "ERROR: TPgSQLStatement (CERN ROOT) was not patched to write binary data into the Unified Database"
           << endl;
      return 0x00;
   }
   ...
   TString        sql  = TString::Format("insert into run_geometry(root_geometry) "
                                                 "values ($1)");
...
UniDbRunGeometry *UniDbRunGeometry::GetRunGeometry(int geometry_id)
{
   ...
   TString        sql  = TString::Format("select geometry_id, root_geometry "
                                                 "from run_geometry "
                                                 "where geometry_id = %d",
                                         geometry_id);
   TSQLStatement *stmt = uni_db->Statement(sql);
...
int UniDbRunGeometry::DeleteRunGeometry(int geometry_id)
{
   ...
   TString        sql  = TString::Format("delete from run_geometry "
                                                 "where geometry_id = $1");
   TSQLStatement *stmt = uni_db->Statement(sql);
...
int UniDbRunGeometry::SetRootGeometry(unsigned char *root_geometry, Long_t size_root_geometry)
{
   ...
   TString        sql  = TString::Format("update run_geometry "
                                                 "set root_geometry = $1 "
                                                 "where geometry_id = $2");
   TSQLStatement *stmt = uni_db->Statement(sql);
...
void UniDbRunGeometry::Print()
{
   ...
   cout << ". geometry_id: " << i_geometry_id << ". root_geometry: " << (void *)blob_root_geometry
...
// -----   Creating new run geometry in the database  ---------------------------
UniDbRunGeometry *UniDbRunGeometry::CreateRunGeometry(int geometry_id, unsigned char *root_geometry,
                                                      Long_t size_root_geometry)
{
   TString patch_check("$SIMPATH/pgsql_patched");
   gSystem->ExpandPathName(patch_check);
   ...
   TString        sql  = TString::Format("insert into run_geometry(geometry_id, root_geometry) "
                                                 "values ($1,$2)");
   TSQLStatement *stmt = uni_db->Statement(sql);
...
TObjArray *UniDbRunGeometry::GetAll()
{
   ...
   while (stmt->NextResultRow()) {
      ...
      arrayResult->Add(
         (TObject *)new UniDbRunGeometry(connRun, tmp_geometry_id, tmp_root_geometry, tmp_sz_root_geometry));
   }
