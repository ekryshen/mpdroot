...
private:
   ...
   // Constructor
   UniDbRun(UniConnection *connUniDb, int period_number, int run_number, TString file_path, TString beam_particle,
            TString *target_particle, double *energy, TDatime start_datetime, TDatime *end_datetime, int *event_count,
            double *field_voltage, int *file_size, int *geometry_id, TString *file_md5);
...
public:
   ...
   static UniDbRun *CreateRun(int period_number, int run_number, TString file_path, TString beam_particle,
   ...
   {
      if (str_target_particle == NULL)
         return NULL;
      else
         return new TString(*str_target_particle);
   }
   ...
   {
      if (ts_end_datetime == NULL)
         return NULL;
      else
         return new TDatime(*ts_end_datetime);
   }
   /// get event count of the current run
   int *GetEventCount()
   {
      if (i_event_count == NULL)
         return NULL;
      else
         return new int(*i_event_count);
   }
   /// get field voltage of the current run
   double *GetFieldVoltage()
   {
      if (d_field_voltage == NULL)
         return NULL;
      else
         return new double(*d_field_voltage);
   }
   ...
   int *GetFileSize()
   {
      if (i_file_size == NULL)
         ...
         return new int(*i_file_size);
   }
   /// get geometry id of the current run
   int *GetGeometryId()
   {
      if (i_geometry_id == NULL)
         return NULL;
      else
         return new int(*i_geometry_id);
   }
   /// get file md5 of the current run
   TString *GetFileMd5()
   {
      if (chr_file_md5 == NULL)
         return NULL;
      else
         return new TString(*chr_file_md5);
   }
   ...
   static int SetRootGeometry(int start_period, int start_run, int end_period, int end_run,
   ...
   static int GetRootGeometry(int period_number, int run_number, unsigned char *&root_geometry,
                              Long_t &size_root_geometry);
   /// write geometry file for runs from start_run_number to end_run_number to the database
   static int WriteGeometryFile(int start_period, int start_run, int end_period, int end_run,
