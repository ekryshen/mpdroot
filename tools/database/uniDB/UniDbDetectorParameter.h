...
private:
   ...
   // Constructor
   UniDbDetectorParameter(UniConnection *connUniDb, int value_id, TString detector_name, int parameter_id,
                          int start_period, int start_run, int end_period, int end_run, int value_key,
...
public:
   ...
   static UniDbDetectorParameter *CreateDetectorParameter(TString detector_name, int parameter_id, int start_period,
   ...
   {
      unsigned char *tmp_parameter_value = new unsigned char[sz_parameter_value];
      memcpy(tmp_parameter_value, blob_parameter_value, sz_parameter_value);
      return tmp_parameter_value;
   }
   ...
   static UniDbDetectorParameter *CreateDetectorParameter(TString detector_name, TString parameter_name,
                                                          int start_period, int start_run, int end_period, int end_run,
   ...
   static int DeleteDetectorParameter(TString detector_name, TString parameter_name, int start_period, int start_run,
                                      int end_period, int end_run, int value_key = 0);
   ...
   /// write a whole file with parameter value (with given detector and parameter names) to the Database from start run
   /// to end one
   static int WriteFile(const char *detector_name, const char *parameter_name, int start_period, int start_run,
                        int end_period, int end_run, const char *file_path);
   /// read a whole file with parameter value (with given detector and parameter names) from the Database for the
   /// selected run
   static int ReadFile(const char *detector_name, const char *parameter_name, int period_number, int run_number,
                       const char *file_path);

   /// get parameters' values corresponding to the specified single condition and set owner for search_condition to
   /// kTRUE
   static TObjArray *Search(UniSearchCondition &search_condition);
   /// get parameters' values corresponding to the specified (vector) conditions and set owner for search_condition to
   /// kTRUE
   ...
   // static int ParseTxt(TString text_file, TString detector_name, TString parameter_name, int start_period, int
   // start_run, int end_period, int end_run);
...
protected:
   ...
   static UniDbDetectorParameter *CreateDetectorParameter(TString detector_name, TString parameter_name,
                                                          int start_period, int start_run, int end_period, int end_run,
                                                          unsigned char *parameter_value, Long_t size_parameter_value,
                                                          enumValueType enum_parameter_type, int value_key = 0);
