...
UniDbRun::UniDbRun(UniConnection *connUniDb, int period_number, int run_number, TString file_path,
                   TString beam_particle, TString *target_particle, double *energy, TDatime start_datetime,
                   TDatime *end_datetime, int *event_count, double *field_voltage, int *file_size, int *geometry_id,
                   TString *file_md5)
{
...
UniDbRun::~UniDbRun()
{
   ...
   if (str_target_particle) delete str_target_particle;
   if (d_energy) delete d_energy;
   if (ts_end_datetime) delete ts_end_datetime;
   if (i_event_count) delete i_event_count;
   if (d_field_voltage) delete d_field_voltage;
   if (i_file_size) delete i_file_size;
   if (i_geometry_id) delete i_geometry_id;
   if (chr_file_md5) delete chr_file_md5;
}

// -----   Creating new run in the database  ---------------------------
UniDbRun *UniDbRun::CreateRun(int period_number, int run_number, TString file_path, TString beam_particle,
                              TString *target_particle, double *energy, TDatime start_datetime, TDatime *end_datetime,
                              int *event_count, double *field_voltage, int *file_size, int *geometry_id,
                              TString *file_md5)
{
   ...
   TString sql =
      TString::Format("insert into run_(period_number, run_number, file_path, beam_particle, target_particle, energy, "
                      "start_datetime, end_datetime, event_count, field_voltage, file_size, geometry_id, file_md5) "
   ...
   return new UniDbRun(connUniDb, tmp_period_number, tmp_run_number, tmp_file_path, tmp_beam_particle,
                       tmp_target_particle, tmp_energy, tmp_start_datetime, tmp_end_datetime, tmp_event_count,
                       tmp_field_voltage, tmp_file_size, tmp_geometry_id, tmp_file_md5);
}
...
UniDbRun *UniDbRun::GetRun(int period_number, int run_number)
{
   ...
   TString sql =
      TString::Format("select period_number, run_number, file_path, beam_particle, target_particle, energy, "
                      "start_datetime, end_datetime, event_count, field_voltage, file_size, geometry_id, file_md5 "
                      "from run_ "
...
int UniDbRun::DeleteRun(int period_number, int run_number)
{
   ...
   TString        sql  = TString::Format("delete from run_ "
...
int UniDbRun::SetPeriodNumber(int period_number)
{
   ...
   TString        sql  = TString::Format("update run_ "
                                                 "set period_number = $1 "
...
{
   cout << "Table 'run_'";
   cout << ". period_number: " << i_period_number << ". run_number: " << i_run_number
        << ". file_path: " << str_file_path << ". beam_particle: " << str_beam_particle
        << ". target_particle: " << (str_target_particle == NULL ? "NULL" : *str_target_particle)
        << ". energy: " << (d_energy == NULL ? "NULL" : TString::Format("%f", *d_energy))
        << ". start_datetime: " << ts_start_datetime.AsSQLString()
        << ". end_datetime: " << (ts_end_datetime == NULL ? "NULL" : (*ts_end_datetime).AsSQLString())
        << ". event_count: " << (i_event_count == NULL ? "NULL" : TString::Format("%d", *i_event_count))
        << ". field_voltage: " << (d_field_voltage == NULL ? "NULL" : TString::Format("%f", *d_field_voltage))
        << ". file_size: " << (i_file_size == NULL ? "NULL" : TString::Format("%d", *i_file_size))
        << ". geometry_id: " << (i_geometry_id == NULL ? "NULL" : TString::Format("%d", *i_geometry_id))
        << ". file_md5: " << (chr_file_md5 == NULL ? "NULL" : *chr_file_md5) << endl;
...
int UniDbRun::GetRunNumbers(int start_period, int start_run, int end_period, int end_run, UniqueRunNumber *&run_numbers)
{
   ...
   TString        sql  = TString::Format("select period_number, run_number "
                                                 "from run_ "
                                                 "where (not (((%d < period_number) or ((%d = period_number) and (%d < run_number))) "
                                                 "or ((%d > period_number) or ((%d = period_number) and (%d > run_number))))) "
                                                 "order by period_number, run_number",
                                         end_period, end_period, end_run, start_period, start_period, start_run);
   TSQLStatement *stmt = uni_db->Statement(sql);
...
int UniDbRun::SetRootGeometry(int start_period, int start_run, int end_period, int end_run,
                              unsigned char *root_geometry, Long_t size_root_geometry)
{
   ...
   if (run_count < 0) return -10 - run_count;

   for (int i = 0; i < run_count; i++) {
      cout << "Setting geometry for run_period:run   " << pUniqueRuns[i].period_number << ":"
           << pUniqueRuns[i].run_number << "..." << endl;

      UniDbRun *pCurRun = UniDbRun::GetRun(pUniqueRuns[i].period_number, pUniqueRuns[i].run_number);
      if (pCurRun == NULL) {
         cout << "ERROR: getting of run " << pUniqueRuns[i].period_number << ":" << pUniqueRuns[i].run_number
...
int UniDbRun::GetRootGeometry(int period_number, int run_number, unsigned char *&root_geometry,
...
TObjArray *UniDbRun::Search(TObjArray &search_conditions)
{
   ...
   TString sql =
      TString::Format("select run_number, period_number, file_path, beam_particle, target_particle, energy, "
                      "start_datetime, end_datetime, event_count, field_voltage, file_size, geometry_id, file_md5 "
                      "from run_");
   ...
   while (stmt->NextResultRow()) {
      ...
      arrayResult->Add((TObject *)new UniDbRun(connRun, tmp_period_number, tmp_run_number, tmp_file_path,
                                               tmp_beam_particle, tmp_target_particle, tmp_energy, tmp_start_datetime,
                                               tmp_end_datetime, tmp_event_count, tmp_field_voltage, tmp_file_size,
                                               tmp_geometry_id, tmp_file_md5));
   }
...
UniqueRunNumber *UniDbRun::FindPreviousRun(int run_period, int run_number)
{
   ...
   TString        sql  = TString::Format("select period_number, run_number "
                                                 ...
                                                 "limit 1",
                                         run_period, run_period, run_number);
   TSQLStatement *stmt = uni_db->Statement(sql);
