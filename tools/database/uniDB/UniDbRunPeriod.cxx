...
UniDbRunPeriod::UniDbRunPeriod(UniConnection *connUniDb, int period_number, TDatime start_datetime,
...
UniDbRunPeriod *UniDbRunPeriod::CreateRunPeriod(int period_number, TDatime start_datetime, TDatime *end_datetime)
{
   ...
   TString        sql  = TString::Format("insert into run_period(period_number, start_datetime, end_datetime) "
                                                 "values ($1, $2, $3)");
...
UniDbRunPeriod *UniDbRunPeriod::GetRunPeriod(int period_number)
{
   ...
   TString        sql  = TString::Format("select period_number, start_datetime, end_datetime "
                                                 "from run_period "
...
int UniDbRunPeriod::DeleteRunPeriod(int period_number)
{
   ...
   TString        sql  = TString::Format("delete from run_period "
...
int UniDbRunPeriod::SetPeriodNumber(int period_number)
{
   ...
   TString        sql  = TString::Format("update run_period "
                                                 "set period_number = $1 "
...
void UniDbRunPeriod::Print()
{
   ...
   cout << ". period_number: " << i_period_number << ". start_datetime: " << ts_start_datetime.AsSQLString()
        << ". end_datetime: " << (ts_end_datetime == NULL ? "NULL" : (*ts_end_datetime).AsSQLString()) << endl;
...
int UniDbRunPeriod::GetRunNumbers(int period_number, UniqueRunNumber *&run_numbers)
{
   ...
   TString        sql  = TString::Format("select run_number "
                                                 "from run_ "
                                                 "where period_number = %d "
                                                 "order by run_number",
                                         period_number);
   ...
   vector<int> vecRuns;
   while (stmt->NextResultRow()) vecRuns.push_back(stmt->GetInt(0));
...
int UniDbRunPeriod::GetFirstRunNumber(int period_number)
{
   ...
   TString        sql  = TString::Format("select min(run_number) "
                                                 "from run_ "
                                                 "where period_number = %d ",
                                         period_number);
   TSQLStatement *stmt = uni_db->Statement(sql);
   ...
   if (stmt->NextResultRow()) min_number = stmt->GetInt(0);
...
int UniDbRunPeriod::GetLastRunNumber(int period_number)
{
   ...
   TString        sql  = TString::Format("select max(run_number) "
                                                 "from run_ "
                                                 "where period_number = %d ",
                                         period_number);
   TSQLStatement *stmt = uni_db->Statement(sql);
   ...
   if (stmt->NextResultRow()) max_number = stmt->GetInt(0);
