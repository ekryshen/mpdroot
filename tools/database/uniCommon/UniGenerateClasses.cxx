...
UniGenerateClasses::~UniGenerateClasses()
{
   if (arrTableJoin) delete arrTableJoin;
}
...
int UniGenerateClasses::GenerateClasses(UniConnectionType connection_type, TString class_prefix, bool isOnlyUpdate)
{
   ...
   enumDBMS curDBMS;
   if (strcmp(uni_db->GetDBMS(), "MySQL") == 0)
      curDBMS = MySQL;
   else if (strcmp(uni_db->GetDBMS(), "PgSQL") == 0)
      curDBMS = PgSQL;
   else {
   ...
   if (res_code < 0) {
      cout << "CRITICAL ERROR: creating of the directory '" << strClassDir << "' was failed, error code: " << res_code
           << endl;
      return -11;
   }
   ...
   while (obj = next()) {
      ...
      if ((curDBMS == PgSQL) && (strTableName.BeginsWith("pg_") || strTableName.BeginsWith("sql_"))) continue;
      ...
      if (curDBMS == MySQL) {
         sql = TString::Format("SELECT ordinal_position, column_name, data_type, (is_nullable = 'YES') AS is_nullable, "
                               "(extra = 'auto_increment') AS is_identity, (column_key = 'PRI') AS is_primary, "
         ...
         sql = TString::Format(
            "SELECT DISTINCT a.attnum as ordinal_position, a.attname as column_name, format_type(a.atttypid, "
            "a.atttypmod) as data_type, "
            "a.attnotnull as is_nullable, pg_get_expr(def.adbin, def.adrelid) as is_default, coalesce(i.indisprimary, "
            "false) as is_primary, coalesce(i.indisunique, false) as is_unique, atttypmod as type_parameter "
      ...
      while (row = res->Next()) {
         ...
         case TSQLServer::kSQL_TIMESTAMP: {
            // cout<<"TimeStamp. Size: "<<pColumnInfo->GetSize()<<". Length: "<<pColumnInfo->GetSize()<<". Scale:
            // "<<pColumnInfo->GetScale()<<endl;
            ...
            if (strDataType == "bit") {
               ...
               if (strDataType == "datetime") {
                  ...
                  if (strDataType.BeginsWith("character")) {
                     ...
                     cout << "ERROR: no corresponding column type: " << row->GetField(2)
      ...
      if (isJoin) {
         ...
         while (cur_col = (structColumnInfo *)nextCol()) {
            hFile << (TString::Format("\t/// Table: %s - column %s (read-only)\n",
                                      curTableJoin->strJoinTableName.Data(), cur_col->strColumnNameSpace.Data()))
                        .Data();
      ...
      if (isJoin) {
         ...
         while (cur_col = (structColumnInfo *)nextCol())
            hFile
               << (TString::Format(", %s %s", cur_col->strVariableType.Data(), cur_col->strColumnName.Data())).Data();
      } // for join table
      hFile << ");\n";
      ...
      if (isOnlyUpdate) {
         ...
         while (getline(oldFile, cur_line)) {
            string trim_line = trim(cur_line);
            if (trim_line.substr(0, 27) == "/* END OF PRIVATE GENERATED") break;
         }
      ...
      for (vector<structColumnInfo *>::iterator it = vecColumns.begin(); it != vecColumns.end(); ++it) {
         structColumnInfo *cur_col = *it;
         if (cur_col->isIdentity) continue;
      ...
      for (vector<structColumnInfo *>::iterator it = vecColumns.begin(); it != vecColumns.end(); ++it) {
         structColumnInfo *cur_col = *it;
         if (cur_col->isUnique) is_flag = true;
         if (!cur_col->isPrimary) continue;
      ...
      if (is_flag) {
         for (vector<structColumnInfo *>::iterator it = vecColumns.begin(); it != vecColumns.end(); ++it) {
            structColumnInfo *cur_col = *it;
            if (!cur_col->isUnique) continue;

            hFile << (TString::Format("\t/// get %s from the database\n", strTableNameSpace.Data())).Data();
            hFile << (TString::Format("\tstatic %s* Get%s(%s %s);\n", strClassName.Data(), strShortTableName.Data(),
      ...
      for (vector<structColumnInfo *>::iterator it = vecColumns.begin(); it != vecColumns.end(); ++it) {
         ...
         hFile << (TString::Format("\t/// get %s of the current %s\n", cur_col->strColumnNameSpace.Data(),
                                   strTableNameSpace.Data()))
                     .Data();
         hFile << (TString::Format("\t%s Get%s() {", cur_col->strVariableType.Data(),
         ...
         if (cur_col->isNullable) {
            ...
            if (cur_col->isBinary) {
               ...
               hFile << (TString::Format("{%s %s = new %s[%s]; ", cur_col->strVariableType.Data(),
                                         cur_col->strTempVariableName.Data(), cur_col->strVariableTypePointer.Data(),
                                         next_col->strVariableName.Data()))
                           .Data();
               hFile << (TString::Format("memcpy(%s, %s, %s); ", cur_col->strTempVariableName.Data(),
                                         cur_col->strVariableName.Data(), next_col->strVariableName.Data()))
                           .Data();
               hFile << (TString::Format("return %s;}", cur_col->strTempVariableName.Data())).Data();
            } else
               hFile << (TString::Format("return new %s(*%s);", cur_col->strVariableTypePointer.Data(),
      ...
      if (isJoin) {
         ...
         for (int i = 0; i < curTableJoin->arrManualFieldNames->GetEntriesFast(); i++) {
            ...
            hFile << (TString::Format("\t/// get %s of %s\n", cur_col->strColumnNameSpace.Data(),
                                      curTableJoin->strJoinTableName.Data()))
                        .Data();
            hFile << (TString::Format("\t%s Get%s() {", cur_col->strVariableType.Data(),
                                      cur_col->strShortVariableName.Data()))
                        .Data();
      ...
      for (vector<structColumnInfo *>::iterator it = vecColumns.begin(); it != vecColumns.end(); ++it) {
         ...
         if (!cur_col->isBinary) {
            ...
            hFile << (TString::Format("\t/// set %s of the current %s\n", cur_col->strColumnNameSpace.Data(),
                                      strTableNameSpace.Data()))
                        .Data();
            hFile << (TString::Format("\tint Set%s(%s %s, ", cur_col->strShortVariableName.Data(),
                                      cur_col->strVariableType.Data(), cur_col->strColumnName.Data()))
                        .Data();
            ++it;
            cur_col = *it;
      ...
      if (isOnlyUpdate) {
         ...
         while (getline(oldFile, cur_line)) {
            string trim_line = trim(cur_line);
            if (trim_line.substr(0, 26) == "/* END OF PUBLIC GENERATED") break;
         }

         while (getline(oldFile, cur_line)) hFile << cur_line << endl;
      } else {
         hFile << (TString::Format("\n ClassDef(%s,1);\n", strClassName.Data())).Data();
         hFile << "};\n";
         hFile << "\n#endif\n";
      ...
      if (isJoin) {
         ...
         structColumnInfo *cur_col;
         while (cur_col = (structColumnInfo *)nextCol())
            cxxFile << (TString::Format("\t%s = %s;\n", cur_col->strVariableName.Data(), cur_col->strColumnName.Data()))
                          .Data();
      } // for join table
      cxxFile << "}\n\n";
      ...
      cxxFile << (TString::Format("// -----   Creating new %s in the database  ---------------------------\n",
                                  strTableNameSpace.Data()))
                    .Data();
      cxxFile << (TString::Format("%s* %s::Create%s(", strClassName.Data(), strClassName.Data(),
                                  strShortTableName.Data()))
                    .Data();
      count = 0;
      for (vector<structColumnInfo *>::iterator it = vecColumns.begin(); it != vecColumns.end(); ++it) {
      ...
      if (isJoin) {
         ...
         while (cur_col = (structColumnInfo *)nextCol()) {
            ...
            if (count == 0)
               ...
               cxxFile << (TString::Format(", %s %s", cur_col->strVariableType.Data(), cur_col->strColumnName.Data()))
                             .Data();
      ...
      for (vector<structColumnInfo *>::iterator it = vecColumns.begin(); it != vecColumns.end(); ++it) {
         ...
         if (cur_col->isNullable)
            cxxFile << (TString::Format("\tif (%s == NULL)\n\t\tstmt->SetNull(%d);\n\telse\n\t",
                                        cur_col->strColumnName.Data(), count))
                          .Data();

         if (!cur_col->isBinary)
            cxxFile << (TString::Format("\tstmt->Set%s(%d, %s);\n", cur_col->strStatementType.Data(), count,
                                        cur_col->strColumnPointer.Data()))
                          .Data();
         else {
            cxxFile << (TString::Format("\tstmt->Set%s(%d, %s, ", cur_col->strStatementType.Data(), count,
                                        cur_col->strColumnName.Data()))
                          .Data();
            ++it;
            cur_col = *it;
      ...
      if (strIdentityColumnName != "") {
         ...
         if (curDBMS == MySQL) {
            ...
            cxxFile << (TString::Format("\tTSQLStatement* stmt_last = uni_db->Statement(\"SELECT "
                                        "currval(pg_get_serial_sequence('%s','%s'))\");\n",
                                        strTableName.Data(), strIdentityColumnName.Data()))
                          .Data();
         }
      ...
      if (isJoin) {
         ...
         while (cur_col = (structColumnInfo *)nextSel()) {
            ...
            count++;
            if (cur_col->isBinary) nextSel();
         }
         cxxFile
            << (TString::Format(" \"\n\t\t\"from %s \"\n\t\t\"where", curTableJoin->strJoinTableName.Data())).Data();
         if (curTableJoin->strJoinField.strStatementType == "String")
            cxxFile << (TString::Format(" lower(%s) = lower('%%%s')", curTableJoin->strJoinField.strColumnName.Data(),
                                        curTableJoin->strJoinField.strPrintfType.Data()))
                          .Data();
         else
            cxxFile << (TString::Format(" %s = %%%s", curTableJoin->strJoinField.strColumnName.Data(),
                                        curTableJoin->strJoinField.strPrintfType.Data()))
                          .Data();
         cxxFile << "\"";
         ...
         cxxFile << "\tif (!stmt->Process())\n\t{\n";
         cxxFile << (TString::Format(
                        "\t\tcout<<\"ERROR: getting join record from DB has been failed for '%s' table\"<<endl;\n\n",
                        curTableJoin->strJoinTableName.Data()))
                       .Data();
         cxxFile << "\t\tdelete stmt;\n";
            ...
            if (cur_col->isNullable) {
               ...
               if (cur_col->isBinary) {
                  ...
                  cxxFile << (TString::Format("\tstmt->Get%s(%d, (void*&)%s, %s);\n", StatementType.Data(), count,
                                              TempVar.Data(), cur_col->strTempVariableName.Data()))
                                .Data();
               } else
                  cxxFile << (TString::Format("\t%s = stmt->Get%s(%d);\n", TempVar.Data(), StatementType.Data(), count))
                                .Data();
            }
      ...
      for (vector<structColumnInfo *>::iterator it = vecColumns.begin(); it != vecColumns.end(); ++it) {
         ...
         TString TempColumnName = cur_col->strColumnName, TempVar = cur_col->strTempVariableName,
                 VariableTypePointer = cur_col->strVariableTypePointer;
         cxxFile << (TString::Format("\t%s %s;\n", cur_col->strVariableType.Data(), TempVar.Data())).Data();
         ...
         if (cur_col->isNullable) {
            ...
            if (cur_col->isBinary) {
               ...
               cur_col = *it;
               cxxFile << (TString::Format("\t{\n\t\t%s %s = %s;\n", cur_col->strVariableType.Data(),
                                           cur_col->strTempVariableName.Data(), cur_col->strColumnName.Data()))
                             .Data();
      ...
      cxxFile << (TString::Format("// -----  Get %s from the database  ---------------------------\n",
                                  strTableNameSpace.Data()))
                    .Data();
      cxxFile << (TString::Format("%s* %s::Get%s(", strClassName.Data(), strClassName.Data(), strShortTableName.Data()))
                    .Data();
      count   = 0;
      is_flag = false;
      for (vector<structColumnInfo *>::iterator it = vecColumns.begin(); it != vecColumns.end(); ++it) {
      ...
      if (isJoin) {
         ...
         while (cur_col = (structColumnInfo *)nextCol()) {
            ...
            count++;
            if (cur_col->isBinary) nextCol();
         }
      } // for join table
      cxxFile << (TString::Format(" \"\n\t\t\"from %s", strTableName.Data())).Data();
      // for join table
      if (isJoin) {
         cxxFile << (TString::Format(" join %s on %s.%s=%s.%s", curTableJoin->strJoinTableName.Data(),
                                     strTableName.Data(), curTableJoin->strJoinField.strColumnName.Data(),
                                     curTableJoin->strJoinTableName.Data(),
                                     curTableJoin->strJoinField.strColumnName.Data()))
                       .Data();
      } // for join table
      cxxFile << " \"\n\t\t\"where";
      count = 0;
      for (vector<structColumnInfo *>::iterator it = vecColumns.begin(); it != vecColumns.end(); ++it) {
         ...
         if (count > 0) cxxFile << " and";

         if (cur_col->strStatementType == "String")
            cxxFile << (TString::Format(" lower(%s) = lower('%%%s')", cur_col->strColumnName.Data(),
                                        cur_col->strPrintfType.Data()))
                          .Data();
         else
            cxxFile
               << (TString::Format(" %s = %%%s", cur_col->strColumnName.Data(), cur_col->strPrintfType.Data())).Data();
      ...
      if (is_flag) {
         for (vector<structColumnInfo *>::iterator it = vecColumns.begin(); it != vecColumns.end(); ++it) {
            ...
            cxxFile << (TString::Format("%s* %s::Get%s(%s %s)\n{\n", strClassName.Data(), strClassName.Data(),
                                        strShortTableName.Data(), cur_col->strVariableType.Data(),
                                        cur_col->strColumnName.Data()))
                          .Data();
            ...
            cxxFile << "\tTString sql = TString::Format(\n\t\t\"select";
            count = 0;
            for (vector<structColumnInfo *>::iterator it_inner = vecColumns.begin(); it_inner != vecColumns.end();
                 ++it_inner) {
               ...
               count++;
               if (current_col->isBinary) ++it_inner;
            }
            ...
            if (cur_col->strStatementType == "String")
               cxxFile << (TString::Format(" \"\n\t\t\"where lower(%s) = lower('%%%s')\", %s.Data());\n",
                                           cur_col->strColumnName.Data(), cur_col->strPrintfType.Data(),
                                           cur_col->strColumnName.Data()))
                             .Data();
            else
               cxxFile << (TString::Format(" \"\n\t\t\"where %s = %%%s\", %s);\n", cur_col->strColumnName.Data(),
                                           cur_col->strPrintfType.Data(), cur_col->strColumnName.Data()))
                             .Data();
               ...
               if (current_col->isNullable) {
                  ...
                  if (current_col->isBinary) {
                     ...
                     cxxFile << (TString::Format("\t\t%s %s = 0;\n", current_col->strVariableType.Data(),
                                                 current_col->strTempVariableName.Data()))
                                   .Data();
                     cxxFile << (TString::Format("\t\tstmt->Get%s(%d, (void*&)%s, %s);\n\t}\n", StatementType.Data(),
                                                 count, TempVar.Data(), current_col->strTempVariableName.Data()))
                                   .Data();
                  } else
                     cxxFile << (TString::Format("\t%s = new %s(stmt->Get%s(%d));\n", TempVar.Data(),
                                                 VariableTypePointer.Data(), StatementType.Data(), count))
                  ...
                  if (current_col->isBinary) {
                     ...
                     cxxFile << (TString::Format("\t%s %s = 0;\n", current_col->strVariableType.Data(),
                                   ...
                                   .Data();
                     cxxFile << (TString::Format("\tstmt->Get%s(%d, (void*&)%s, %s);\n", StatementType.Data(), count,
                                                 TempVar.Data(), current_col->strTempVariableName.Data()))
                                   .Data();
                  } else
         ...
         for (vector<structColumnInfo *>::iterator it = vecColumns.begin(); it != vecColumns.end(); ++it) {
            ...
            if (curDBMS == MySQL) {
               if (cur_col->strStatementType == "String")
                  cxxFile << (TString::Format("\tTString sql = TString::Format(\n\t\t\"delete from %s \"\n\t\t\"where "
                                              "lower(%s) = lower(?)\");\n",
                                              strTableName.Data(), cur_col->strColumnName.Data()))
                                .Data();
               else
                  cxxFile << (TString::Format(
      ...
      for (vector<structColumnInfo *>::iterator it = vecColumns.begin(); it != vecColumns.end(); ++it) {
         ...
         if (cur_col->isNullable) {
            ...
            if (cur_col->isBinary) {
               ...
               if (cur_col->isDateTime) cxxFile << ".AsSQLString()";
               if (cur_col->isTimeStamp) cxxFile << ".AsString(\"s\")";
               cxxFile << ";\n";
            }
         } else {
            if (cur_col->isBinary) {
               cxxFile
                  << (TString::Format("\t\t%s %s = NULL;\n", cur_col->strVariableType.Data(), TempVar.Data())).Data();
               ++it;
               cur_col = *it;
               cxxFile << (TString::Format("\t\t%s %s=0;\n", cur_col->strVariableType.Data(),
                                           cur_col->strTempVariableName.Data()))
      ...
      for (vector<structColumnInfo *>::iterator it = vecColumns.begin(); it != vecColumns.end(); ++it) {
         ...
         if (!cur_col->isBinary)
            cxxFile << (TString::Format("int %s::Set%s(%s %s)\n{\n", strClassName.Data(),
                                        cur_col->strShortVariableName.Data(), cur_col->strVariableType.Data(),
                                        cur_col->strColumnName.Data()))
                          .Data();
         else {
            cxxFile << (TString::Format("int %s::Set%s(%s %s, ", strClassName.Data(),
                                        cur_col->strShortVariableName.Data(), cur_col->strVariableType.Data(),
                                        cur_col->strColumnName.Data()))
                          .Data();
            ++it;
         ...
         for (vector<structColumnInfo *>::iterator it_inner = vecColumns.begin(); it_inner != vecColumns.end();
              ++it_inner) {
            structColumnInfo *current_col = *it_inner;
            if (!current_col->isPrimary) continue;
         ...
         if (!cur_col->isBinary)
            ...
            cxxFile << (TString::Format("\tstmt->Set%s(0, %s, ", cur_col->strStatementType.Data(),
                                        cur_col->strColumnName.Data()))
                          .Data();
         ...
         for (vector<structColumnInfo *>::iterator it_inner = vecColumns.begin(); it_inner != vecColumns.end();
            ...
            cxxFile << (TString::Format("\tstmt->Set%s(%d, %s);\n", current_col->strStatementType.Data(), count,
                                        current_col->strVariableName.Data()))
                          .Data();
            count++;
         }
         ...
         if (cur_col->isNullable) {
            ...
            if (cur_col->isBinary) {
               cxxFile << (TString::Format("\t{\n\t\t%s %s = %s;\n", temp_col->strVariableType.Data(),
                                           temp_col->strVariableName.Data(), cur_col->strColumnName.Data()))
                             .Data();
               cxxFile << (TString::Format("\t\t%s = new %s[%s];\n", cur_col->strVariableName.Data(),
                                           cur_col->strVariableTypePointer.Data(), temp_col->strVariableName.Data()))
                             .Data();
               cxxFile << (TString::Format("\t\tmemcpy(%s, %s, %s);\n\t}\n", cur_col->strVariableName.Data(),
                                           cur_col->strColumnName.Data(), temp_col->strVariableName.Data()))
                             .Data();
            } else
               cxxFile << (TString::Format("\t%s = new %s(*%s);\n", cur_col->strVariableName.Data(),
                                           cur_col->strVariableTypePointer.Data(), cur_col->strColumnName.Data()))
                             .Data();
         } else {
            if (cur_col->isBinary) {
      ...
      for (vector<structColumnInfo *>::iterator it = vecColumns.begin(); it != vecColumns.end(); ++it) {
         ...
         if (cur_col->isBinary) {
            cxxFile << (TString::Format("<<\". %s: \"<<(void*)%s", cur_col->strColumnName.Data(),
                                        cur_col->strVariableName.Data()))
                          .Data();
            ++it;
            cur_col = *it;
            ...
            if (cur_col->isNullable) {
               if (cur_col->isDateTime)
                  cxxFile << (TString::Format("<<\". %s: \"<<(%s == NULL? \"NULL\": (*%s).AsSQLString())",
                                              cur_col->strColumnName.Data(), cur_col->strVariableName.Data(),
                                              cur_col->strVariableName.Data()))
                                .Data();
               else {
                  if (cur_col->isTimeStamp)
                     ...
                     if (cur_col->strPrintfType == "s")
                        ...
                        cxxFile << (TString::Format(
                                       "<<\". %s: \"<<(%s == NULL? \"NULL\": TString::Format(\"%%%s\", *%s))",
                                       cur_col->strColumnName.Data(), cur_col->strVariableName.Data(),
                                       cur_col->strPrintfType.Data(), cur_col->strVariableName.Data()))
                                      .Data();
                  }
               }
            } else {
               cxxFile << (TString::Format("<<\". %s: \"<<%s", cur_col->strColumnName.Data(),
                                           cur_col->strVariableName.Data()))
                             .Data();
               if (cur_col->isDateTime) cxxFile << ".AsSQLString()";
               if (cur_col->isTimeStamp) cxxFile << ".AsString(\"s\")";
      ...
      if (isOnlyUpdate) {
         ...
         while (getline(oldFile, cur_line)) {
            string trim_line = trim(cur_line);
            if (trim_line.substr(0, 25) == "/* END OF GENERATED CLASS") break;
         }

         while (getline(oldFile, cur_line)) cxxFile << cur_line << endl;
      } else {
         cxxFile << "\n// -------------------------------------------------------------------\n";
