...
// enumeration of table columns for Search Conditions (UniDbRun::Search, UniDbDetectorParameters::Search function).
...
enum enumColumns {
   ...
   columnEnergy, // UniDbRun
   columnStartDatetime,
   columnEndDateTime,
   columnEventCount,
   columnFieldVoltage,
   columnFileSize, // UniDbRun
   columnDetectorName,
   columnParameterName,
   columnStartPeriod,
   columnStartRun,
   columnEndPeriod,
   columnEndRun,
   columnValueKey, // UniDbDetectorParameter
   columnRecordDate,
   columnShiftLeader,
   columnType,
   columnTrigger,
   ...
   columnSp41,
   columnSp57,
   columnVkm2, // ElogDbRecord +
               // {columnPeriodNumber,columnRunNumber,columnBeamParticle,columnEnergy,columnTargetParticle}
   columnGeneratorName,
   columnCentrality,
   columnFileDesc,
   columnError
}; // UniDbSimulationFile +
   // {columnFilePath,columnBeamParticle,columnTargetParticle,columnEnergy,columnEventCount,columnFileSize}
