...
UniConnection::UniConnection(TSQLServer *pSQLServer) { server_db = pSQLServer; }
...
std::cout << "DEBUG!!!! DB STRING " << conString << "\n";
  ...
  if (it != UniConnection::mapConnection->end())
    ...
    switch (connection_type) {
    case UNIFIED_DB:
std::cout << "DEBUG!!!! DB LOGIN " << UNI_DB_USERNAME << " " << UNI_DB_PASSWORD << "\n";
      pSQLServer =
          TSQLServer::Connect(conString, UNI_DB_USERNAME, UNI_DB_PASSWORD);
      break;
//    case TANGO_DB:
//      pSQLServer =
//          TSQLServer::Connect(conString, TANGO_DB_USERNAME, TANGO_DB_PASSWORD);
//      break;
//    case ELOG_DB:
//      pSQLServer =
//          TSQLServer::Connect(conString, ELOG_DB_USERNAME, ELOG_DB_PASSWORD);
//      break;
    }
    if (pSQLServer == 0x00) {
      cout << "ERROR: database connection was not established (m.b. wrong "
              "login or password)" << endl;
      return 0x00;
    }
    // cout<<"Server info: "<<pSQLServer->ServerInfo()<<endl;

    UniConnection::mapConnection->insert(
        pairSQLServer(conString.Data(), pSQLServer));
  }
...
UniConnection *UniConnection::Open(enumDBMS database_type,
                                   const char *strDBHost, const char *strDBName,
                                   const char *strUID,
  ...
  TString conString =
      TString::Format("%s://%s/%s", db_type, strDBHost, strDBName);
