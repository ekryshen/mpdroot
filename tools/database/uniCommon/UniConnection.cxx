...
UniConnection *UniConnection::Open(UniConnectionType connection_type)
{
   ...
   case UNIFIED_DB: conString = TString::Format("pgsql://%s/%s", UNI_DB_HOST, UNI_DB_NAME); break;
   default: {
      cout << "ERROR: incorrect database connection type!" << endl;
      return 0x00;
   }
   }
   if (UniConnection::mapConnection == NULL) UniConnection::mapConnection = new mapSQLServer();
   ...
   if (it != UniConnection::mapConnection->end())
      ...
      switch (connection_type) {
      case UNIFIED_DB: pSQLServer = TSQLServer::Connect(conString, UNI_DB_USERNAME, UNI_DB_PASSWORD); break;
      }
      if (pSQLServer == 0x00) {
...
// -------------------------------------------------------------------
UniConnection *UniConnection::Open(enumDBMS database_type, const char *strDBHost, const char *strDBName,
                                   const char *strUID, const char *strPassword)
{
   char *db_type;
   switch (database_type) {
   case MySQL: db_type = (char *)"mysql"; break;
   case PgSQL: db_type = (char *)"pgsql"; break;
   default: {
