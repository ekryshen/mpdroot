...
protected:
   ...
   void Write(unsigned char *&destination, unsigned char *value, uint64_t size)
   {
      if (size > 0) {
         ...
         cout << "ERROR: count of bytes for the value should be greater than zero. The value was not written to the "
                 "database!"
              << endl;
   }
   ...
   void Read(unsigned char *&source, double ***value, uint8_t &size1, uint8_t &size2, uint8_t &size3)
   {
      ...
      for (int i = 0; i < size1; i++) {
         ...
         for (int j = 0; j < size2; j++) value[i][j] = new double[size3];
      }
   ...
   void Write(unsigned char *&destination, vector<double> &value)
   {
      ...
      if (size > 0) {
         ...
         cout
            << "ERROR: vector size of the value should be greater than zero. The value was not written to the database!"
            << endl;
   }
};
#endif
