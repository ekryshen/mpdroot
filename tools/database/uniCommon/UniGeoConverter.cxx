...
int UniGeoConverter::RecursiveNodeChanging(TSQLServer *uni_db, TGeoNode *node, int parent_node_id)
{
   for (int i = 0; i < node->GetNdaughters(); i++) {
      ...
      if (parent_node_id == -1) cout << "Geometry for detector " << curVolume->GetName() << " is writing..." << endl;
      ...
      if (stmt_select->Process()) {
         ...
         if (!stmt_select->NextResultRow()) {
            ...
            strStatement = TString::Format("INSERT INTO geometry_media(media_name, density, radiation_length, "
                                           "interaction_length, sensitivity_flag, "
                                           "field_flag, field_max, ang_deviation_max, step_max, energy_loss_max, "
                                           "b_crossing_precision, step_min, media_desc) "
                                           "VALUES (\"%s\", %f, %f, %f, %d, %d, %f, %f, %f ,%f, %f, %f, null)",
                                           pMedium->GetName(), pMaterial->GetDensity(), pMaterial->GetRadLen(),
                                           pMaterial->GetIntLen(), (int)pMedium->GetParam(0), (int)pMedium->GetParam(1),
                                           pMedium->GetParam(2), pMedium->GetParam(3), pMedium->GetParam(4),
                                           pMedium->GetParam(5), pMedium->GetParam(6), pMedium->GetParam(7));
            ...
            if (pMaterial->IsMixture()) {
               stmt_insert = uni_db->Statement(
                  "INSERT INTO media_element(media_name, atomic_weight, atomic_number, relative_weight, Natoms) "
                  "VALUES (?, ?, ?, ?, ?)");
               ...
               strStatement = TString::Format(
                  "INSERT INTO media_element(media_name, atomic_weight, atomic_number, relative_weight, Natoms) "
                  "VALUES (\"%s\", %f, %f, null, null)",
                  pMedium->GetName(), pMaterial->GetA(), pMaterial->GetZ());
               stmt_insert = uni_db->Statement(strStatement);
            ...
            if (pMedium->GetName() != stmt_select->GetString(0)) pMedium->SetName(stmt_select->GetString(0));
      ...
      if (pShape->TestShapeBit(TGeoShape::kGeoPgon))
         curShape = dbGeoPgon;
      else if (pShape->TestShapeBit(TGeoShape::kGeoPcon))
         curShape = dbGeoPcon;
      else if (pShape->TestShapeBit(TGeoShape::kGeoTubeSeg))
         curShape = dbGeoTubeSeg;
      else if (pShape->TestShapeBit(TGeoShape::kGeoTube))
         curShape = dbGeoTube;
      else if (pShape->TestShapeBit(TGeoShape::kGeoConeSeg))
         curShape = dbGeoConeSeg;
      else if (pShape->TestShapeBit(TGeoShape::kGeoCone))
         curShape = dbGeoCone;
      else if (pShape->TestShapeBit(TGeoShape::kGeoSph))
         curShape = dbGeoSphere;
      else if (pShape->TestShapeBit(TGeoShape::kGeoTorus))
         curShape = dbGeoTorus;
      else if (pShape->TestShapeBit(TGeoShape::kGeoTrd1))
         curShape = dbGeoTrd1;
      else if (pShape->TestShapeBit(TGeoShape::kGeoTrd2))
         curShape = dbGeoTrd2;
      else if (pShape->TestShapeBit(TGeoShape::kGeoTrap))
         curShape = dbGeoTrap;
      else if (pShape->TestShapeBit(TGeoShape::kGeoPara))
         curShape = dbGeoPara;
      else if (pShape->TestShapeBit(TGeoShape::kGeoComb))
         curShape = dbGeoComb;
      else if (pShape->TestShapeBit(TGeoShape::kGeoArb8))
         curShape = dbGeoArb8;
      else if (pShape->TestShapeBit(TGeoShape::kGeoEltu))
         curShape = dbGeoEltu;
      else if (pShape->TestShapeBit(TGeoShape::kGeoXtru))
         curShape = dbGeoXtru;
      ...
      if (parent_node_id == -1) {
         ...
         if (stmt_select->Process()) {
            ...
            // if the component exist
               ...
               if (curVolume->GetName() != stmt_select->GetString(0)) curVolume->SetName(stmt_select->GetString(0));
         ...
         strStatement = TString::Format("INSERT INTO geometry_node(node_name, parent_node_id, media_name, shape_index) "
                                        "VALUES (\"%s\", null, \"%s\", %d)",
                                        child->GetName(), pMedium->GetName(), (int)curShape);
         stmt_insert  = uni_db->Statement(strStatement);
         ...
         strStatement = TString::Format("INSERT INTO geometry_node(node_name, parent_node_id, media_name, shape_index) "
                                        "VALUES (\"%s\", %d, \"%s\", %d)",
                                        child->GetName(), parent_node_id, pMedium->GetName(), (int)curShape);
         stmt_insert  = uni_db->Statement(strStatement);
      ...
      if (child->GetNdaughters() != 0) RecursiveNodeChanging(uni_db, child, last_id);
   } // for(int i = 0; i < node->GetNdaughters(); i++)
