...
enum enumValueType : unsigned int {
   ...
   GemPedestalType,
   TriggerMapType,
   LorentzShiftType, // detector-dependent types
   MapBoolType,
   MapIntType,
   MapDVectorType,
   AlignmentType,
...
struct IIValue : public UniValue {
   ...
   {
      Write(destination, value1);
      Write(destination, value2);
   }
};
...
struct DchMapValue : public UniValue {
   ...
   void          ReadValue(unsigned char *source)
   {
      ...
      Read(source, crate);
      Read(source, slot);
      Read(source, channel_low);
      Read(source, channel_high);
   }
   void WriteValue(unsigned char *destination)
   {
      Write(destination, plane);
      Write(destination, group);
      Write(destination, crate);
      Write(destination, slot);
      Write(destination, channel_low);
      Write(destination, channel_high);
   }
};
...
struct GemMapValue : public UniValue {
   ...
   void WriteValue(unsigned char *destination)
   {
      Write(destination, serial);
      Write(destination, id);
      Write(destination, station);
      Write(destination, channel_low);
      Write(destination, channel_high);
      Write(destination, hotZone);
   }
};
...
struct GemPedestalValue : public UniValue {
   ...
   void          ReadValue(unsigned char *source)
   {
      Read(source, serial);
      Read(source, channel);
      Read(source, pedestal);
      Read(source, noise);
   }
   void WriteValue(unsigned char *destination)
   {
      Write(destination, serial);
      Write(destination, channel);
      Write(destination, pedestal);
      Write(destination, noise);
   }
};
...
struct AlignmentValue : public UniValue {
   ...
   {
      Write(destination, station);
      Write(destination, module);
      Write(destination, value, 3);
   }
};
