...
int system_command_linux(string aCommand, string &result)
{
   ...
   char      buf[BUFSIZE];
   if (fgets(buf, BUFSIZE, f) != NULL) result = buf;
...
string get_app_name_linux()
{
   ...
   if (last_pos != string::npos) fRes.erase(last_pos);
...
double byte_size_to_double(string byte_size_in_string, char convert_to)
{
   ...
   if (convert_dim != value_dim) number *= pow(1024, convert_dim - value_dim);
...
// 8 following functions: extract first number or last number in string, only positive number by default (result number
// has string type)
string find_first_number(string const &str, bool isOnlyPositive)
{
   ...
   if (n != string::npos) {
      ...
      if ((!isOnlyPositive) && (n > 0) && ((str[n - 1] == '+') || (str[n - 1] == '-'))) n--;
...
string find_last_number(string const &str, bool isOnlyPositive)
{
   ...
   if (n != string::npos) {
      ...
      if (m != string::npos) {
         if ((!isOnlyPositive) && ((str[m] == '+') || (str[m] == '-'))) m--;
...
string find_first_number(string const &str, size_t &beg_pos, size_t &end_pos, bool isOnlyPositive)
{
   ...
   if (beg_pos != string::npos) {
      ...
      if ((!isOnlyPositive) && (beg_pos > 0) && ((str[beg_pos - 1] == '+') || (str[beg_pos - 1] == '-'))) beg_pos--;
...
string find_last_number(string const &str, size_t &beg_pos, size_t &end_pos, bool isOnlyPositive)
{
   ...
   if (end_pos != string::npos) {
      ...
      if (m != string::npos) {
         if ((!isOnlyPositive) && ((str[m] == '+') || (str[m] == '-'))) beg_pos--;
...
{
   if (input_char_array == NULL) return NULL;
   ...
   for (int i = 0; i < length; i++) lower[i] = tolower(input_char_array[i]);
...
string get_file_name(string path)
{
   ...
   if (last_slash_idx != string::npos) path.erase(0, last_slash_idx + 1);

   // Remove extension if present.
   size_t period_idx = path.rfind('.');
   if (period_idx != string::npos) path.erase(period_idx);
