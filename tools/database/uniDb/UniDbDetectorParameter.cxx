...
UniDbDetectorParameter::UniDbDetectorParameter(UniConnection *connUniDb, int value_id, TString detector_name,
                                               int parameter_id, int start_period, int start_run, int end_period,
                                               int end_run, int value_key, unsigned char *parameter_value,
...
{
   if (connectionUniDb) delete connectionUniDb;
   if (blob_parameter_value) delete[] blob_parameter_value;
}

// -----   Creating new detector parameter in the database  ---------------------------
UniDbDetectorParameter *UniDbDetectorParameter::CreateDetectorParameter(TString detector_name, int parameter_id,
                                                                        int start_period, int start_run, int end_period,
                                                                        int end_run, int value_key,
                                                                        ...
                                                                        Long_t         size_parameter_value)
{
   ...
   TString sql = TString::Format("insert into detector_parameter(detector_name, parameter_id, start_period, start_run, "
                                 "end_period, end_run, value_key, parameter_value) "
   ...
   int            value_id;
   TSQLStatement *stmt_last =
      uni_db->Statement("SELECT currval(pg_get_serial_sequence('detector_parameter','value_id'))");
   ...
   return new UniDbDetectorParameter(connUniDb, tmp_value_id, tmp_detector_name, tmp_parameter_id, tmp_start_period,
                                     tmp_start_run, tmp_end_period, tmp_end_run, tmp_value_key, tmp_parameter_value,
                                     tmp_sz_parameter_value);
}
...
UniDbDetectorParameter *UniDbDetectorParameter::GetDetectorParameter(int value_id)
{
   ...
   TString sql = TString::Format("select value_id, detector_name, parameter_id, start_period, start_run, end_period, "
                                 "end_run, value_key, parameter_value "
...
bool UniDbDetectorParameter::CheckDetectorParameterExists(int value_id)
{
   ...
   TString        sql  = TString::Format("select 1 "
                                                 "from detector_parameter "
                                                 "where value_id = %d",
                                         value_id);
   TSQLStatement *stmt = uni_db->Statement(sql);
...
int UniDbDetectorParameter::DeleteDetectorParameter(int value_id)
{
   ...
   TString        sql  = TString::Format("delete from detector_parameter "
                                                 "where value_id = $1");
   TSQLStatement *stmt = uni_db->Statement(sql);
...
void UniDbDetectorParameter::Print()
{
   ...
   cout << ". value_id: " << i_value_id << ". detector_name: " << str_detector_name
        << ". parameter_id: " << i_parameter_id << ". start_period: " << i_start_period
        << ". start_run: " << i_start_run << ". end_period: " << i_end_period << ". end_run: " << i_end_run
        << ". value_key: " << i_value_key << ". parameter_value: " << (void *)blob_parameter_value
        << ", binary size: " << sz_parameter_value << endl;
...
unsigned char *UniDbDetectorParameter::GetUNC(enumValueType enum_parameter_type)
{
   if (enum_parameter_type > -1) {
      ...
      TString        sql  = TString::Format("select parameter_name, parameter_type "
                                                    "from parameter_ "
      ...
      if (parameter_type != enum_parameter_type) {
         cout << "CRITICAL ERROR: the parameter with name '" << parameter_name
              << "' is not corresponding the given type: "
                 "Database Type - "
              << parameter_type << ", but user type - " << enum_parameter_type << endl;
         return NULL;
      }
   }
...
int UniDbDetectorParameter::SetUNC(unsigned char *p_parameter_value, Long_t size_parameter_value)
{
   ...
   TString sql = TString::Format("update detector_parameter "
                                 "set parameter_value = $1 "
                                 "where detector_name = $2 and parameter_id = $3 and start_period = $4 and start_run = "
                                 "$5 and end_period = $6 and end_run = $7 and value_key = $8");
...
UniDbDetectorParameter *UniDbDetectorParameter::CreateDetectorParameter(
   ...
   unsigned char *p_parameter_value, Long_t size_parameter_value, enumValueType enum_parameter_type, int value_key)
{
   ...
   //{
   //    cout<<"ERROR: TPgSQLStatement (CERN ROOT) was not patched to write binary data into the Unified
   //    Database"<<endl; return 0x00;
   //}

   if (((end_period < start_period) or ((end_period == start_period) and (end_run < start_run))) or
       ((start_period > end_period) or ((start_period == end_period) and (start_run > end_run)))) {
   ...
   // cout<<"DEBUG: detector_name = "<<detector_name<<". parameter_id = "<<parameter_id<<". start_period =
   // "<<start_period<<". start_run = "<<start_run<<". end_period = "<<end_period<<". end_run = "<<end_run<<". value_key
   // = "<<value_key<<". p_parameter_value[0] = "<<p_parameter_value[0]<<". size_parameter_value =
   // "<<size_parameter_value<<endl;
   stmt->NextIteration();
   stmt->SetString(0, detector_name);
   stmt->SetInt(1, parameter_id);
   stmt->SetInt(2, start_period);
   ...
   return new UniDbDetectorParameter(connUniDb, last_id, detector_name, parameter_id, start_period, start_run,
                                     end_period, end_run, value_key, p_parameter_value, size_parameter_value);
}

// write detector parameter value presented by an array (integer value key is optional, default, 0)
UniDbDetectorParameter *UniDbDetectorParameter::CreateDetectorParameter(TString detector_name, TString parameter_name,
                                                                        int start_period, int start_run, int end_period,
                                                                        int end_run, vector<UniValue *> parameter_value,
                                                                        int value_key)
{
   ...
   UniDbDetectorParameter *pDetectorParameter = UniDbDetectorParameter::CreateDetectorParameter(
      detector_name, parameter_name, start_period, start_run, end_period, end_run, p_parameter_value,
      size_parameter_value, parameter_type, value_key);
   if (pDetectorParameter == 0x00) delete[] p_parameter_value;
...
UniDbDetectorParameter *UniDbDetectorParameter::CreateDetectorParameter(TString detector_name, TString parameter_name,
                                                                        int start_period, int start_run, int end_period,
                                                                        int end_run, UniValue *parameter_value,
                                                                        int value_key)
{
                                                                     ...
                                                                     TDatime *usage_date)
{
   ...
   TString sql = TString::Format(
      ...
      "(not (((%d < start_period) or ((%d = start_period) and (%d < start_run))) or ((%d > end_period) or ((%d = "
      "end_period) and (%d > end_run))))) and "
      "value_key = %d",
   ...
   return new UniDbDetectorParameter(connUniDb, tmp_value_id, tmp_detector_name, tmp_parameter_id, tmp_start_period,
                                     tmp_start_run, tmp_end_period, tmp_end_run, value_key, tmp_parameter_value,
                                     tmp_sz_parameter_value);
}

// delete detector parameter value (integer value key is optional, default, 0)
int UniDbDetectorParameter::DeleteDetectorParameter(TString detector_name, TString parameter_name, int start_period,
                                                    int start_run, int end_period, int end_run, int value_key)
{
   UniConnection *connUniDb = UniConnection::Open(UNIFIED_DB);
   if (connUniDb == 0x00) return 0x00;
   ...
   TString sql = TString::Format("delete from detector_parameter "
                                 "where lower(detector_name) = lower($1) and parameter_id IN (select parameter_id from "
                                 "parameter_ where lower(parameter_name) = lower($2)) and start_period = $3 and "
                                 "start_run = $4 and end_period = $5 and end_run = $6 and value_key = $7");
...
int UniDbDetectorParameter::GetValue(vector<UniValue *> &parameter_value)
{
   ...
   if (p_parameter_value == NULL) return -1;
...
{
   unsigned char *p_parameter_value = GetUNC();
   if (p_parameter_value == NULL) return NULL;
...
int UniDbDetectorParameter::SetValue(vector<UniValue *> parameter_value)
{
   ...
   size_t size_parameter_value = 0;
   for (int i = 0; i < parameter_value.size(); i++) size_parameter_value += parameter_value[i]->GetStorageSize();
   unsigned char *p_parameter_value = new unsigned char[size_parameter_value];
...
// write a whole file with parameter value (with given detector and parameter names) to the Database from start run to
// end one
int UniDbDetectorParameter::WriteFile(const char *detector_name, const char *parameter_name, int start_period,
                                      int start_run, int end_period, int end_run, const char *file_path)
{
   ...
   pValue->value                           = buffer;
   UniDbDetectorParameter *pParameterValue = UniDbDetectorParameter::CreateDetectorParameter(
      detector_name, parameter_name, start_period, start_run, end_period, end_run, pValue);
...
// read a whole file with parameter value (with given detector and parameter names) from the Database for the selected
// run
int UniDbDetectorParameter::ReadFile(const char *detector_name, const char *parameter_name, int period_number,
                                     int run_number, const char *file_path)
{
   UniDbDetectorParameter *pDetectorParameter =
      UniDbDetectorParameter::GetDetectorParameter(detector_name, parameter_name, period_number, run_number);
   if (pDetectorParameter == NULL) return -1;
...
TObjArray *UniDbDetectorParameter::Search(TObjArray &search_conditions)
{
   ...
   TString sql = TString::Format("select value_id, detector_name, p.parameter_id, start_period, start_run, end_period, "
                                 "end_run, value_key, parameter_value "
                                 "from detector_parameter dp join parameter_ p on dp.parameter_id = p.parameter_id");
   ...
   while (curCondition = (UniSearchCondition *)next()) {
      ...
      default:
         cout << "ERROR: the comparison operator in the search condition was not defined, the condition is skipped"
              << endl;
         continue;
      }
      ...
      case 0:
         if ((curCondition->GetCondition() != conditionNull) && (curCondition->GetCondition() != conditionNotNull))
            continue;
         break;
   ...
   while (stmt->NextResultRow()) {
      ...
      arrayResult->Add((TObject *)new UniDbDetectorParameter(
         connPar, tmp_value_id, tmp_detector_name, tmp_parameter_id, tmp_start_period, tmp_start_run, tmp_end_period,
         tmp_end_run, tmp_value_key, tmp_parameter_value, tmp_sz_parameter_value));
   }
