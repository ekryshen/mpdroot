...
// <cycle child="parent name" table_name="database table to write parsing values" delimiter="delimeters between
// elements" skip="number of skipping text lines">
//   - cycle for elements separated by delimiters and write them to the database (childs of parent with given name if it
//   was set)
// <element action=["skip", "update", "write", "multi", "parse"], where:
// action="skip" - skip current element
// action="update" column_name="database column name" type="type name"/> - update database table row with database
// column name being equal the cuurent element action="write" column_name="database column name" type="type name"/> -
// convert current element to the given type (int, double, string, datetime, hex, binary) and write to the given
// database table column action="multi"> <subelement action...> <subelement action...> ... </element> action="parse"
// start_index="parse from start symbol" parse_type=["counter" - current element number from 1, "value:fixed_value",
// "parsed type name + 'int_array' or 'double_array'"] delimiter="delimeter symbol if parsed array"
// column_name="database column name" type="C++ type"/>
//   - converted value from start symbol (default, 0) OR special values to write to the database table column
// action="parse"  parse_type=["counter" - current element number from 1, "value:fixed_value"] column_name="database
// column name" type="type name"/> - special values to write to the database table column
// </cycle>
...
struct structParseRow {
   ...
   structParseRow(TString column_name, TString statement_type, bool is_parse = false, int start_index = 0,
                  TString parse_type = "", TString delimiter = "")
   {
...
struct structParseSchema {
   ...
   structParseSchema() { isUpdate = false; }
...
public:
   ...
   int ParseTxt2Struct(TString txtName, TString schemaPath, vector<structParseValue *> &parse_values,
                       vector<structParseSchema> &vecElements, int iVerbose = 1);
