...
int parse_cycle_statement(xmlNodePtr &cur_schema_node, vector<structParseSchema> &vecElements, int &skip_count,
                          char &delimiter_char, int &column_count)
{
   vecElements.clear();
...
TString prepare_sql_code(vector<structParseSchema> vecElements, TString strTableName, bool isUpdate)
{
   ...
   if (!isUpdate) {
      ...
      for (vector<structParseSchema>::iterator it = vecElements.begin(); it != vecElements.end(); ++it) {
         structParseSchema schema = *it;
         if ((schema.isSkip) || (schema.isUpdate)) continue;
      ...
      for (vector<structParseSchema>::iterator it = vecElements.begin(); it != vecElements.end(); ++it) {
         structParseSchema schema = *it;
         if (!schema.isUpdate) continue;

         for (int j = 0; j < schema.vecRows.size(); j++) {
            structParseRow row = schema.vecRows[j];
            if (count == 0) sql += TString::Format("%s = $1", row.strColumnName.Data());
            // else
...
int write_string_to_db(string &write_string, TSQLStatement *stmt, structParseSchema &schema, int &count,
                       int cycle_counter)
{
   int tmp_count;
   ...
   for (int i = 0; i < schema.vecRows.size(); i++) {
      ...
      Long_t         size_array = -1;
      if (row.isParse) {
         if (row.iStartIndex > 0) token = token.substr(row.iStartIndex, token.length() - row.iStartIndex);

         if (row.strParseType != "") {
            if (row.strParseType == "counter") token = int_to_string(cycle_counter);
            if (row.strParseType(0, 5) == "value") token = row.strParseType(6, row.strParseType.Length() - 6).Data();

            if (row.strParseType == "int") {
               int last_digit;
               for (last_digit = 0; last_digit < token.length(); last_digit++) {
                  if (!isdigit(token[last_digit])) break;
               }
               last_digit++;
            ...
            if (row.strParseType == "int_array") {
               ...
               for (int j = 0; j < size_int; j++) pIntArray[j] = vecInt[j];
            ...
            if (row.strParseType == "double_array") {
               ...
               for (int j = 0; j < size_double; j++) pDoubleArray[j] = vecDouble[j];
   ...
   if (schema.isUpdate) count = tmp_count;
   ...
   while (cur_schema_node) {
         ...
         while (attribute && attribute->name && attribute->children) {
            if (strcmp((char *)attribute->name, "down") == 0) {
               ...
               for (int i = 0; i < count; i++) {
                  ...
                  if (cur_xml_node->type != XML_ELEMENT_NODE) cur_xml_node = cur_xml_node->next;
      ...
      // PARSE CYCLE
         ...
         for (cur_xml_node = cur_xml_node->children; cur_xml_node; cur_xml_node = cur_xml_node->next) {
            if (strcmp((char *)cur_xml_node->name, strChildName.Data())) continue;
...
int UniParser::ParseCsv2Db(TString csvName, TString schemaPath, bool isUpdate)
{
   ...
   while (cur_schema_node) {
      ...
      // PARSE CYCLE
         ...
         for (int i = 0; i < skip_count; i++) getline(csvFile, cur_line);
...
int UniParser::ParseTxt2Db(TString txtName, TString schemaPath, bool isUpdate)
{
   ...
   while (cur_schema_node) {
      ...
      if (strcmp((char *)cur_schema_node->name, "skip") == 0) {
         ...
         if (strLineCount != "") skip_line_count = atoi(strLineCount.c_str());

         for (int i = 0; i < skip_line_count; i++) getline(txtFile, cur_line);
      }
      ...
      // PARSE CYCLE
         ...
         for (int i = 0; i < skip_count; i++) getline(txtFile, cur_line);
...
int UniParser::ParseTxtNoise2Db(int period_number, TString txtName, TString schemaPath)
{
   ...
   while (getline(txtFile, cur_line)) {
      ...
      while (getline(line_stream, token, ' ')) {
         if (num == 1) run_number = atoi(token.c_str());
         if (num == 2) row_count = atoi(token.c_str());
      ...
      for (int i = 0; i < row_count; i++) {
         ...
         // parse tokens by space separated
         while (getline(line_stream, token, ' ')) {
            if (num == 1) slot_number = atoi(token.c_str());
            ...
            if (num > 1) {
               ...
               if (index_sym == string::npos) {
                  ...
                  if (channel_number == 0) {
                     if (!is_string_number(token)) continue;
                  }
      ...
      */
      ...
      if (pDetectorParameter == NULL) continue;
...
int UniParser::ParseTxt2Struct(TString txtName, TString schemaPath, vector<structParseValue *> &parse_values,
                               vector<structParseSchema> &vecElements, int iVerbose)
{
         ...
         while (getline(txtFile, cur_line)) {
            ...
            while (getline(line_stream, token, delimiter_char)) {
               ...
               if (schema.vecRows[0].strStatementType == "int") {
                  ...
                  if (schema.vecRows[0].strStatementType == "double") {
                     ...
                     if (iVerbose > 0)
                        cout << "ERROR: type of the column is not supported: " << schema.vecRows[0].strStatementType
                             << endl;
                     return -5;
                  }
               }
            ...
            if ((notFirstLine) && (dtSpillPrevious > dtSpillEnd)) {
               if ((!isSkipLines) && (iVerbose > 0))
                  cout << endl
                       << "ERROR: The time sequence of the lines is corrupted. The following lines is skipped:" << endl;
            ...
            // Beam All: "<<st->beam_all<<". Trigger DAQ: "<<st->trigger_daq<<". Trigger All: "<<st->trigger_all<<endl;
...
bool check_element(const string &str, size_t pos, string element)
{
   ...
   if ((pos + element_length) > str_length) return false;

   for (int i = 1; i < element_length; i++) {
      if (str[pos + i] != element[i]) return false;
   }
...
int UniParser::ConvertElogCsv(TString csvName, char separate_symbol)
{
   ...
   while (getline(csvFile, cur_line)) {
      ...
      while (getline(line_stream, token, separate_symbol)) {
         switch (index) {
         case 0: record_id = token; break;
         case 1: record_date = token; break;
         case 2: author = trim(token, "\""); break;
         case 3: record_type = trim(token, "\""); break;
         case 4: break;
         case 5: subject = trim(token, "\""); break;
         case 6: run_number = trim(token, "\""); break;
         case 7: shift_leader = trim(token, "\""); break;
         case 8: trigger_info = trim(token, "\""); break;
         case 9: daq_status = trim(token, "\""); break;
         case 10: magnet_field = trim(token, "\""); break;
         case 11: beam = trim(token, "\""); break;
         case 12: record_comment = trim(token, "\""); break;
         default: {
      ...
      if ((strAuthorLower == "shift") || (strAuthorLower == "none")) author = "";
      if (author != "") {
      ...
      if (type_lower == "other") {
         ...
         if (ind != string::npos) {
            type_lower = "new run";
            if (subject_lower.length() == 7) subject = "";
         }
      }
      ...
      if (strRunNumber != "") {
         if (strRunNumber.substr(0, 3) == "run") strRunNumber = strRunNumber.substr(3);
      ...
      if ((strLeaderLower == "shift") || (strLeaderLower == "none")) shift_leader = "";
      if (shift_leader != "") {
      ...
      if (strTriggerLower == "none") trigger_info = "";
      if (trigger_info != "") {
      ...
      for (int i = 0; i < beam_lower.length(); i++) {
         if (strBeam.empty()) {
            ...
            case 'c': {
               // if "C"
               if (check_element(beam_lower, i, "c")) strBeam = "C";

               break;
            } // case 'c':
            case 'd': {
               // if "d"
               if (check_element(beam_lower, i, "d")) strBeam = "d";
            ...
            case 'p': {
               // if "Pb"
               if (check_element(beam_lower, i, "pb")) strTarget = "Pb";

               break;
            } // case 'p':
            case 'a': {
               // if "Al"
               if (check_element(beam_lower, i, "al")) strTarget = "Al";
         ...
         if (strTarget != "") {
            ...
            if (ind != string::npos) {
               ...
               if (strTargetWidth.empty()) {
                  cout << "ERROR: no target width before 'mm' found: " << beam_lower << " for record id: " << record_id
                       << endl;
                  continue;
               }
            }
