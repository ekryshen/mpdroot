...
private:
   ...
   // Constructor
   UniDbSimulationFile(UniConnection *connUniDb, int file_id, TString file_path, TString generator_name,
                       TString beam_particle, TString *target_particle, double *energy, TString centrality,
                       int *event_count, TString *file_desc, int *file_size, TString *file_md5);
...
public:
   ...
   static UniDbSimulationFile *CreateSimulationFile(TString file_path, TString generator_name, TString beam_particle,
                                                    TString *target_particle, double *energy, TString centrality,
                                                    int *event_count, TString *file_desc, int *file_size,
                                                    TString *file_md5);
   ...
   TString *GetFileDesc()
   {
      if (str_file_desc == NULL)
         return NULL;
      else
         return new TString(*str_file_desc);
   }
   ...
   /// get simulation files corresponding to the specified (vector) conditions and set owner for search_condition to
   /// kTRUE
