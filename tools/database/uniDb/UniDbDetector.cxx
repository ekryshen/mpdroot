...
UniDbDetector::~UniDbDetector()
{
   ...
   if (str_description) delete str_description;
}
...
UniDbDetector *UniDbDetector::CreateDetector(TString detector_name, TString *description)
{
   ...
   TString        sql  = TString::Format("insert into detector_(detector_name, description) "
                                                 "values ($1, $2)");
...
UniDbDetector *UniDbDetector::GetDetector(TString detector_name)
{
   ...
   TString        sql  = TString::Format("select detector_name, description "
                                                 "from detector_ "
                                                 "where lower(detector_name) = lower('%s')",
                                         detector_name.Data());
   TSQLStatement *stmt = uni_db->Statement(sql);
...
int UniDbDetector::DeleteDetector(TString detector_name)
{
   ...
   TString        sql  = TString::Format("delete from detector_ "
...
int UniDbDetector::SetDetectorName(TString detector_name)
{
   ...
   TString        sql  = TString::Format("update detector_ "
                                                 "set detector_name = $1 "
...
void UniDbDetector::Print()
{
   ...
   cout << ". detector_name: " << str_detector_name
        << ". description: " << (str_description == NULL ? "NULL" : *str_description) << endl;
