...
public:
   ...
   static int  setTpcVelocityMap(size_t periodID, size_t runID, std::vector<std::vector<std::vector<double>>> &map); // return ok status
   //TPC Const
   static bool setTpcConsts(int sector_count,double sector_phi_rad,double z_min,double drift_length,int timebin_count, double timebin_length, std::vector<int>& row_count,std::vector<double>& pad_height,std::vector<double>& pad_width,std::vector<int>& pad_count,double ypadplane_offset, double ypadplane2padarea_offset, size_t periodID=0, size_t runID=0);
