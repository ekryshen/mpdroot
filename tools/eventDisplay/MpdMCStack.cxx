...
MpdMCStack::MpdMCStack(const char *name, Int_t iVerbose) : FairTask(name, iVerbose), fEveTrList(new TObjArray(16)) {}
...
void MpdMCStack::Exec(Option_t * /*option*/)
{
   ...
   for (Int_t i = 0; i < fTrackList->GetEntriesFast(); i++) {
      ...
      if (mass >= 0) ene = TMath::Sqrt(mass * mass + Ptot.Mag2());
      ...
      TParticle *P = new TParticle(tr->GetPdgCode(), i, MotherId, -1, -1, -1, Ptot.Px(), Ptot.Py(), Ptot.Pz(), ene,
                                   Vertex.x(), Vertex.z(), Vertex.z(), time);
