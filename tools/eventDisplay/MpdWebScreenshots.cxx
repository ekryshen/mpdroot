...
MpdWebScreenshots::MpdWebScreenshots() : FairTask("MpdWebScreenshots", 0) {}
...
int MpdWebScreenshots::daemonize()
{
   ...
   // Change the current working directory. This prevents the current directory from being locked; hence not being able
   // to remove it.
   if ((chdir("/")) < 0) return -4;
...
void MpdWebScreenshots::sendHeader(const char *Status_code, char *Content_Type, int TotalSize, int socket)
{
   ...
   char *message = (char *)malloc((strlen(head) + strlen(content_head) + strlen(server_head) + strlen(length_head) +
                                   strlen(date_head) + strlen(newline) + strlen(Status_code) + strlen(Content_Type) +
                                   strlen(contentLength) + 28 + sizeof(char)) *
                                  2);
...
{
   if (start >= strlen(input)) return -1;
...
int MpdWebScreenshots::start(int webPort, TString output_dir)
{
   ...
   while (1) acceptConnection(currentSocket, output_dir);
}
