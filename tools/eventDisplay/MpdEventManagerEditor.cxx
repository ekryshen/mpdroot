...
MpdEventManagerEditor::MpdEventManagerEditor(const TGWindow *p, Int_t width, Int_t height, UInt_t options, Pixel_t back)
   : TGedFrame(p, width, height, options | kVerticalFrame, back), fObject(0),
     fEventManager(MpdEventManager::Instance()), fEventTime(nullptr), fCurrentPDG(0), fVizPri(0), fMinEnergy(0),
     fMaxEnergy(0), isMagnetFound(false), iThreadState(0), iEventCount(-1), isStreamSource(false), fCurrentEvent(0)
{
...
void MpdEventManagerEditor::Init()
{
   ...
   fCurrentEvent = new TGNumberEntry(f, 0., 6, -1, TGNumberFormat::kNESInteger, TGNumberFormat::kNEANonNegative,
                                     TGNumberFormat::kNELLimitMinMax, 0, iEventCount - 1);
   ...
   if (iEventCount < 1) fCurrentEvent->SetState(kFALSE);
   ...
   fCurrentPDG = new TGNumberEntry(f1, 0., 12, -1, TGNumberFormat::kNESInteger, TGNumberFormat::kNEAAnyNumber,
...
void MpdEventManagerEditor::ShowRecoPoints(Bool_t is_show)
{
   ...
   if (fEventManager->fgRedrawRecoPointsReqired) fEventManager->GotoEvent(fCurrentEvent->GetIntNumber());
...
void MpdEventManagerEditor::SelectEvent()
{
   ...
   if (!fEventManager->isOnline) {
      ...
      if ((fEventManager->isZDCModule) && (fShowMCPoints->IsOn())) RedrawZDC(false);
...
// clicking 'Update event' button to update event display OR 'Start online display' button to start/stop online event
// display
void MpdEventManagerEditor::UpdateEvent()
{
   ...
   // if ONLINE mode
      ...
      if ((fEventManager->isZDCModule) && (fShowMCPoints->IsOn())) par_online_display->isZDCRedraw = true;
...
void *RunOnlineDisplay(void *ptr)
{
   ...
   for (int i = current_event + 1; ((stream_source) || (i < fEditor->iEventCount)); i++) {
      if (fEditor->iThreadState == 0) break;
      do {
      ...
      // highlight ZDC modules if ZDC present
      if (isZDCRedraw) fEditor->RedrawZDC();
