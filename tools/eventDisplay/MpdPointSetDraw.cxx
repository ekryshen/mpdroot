...
MpdPointSetDraw::MpdPointSetDraw()
   : FairTask("MpdPointSetDraw", 0), fVerbose(0), fEventManager(nullptr), fPointList(nullptr), fq(nullptr), fColor(0),
     fStyle(0)
{
}
...
MpdPointSetDraw::MpdPointSetDraw(const char *name, Color_t color, Style_t mstyle, Int_t iVerbose)
   : FairTask(name, iVerbose), fVerbose(iVerbose), fEventManager(nullptr), fPointList(nullptr), fq(nullptr),
     fColor(color), fStyle(mstyle)
{
}
...
void MpdPointSetDraw::Exec(Option_t * /*option*/)
{
   ...
   for (Int_t i = 0; i < npoints; i++) {
      ...
      if (p != 0) {
         ...
         cout << "CRITICAL ERROR: MpdPointSetDraw::Exec() point is not TObject" << endl;
   }
