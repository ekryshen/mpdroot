...
BmnTdaqSource::BmnTdaqSource() : FairOnlineSource()
{
...
BmnTdaqSource::BmnTdaqSource(const BmnTdaqSource &source)
   : FairOnlineSource(source), strPartitionName(source.GetPartitionName()), strSamplingType(source.GetSamplingType()),
     strSamplingName(source.GetSamplingName()), isDispersion(false), iWait(10000), // ms
     iRepetitions(1), iBufferSize(1),                                              // event count
     iAsync(0), iTimeout(10000),                                                   // ms
     iVerbose(source.GetVerbose()), iEventNumber(0), fEventHeader(source.fEventHeader), fGemDigits(source.fGemDigits),
     fT0Digits(source.fT0Digits), fTof1Digits(source.fTof1Digits)
{
}
...
Bool_t BmnTdaqSource::Init()
{
   ...
   CmdArgBool    lvl1_bits_logic('l', "lvl1-bits-logic",
                                 "logic attribute for the LVL1 bit pattern , AND if given OR otherwise");
   CmdArgInt     lvl1_bits_origin('o', "lvl1-bits-origin", "origin",
                                  "origin of the LVL1 bit pattern, 0 - BEFORE_PRESCALE\n"
   ...
   for (size_t i = 0; i < lvl1_bits.count(); i++) L1_bits.push_back(lvl1_bits[i]);

   vector<string> STREAM_names;
   for (size_t i = 0; i < stream_names.count(); i++) STREAM_names.push_back((const char *)stream_names[i]);

   emon::Logic L1_logic     = !(lvl1_bits.flags() && CmdArg::GIVEN)        ? emon::logic::IGNORE
                              : (lvl1_bits_logic.flags() && CmdArg::GIVEN) ? emon::logic::AND
                                                                           : emon::logic::OR;
   emon::Logic STREAM_logic = !(stream_type.flags() && CmdArg::GIVEN)   ? emon::logic::IGNORE
                              : (stream_logic.flags() && CmdArg::GIVEN) ? emon::logic::AND
                                                                        : emon::logic::OR;

   /*emon::SelectionCriteria criteria(emon::L1TriggerType((unsigned char)(int)lvl1_type, !(lvl1_type.flags() &&
      CmdArg::GIVEN)), emon::SmartBitValue(L1_bits, L1_logic, (emon::Origin)(int)lvl1_bits_origin),
...
Int_t BmnTdaqSource::ReadEvent(UInt_t)
{
   ...
   try {
      // retrieve the event from the buffer, this either blocks (when iAsync == 0) or it will throw NoMoreEvents (when
      // iAsync == 1) you can pass a timeout in milliseconds when in synchronous mode after the timeout, NoMoreEvents
      // will be thrown
      if (iVerbose > 1) {
         if (iAsync)
      ...
      // we actually have to exit here, or an uncatched NotInitialized exception will be thrown on deletion of the
      // iterator
      ers::fatal(ex);
      return 2;
   }
   ...
   if (iVerbose > 0)
      cout << "Event count = " << iEventNumber << " Buffer occupancy = [" << it->eventsAvailable() << "/" << iBufferSize
           << "]" << endl;
   if (iVerbose > 1) {
...
void BmnTdaqSource::Close()
{
   ...
   if (it.get()) ERS_LOG("End of run, monitor dropped " << it->eventsDropped() << " events.");
