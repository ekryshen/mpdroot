...
public:
      ...
      while (1) {
         ...
         for (Int_t i = n - 1; i >= 0; i--) {
            if (regex_match(namelist[i]->d_name, re)) {
               ...
               for (Int_t j = i; i >= 0; i--) free(namelist[i]);
               break;
            }
   ...
   void sampleEvent(EventChannel &cc)
   {
      ...
      while (!isFoundEvent) {
         ...
         if (isNewFile) {
            if (AttachRawFile(strRawData, strCurrentRaw) != 0) return;
            isNewFile = false;
         }
         ...
         if ((iterDigi.header->GetEntriesFast() == 0) ||
             (((BmnEventHeader *)iterDigi.header->At(0))->GetType() == kBMNPEDESTAL))
            continue;
      ...
      // ERS_INFO("Event "<<(uiCurrentEvent+1)<<" size is "<<full_length<<" ("<<event_length<<") bytes"<<" + event id +
      // real length");
...
public:
   ...
   PullSampling *startSampling(const SelectionCriteria &criteria) throw(BadCriteria, NoResources)
   {
...
BmnEventPull::BmnEventPull(TString partition_name, TString sampling_type, TString sampling_name, TString raw_data,
                           int run_period, int max_event, int max_channels, int wait_file_sec, Int_t verbose)
{
...
int BmnEventPull::Exec()
{
   ...
   try {
      ...
      emon::EventSampler temp(partition, address,
                              new BmnPullSamplingFactory(strRawData.Data(), iRunPeriod, iWaitFileSec), iMaxChannels);
      sampler = &temp;
      ...
      sampler->wait();
      sampler = 0;
      if (signalSamplerReceived != 0) ERS_LOG("Shutting down BM@N sampler...");
