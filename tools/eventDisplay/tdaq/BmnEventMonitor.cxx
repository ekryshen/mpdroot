...
BmnEventMonitor::BmnEventMonitor(TString partition_name, TString sampling_type, TString sampling_name, int max_events,
                                 Int_t verbose)
{
...
{
   cout << "HERE!!!!!" << endl;
   if (iVerbose > 1) cout << "BmnEventMonitor::Init()" << endl;
   ...
   if (iVerbose > 1) cout << "BmnEventMonitor::Init() IPC was initialized" << endl;
...
void BmnEventMonitor::Exec(Option_t *option)
{
   ...
   if (iEventCount >= iMaxEvents) return;
   ...
   /* DigiArrays* fDigiArrays = (DigiArrays*) (t.ReadObject(DigiArrays::Class()));
      ...
      if (iVerbose > 0)
          cout<<"Event count = "<<iEventCount<<" Buffer occupancy =
      ["<<it->eventsAvailable()<<"/"<<iBufferSize<<"]"<<endl; if (iVerbose > 1)
      {
          cout<<"Event id = "<<event_id<<"\tEvent length = "<<event_length<<"\tFull size = "<<event.size()<<"
      DWORD"<<endl;
          //fDigiArrays->Dump();
          //usleep(3*1000000);
      }
