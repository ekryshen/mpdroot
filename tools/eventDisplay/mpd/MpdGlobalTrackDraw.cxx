...
MpdGlobalTrackDraw::MpdGlobalTrackDraw()
   : FairTask("MpdGlobalTrackDraw", 0), fTrackList(NULL), fTrPr(NULL), fEventManager(NULL), fEveTrList(NULL),
     fTrList(NULL), MinEnergyLimit(-1.), MaxEnergyLimit(-1.), PEnergy(-1.), fCutProbability(0.8)
{
}

// standard constructor
MpdGlobalTrackDraw::MpdGlobalTrackDraw(const char *name, Int_t iVerbose)
   : FairTask(name, iVerbose), fTrackList(NULL), fTrPr(NULL), fEventManager(NULL), fEveTrList(new TObjArray(16)),
     fTrList(NULL), MinEnergyLimit(-1.), MaxEnergyLimit(-1.), PEnergy(-1.), fCutProbability(0.8)
{
}
...
void MpdGlobalTrackDraw::Exec(Option_t * /*option*/)
{
   ...
   for (Int_t i = 0; i < fTrackList->GetEntriesFast(); i++) {
      ...
      if (fParticlePDG) mass = fParticlePDG->Mass();
      if (mass >= 0) energy = TMath::Sqrt(mass * mass + px * px + py * py + pz * pz);

      // define whether track is primary
      bool isPrimary = kalman_track->GetChi2Vertex() < 10;

      // first inner hit of current track in TPC
      MpdTpcHit *firstTpcHit =
         (MpdTpcHit *)fTpcHitList->UncheckedAt(((MpdKalmanHit *)trHits->UncheckedAt(Np - 1))->GetIndex());

      // TParticle(Int_t pdg, Int_t status, Int_t mother1, Int_t mother2, Int_t daughter1, Int_t daughter2, Double_t px,
      // Double_t py, Double_t pz, Double_t etot, Double_t vx, Double_t vy, Double_t vz, Double_t time)
      TParticle *P = new TParticle(particlePDG, i, -1, -1, -1, -1, px, py, pz, energy, firstTpcHit->GetX(),
                                   firstTpcHit->GetY(), firstTpcHit->GetZ(), firstTpcHit->GetTimeStamp());
      ...
      if (fEventManager->IsPriOnly() && (!isPrimary)) continue;
      // skip particles with unequal PDG if PDG number is set
      if ((fEventManager->GetCurrentPDG() != 0) && (fEventManager->GetCurrentPDG() != particlePDG)) continue;
