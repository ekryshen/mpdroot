...
MpdEmcTowerDraw::MpdEmcTowerDraw()
   : FairTask("MpdEmcTowerDraw", 0), fDigitList(nullptr), fEventManager(nullptr), fq(nullptr),
     fEmcMinEnergyThreshold(0), fVerbose(0), fResetRequiredFlag(kFALSE), fRMinEmc(0), fRMaxEmc(0), fBoxHeight(0),
     fSector1StartAngle(0), fNBoxes(0), fNTubes(0), fNSectors(0), fN3Dbins(0), fEneArr(nullptr),
...
MpdEmcTowerDraw::MpdEmcTowerDraw(const char *name, Double_t emcMinEnergyThreshold, Int_t verbose)
   : FairTask(name, verbose), fDigitList(nullptr), fEventManager(nullptr), fq(nullptr),
     fEmcMinEnergyThreshold(emcMinEnergyThreshold), fVerbose(0), fResetRequiredFlag(kFALSE), fRMinEmc(0), fRMaxEmc(0),
     fBoxHeight(0), fSector1StartAngle(0), fNBoxes(0), fNTubes(0), fNSectors(0), fN3Dbins(0), fEneArr(nullptr),
     fGeoPar(new MpdEmcGeoPar)
{
}
...
InitStatus MpdEmcTowerDraw::Init()
{
   ...
   for (UInt_t i = 0; i < GetN3Dbins(); i++) SetEneArr(i, 0);
...
void MpdEmcTowerDraw::FillEnergyLossArray()
{
   ...
   for (UInt_t iPnt = 0; iPnt < fNhits; iPnt++) {
      ...
      TString binPathGeo =
         FindBoxPathZPhiWithGeoManager((GetRMaxEmc() + GetRMinEmc()) / 2.0, dgt->GetZcenter(), dgt->GetPhiCenter());
      ...
      if ((iCur >= 0) && (iCur < GetN3Dbins()))
         ...
         cout << "MpdEmcTowerDraw::FillEnergyLossArray() wrong path = " << binPath
   ...
   cout << endl
        << endl
        << "MpdEmcTowerDraw::FillEnergyLossArray() Real time " << rtime << " s, CPU time " << ctime << " s" << endl;
}
...
void MpdEmcTowerDraw::DrawBoxTowers()
{
         ...
         for (UInt_t tube = 0; tube < (UInt_t)sectorNode->GetVolume()->GetNdaughters(); tube++) {
            ...
            for (UInt_t module0 = 0; module0 < (UInt_t)tubeNode->GetVolume()->GetNdaughters(); module0++) {
               ...
               for (UInt_t box = 0; box < (UInt_t)moduleNode->GetVolume()->GetNdaughters(); box++) {
                  ...
                  TString   curPath =
                     Form("/cave_1/emc1Chamber1_1/%s/%s/%s/%s/%s", chHNode->GetName(), sectorNode->GetName(),
                          tubeNode->GetName(), moduleNode->GetName(), boxNode->GetName());
   ...
   cout << endl
        << endl
        << "MpdEmcTowerDraw::DrawSectorTowers() Real time " << rtime << " s, CPU time " << ctime << " s" << endl;
   return;
}
...
void MpdEmcTowerDraw::ResetBoxTowers()
{
   ...
   cout << endl
        << endl
        << "MpdEmcTowerDraw::ResetBoxTowers() Real time " << rtime << " s, CPU time " << ctime << " s" << endl;
   return;
}
...
UInt_t MpdEmcTowerDraw::FindN3DbinBoxId(TString path)
{
   ...
   return (GetNBoxes() * GetNTubes() * GetNSectors() * (chHId - 1)) + (GetNBoxes() * GetNTubes() * (sectorId - 1)) +
          (GetNBoxes() * (tubeId - 1)) + (boxId - 1);
}
...
TString MpdEmcTowerDraw::FindPathN3DbinBoxId(UInt_t id)
{
   ...
   if (boxId > GetNMiddleBoxesInSuperMod())
      ...
      return Form("/cave_1/emc1Chamber1_1/emc1ChH_%d/emc1Sector_%d/emc1Tube_%d/emc1Module_1/emc1_box_%d", chHId,
                  sectorId, tubeId, boxId);
}
...
TString MpdEmcTowerDraw::FindBoxPathZPhi(Double_t z, Double_t phi)
{
   ...
   if (chHId == 1)
      phiAbs = phi > GetSector1StartAngle() ? phi - GetSector1StartAngle()
                                            : (TMath::TwoPi() + (phi - GetSector1StartAngle()));
   else
      phiAbs =
         phi > GetSector1EndAngle() ? (TMath::TwoPi() - (phi - GetSector1EndAngle())) : (GetSector1EndAngle() - phi);
   ...
   UInt_t   supModRowIdByPhi =
      UInt_t(GetNRowInSuperModByPhi() * GetNSectors() * GetNRowsByPhi() * phiB / (TMath::TwoPi())) + 1;
   UInt_t   supModRowIdByZ;
   Double_t tubeZ = (tubeId - UInt_t((tubeId - 1) / GetNRowsByZ()) * GetNRowsByZ() - 1) * GetLenSuperModule();
   if (chHId == 1)
      supModRowIdByZ =
         UInt_t((GetLenSector() - TMath::Abs(z) - tubeZ) / GetLenSuperModule() * GetNRowInSuperModByZ()) + 1;
   else
   ...
   else if (supModRowIdByPhi == GetNRowInSuperModByPhi()) // bt_box 4-6
