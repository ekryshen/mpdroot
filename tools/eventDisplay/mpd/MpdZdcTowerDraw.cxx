...
MpdZdcTowerDraw::MpdZdcTowerDraw()
   : FairTask("MpdZdcTowerDraw", 0), fDigitList(nullptr), fEventManager(nullptr), fq(nullptr), fVerbose(0),
     fShadow(kFALSE), fResetRequiredFlag(kFALSE), fEneArr(nullptr), fZdcMinEnergyThreshold(0)
...
MpdZdcTowerDraw::MpdZdcTowerDraw(const char *name, Double_t zdcMinEnergyThreshold, Bool_t shadow, Int_t verbose)
   : FairTask(name, verbose), fDigitList(nullptr), fEventManager(nullptr), fq(nullptr), fVerbose(verbose),
     fShadow(shadow), fResetRequiredFlag(kFALSE), fEneArr(nullptr), fZdcMinEnergyThreshold(zdcMinEnergyThreshold)
{
}
...
InitStatus MpdZdcTowerDraw::Init()
{
   ...
   for (UInt_t i = 0; i < GetNumModules() * 2; i++) SetEneArr(i, 0);
   ...
   if (IsActive()) {
      ...
      if (fEventManager->fgShowRecoPointsIsShow) {
         ...
         if (GetResetRequiredFlag()) {
            for (UInt_t i = 0; i < GetNumModules() * 2; i++) SetEneArr(i, 1);
...
void MpdZdcTowerDraw::DrawTowers()
{
   ...
   for (UInt_t zdcId = 0; zdcId < 2; zdcId++) {
      ...
      for (UInt_t module0 = 0; module0 < (UInt_t)zdcNode->GetVolume()->GetNdaughters(); module0++) {
         ...
         // cout<<"GetEneArrValue(iCur) = "<< GetEneArrValue(iCur)<<"; box->GetName() = "<<box->GetName()<<";
         // moduleNode->GetMotherVolume()->GetName() = "<<moduleNode->GetMotherVolume()->GetName()<<";
         // moduleNode->GetName() = "<<moduleNode->GetName()<<"; moduleNode->GetNumber() =
         // "<<moduleNode->GetNumber()<<"; iCur = "<<iCur<<endl;
         ...
         if (GetEneArrValue(iCur) == 0) moduleVolumeCopy->SetVisibility(kFALSE);

         // the internal structure of Module does note alow to change it's shape, so we delete it for the towers
         // visualization
         ...
         if (GetShadowFlag()) RecursiveChangeNodeTransparent(moduleNodeCopy, 98);
