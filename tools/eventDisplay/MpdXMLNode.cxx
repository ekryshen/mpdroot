...
MpdXMLNode::MpdXMLNode(const MpdXMLNode &other) : MpdXMLNode(other.GetName(), other.GetValue())
{
...
void MpdXMLFile::ExportNode(XMLNodePointer_t &nodePointer, TXMLEngine &engine, const MpdXMLNode &node) const
{
   for (int i = 0; i < node.GetNChildren(); i++) {
      ...
      for (int j = 0; j < node.GetChild(i)->GetNAttributes(); j++) {
         engine.NewAttr(child, 0, node.GetChild(i)->GetAttrib(j)->GetName(),
                        node.GetChild(i)->GetAttrib(j)->GetValue());
      }
