...
MpdBoxSetDraw::MpdBoxSetDraw()
   : FairTask("MpdBoxSetDraw", 0), fVerbose(0), fList(NULL), fEventManager(NULL), fManager(NULL), fq(NULL), fX(0.3),
     fY(0.3), fZ(0.3), fTimeWindowPlus(0.), fTimeWindowMinus(0.), fStartTime(0.), fUseEventTime(kTRUE),
     isRedraw(kFALSE), fStartFunctor(), fStopFunctor()
{
}

// -----   Standard constructor   ------------------------------------------
MpdBoxSetDraw::MpdBoxSetDraw(const char *name, Int_t iVerbose)
   : FairTask(name, iVerbose), fVerbose(iVerbose), fList(NULL), fEventManager(NULL), fManager(NULL), fq(NULL), fX(0.3),
     fY(0.3), fZ(0.3), fTimeWindowPlus(0.), fTimeWindowMinus(0.), fStartTime(0.), fUseEventTime(kTRUE),
     isRedraw(kFALSE), fStartFunctor(), fStopFunctor()
{
}
...
void MpdBoxSetDraw::Exec(Option_t * /*option*/)
{
   ...
   if (FairRunAna::Instance()->IsTimeStamp()) {
      ...
      fList = fManager->GetData(GetName(), fStartFunctor, fStartTime, fStopFunctor,
...
void MpdBoxSetDraw::AddBoxes(MpdBoxSet *set, TObject *obj, Int_t i)
{
   ...
   if (fVerbose > 2)
      cout << "MpdBoxSetDraw::Init() Add point " << i << ": " << point.X() << " " << point.Y() << " " << point.Z()
           << " " << endl;
}
