...
MpdEventManager *MpdEventManager::Instance()
{
   return fgRinstance;
}
...
MpdEventManager::MpdEventManager()
   : TEveEventManager("EventManager", ""), fRunAna(FairRunAna::Instance()), EveMCPoints(nullptr), EveMCTracks(nullptr),
     EveRecoPoints(nullptr), EveRecoTracks(nullptr), background_color(1), isDarkColor(true), isZDCModule(nullptr),
     fgShowRecoPointsIsShow(false), fgRedrawRecoPointsReqired(false), fEventEditor(nullptr), fRPhiView(0), fRhoZView(0),
     fMulti3DView(0), fMultiRPhiView(0), fMultiRhoZView(0), fRPhiMng(nullptr), fRhoZMng(nullptr), fRPhiGeomScene(0),
     fRhoZGeomScene(0), fAxesPhi(nullptr), fAxesRho(nullptr), fRPhiPlane{0, 0, 1, 0}, fRhoZPlane{-1, 0, 0, 0},
     fEvent(0), iCurrentEvent(0), fPriOnly(kFALSE), fCurrentPDG(0), fMinEnergy(0), fMaxEnergy(25), fEvtMinEnergy(0),
     fEvtMaxEnergy(12), fLastUsedColor(2001), fXMLConfig("")
{
   fgRinstance = this;
...
// return integer value of color by string storing color number, RGB triple or color name (e.g. kBlue, RGB(51,63,73),
// RGB(#0039E1)) support following MAIN COLOR NAMESET + some additional colors as examples: kWhite, kBlack, kGgray,
...
// kYellow, kOrange (желтый с оттенком), kOrange+2 (оранжевый кор.), kOrange+1 (светло-оранжевый кор.), kOrange+7
// (выделенно-оранжевый) kRed, kViolet, kMagenta (бардовый), kMagenta-6 (светло-бардовый), kPink (темно-розовый)
Int_t MpdEventManager::StringToColor(TString color)
{
   ...
   if (color.BeginsWith("rgb")) {
      ...
      if (triple[0] == '#') {
         if (triple.Length() < 7) {
            cout << triple
                 << " - hex triple size after '#' isn't correct (should have 6 symbols). Color set to default blue"
                 << endl;
            return 600;
         }
   ...
   if (color.Contains("k")) {
      ...
      if (sign_index == kNPOS) sign_index = color.First('-');
      ...
      Int_t color_value;
      if (color_name.EqualTo("kwhite"))
         color_value = 0;
      else if (color_name.EqualTo("kblack"))
         color_value = 1;
      else if (color_name.EqualTo("kgray"))
         color_value = 920;
      else if (color_name.EqualTo("kred"))
         color_value = 632;
      else if (color_name.EqualTo("kgreen"))
         color_value = 416;
      else if (color_name.EqualTo("kblue"))
         color_value = 600;
      else if (color_name.EqualTo("kyellow"))
         color_value = 400;
      else if (color_name.EqualTo("kmagenta"))
         color_value = 616;
      else if (color_name.EqualTo("kcyan"))
         color_value = 432;
      else if (color_name.EqualTo("korange"))
         color_value = 800;
      else if (color_name.EqualTo("kspring"))
         color_value = 820;
      else if (color_name.EqualTo("kteal"))
         color_value = 840;
      else if (color_name.EqualTo("kazure"))
         color_value = 860;
      else if (color_name.EqualTo("kviolet"))
         color_value = 880;
      else if (color_name.EqualTo("kpink"))
         color_value = 900;
      else if (color_name.EqualTo("kkhaki"))
         color_value = 403;
      else {
...
void MpdEventManager::InitColorStructure()
{
   ...
   TString coloring_xml_path = "$VMCWORKDIR/tools/eventDisplay/eventDisplay.xml";
   TString coloring_xsd_path = "$VMCWORKDIR/tools/eventDisplay/coloring.xsd";
            ...
            if ((strcmp((char *)cur_node->name, "text") != 0) // skipping elements with no attributes
               ...
               if (strcmp((char *)cur_node->name, "particle") == 0) {
                  ...
                  while (attribute) {
                     ...
                     if (strcmp((char *)attribute->name, "pdg") == 0) strPDG = (char *)attr_value;
                     if (strcmp((char *)attribute->name, "color") == 0) strColor = (char *)attr_value;
                  ...
                  if ((strPDG != "") && (strColor != "")) fPDGToColor[strPDG.Atoi()] = StringToColor(strColor);
               ...
               while (attribute) {
                  ...
                  if (gVisualizationColoring == selectedColoring) {
                     ...
                     if (strcmp((char *)attribute->name, "color") == 0) level_coloring->fill_color = (char *)attr_value;
...
void MpdEventManager::Init(Int_t visopt, Int_t vislvl, Int_t maxvisnds)
{
   ...
   if (!isDarkColor) gEve->GetDefaultViewer()->GetGLViewer()->UseLightColorSet();
   ...
   if (!isDarkColor) fMulti3DView->GetGLViewer()->UseLightColorSet();
...
void MpdEventManager::SetViewers(TEveViewer *RPhi, TEveViewer *RhoZ)
{
   ...
   if (!isDarkColor) RPhi->GetGLViewer()->UseLightColorSet();
   ...
   if (!isDarkColor) RhoZ->GetGLViewer()->UseLightColorSet();
   ...
   for (UInt_t i = 0; i < vecSelectedColoring.size(); i++) {
      ...
      if (selected_coloring->isRecursiveColoring) {
         for (int j = 0; j < curVolume->GetNdaughters(); j++) {
            ...
            if (child->GetNdaughters() != 0) RecursiveChangeNodeProperty(child, curColor, curTransparency);
         } // for (int j = 0; j < curVolume->GetNdaughters(); j++)
      }    // if (selected_coloring->isRecursiveColoring)
...
void MpdEventManager::SetTransparentGeometry(bool is_on)
{
   ...
   case selectedColoring: {
      ...
      for (UInt_t i = 0; i < vecSelectedColoring.size(); i++) {
         ...
         if (!is_on) curTransparency = selected_coloring->detector_transparency;
         ...
         for (int j = 0; j < curVolume->GetNdaughters(); j++) {
            ...
            if (child->GetNdaughters() != 0) RecursiveChangeNodeTransparent(child, curTransparency);
...
void MpdEventManager::LevelChangeNodeProperty(TGeoNode *node, int level)
{
   for (int i = 0; i < node->GetNdaughters(); i++) {
      ...
      if (level < (int)vecLevelColoring.size()) {
         ...
         if (child->GetNdaughters() != 0) LevelChangeNodeProperty(child, ++level);
      } // if (level < arr_size)
   }
}
...
bool MpdEventManager::ValidateXml(const char *XMLFileName, const char *XSDFileName)
{
   ...
   if (doc == nullptr)
      ...
      xmlSchemaSetValidErrors(cvalid, (xmlSchemaValidityErrorFunc)fprintf, (xmlSchemaValidityWarningFunc)fprintf,
                              stderr);
   ...
   if (schema != nullptr) xmlSchemaFree(schema);
...
MpdEventManager::~MpdEventManager()
{
   if (!vecSelectedColoring.empty()) {
      for (UInt_t i = 0; i < vecSelectedColoring.size(); i++) delete (vecSelectedColoring[i]);
      vecSelectedColoring.clear();
   }
   if (!vecLevelColoring.empty()) {
      for (UInt_t i = 0; i < vecLevelColoring.size(); i++) delete (vecLevelColoring[i]);
...
{
   if (fPDGToColor.find(pdg) != fPDGToColor.end()) return fPDGToColor[pdg];
...
void MpdEventManager::LoadXMLSettings()
{
   ...
   for (int i = 0; i < xml->GetNChildren(); i++) {
      ...
      if (nodename.EqualTo("Detectors")) {
         ...
         if (top_xml != nullptr) LoadXMLDetector(top, top_xml);
   ...
   if (xml->GetNChildren() > 0 && !recursive) {
      for (int i = 0; i < node->GetNdaughters(); i++) {
         ...
         for (int j = 0; j < xml->GetNChildren(); j++) {
            ...
            if (subnode_name == subdetector_name) LoadXMLDetector(node->GetDaughter(i), subnode);
         }
      }
   }
}
