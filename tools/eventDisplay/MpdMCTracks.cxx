...
MpdMCTracks::MpdMCTracks()
   : FairTask("MpdMCTracks", 0), fTrackList(NULL), fTrPr(NULL), fEventManager(NULL), fEveTrList(NULL), fTrList(NULL),
     MinEnergyLimit(-1.), MaxEnergyLimit(-1.), PEnergy(-1.)
{
}
...
MpdMCTracks::MpdMCTracks(const char *name, Int_t iVerbose)
   : FairTask(name, iVerbose), fTrackList(NULL), fTrPr(NULL), fEventManager(NULL), fEveTrList(new TObjArray(16)),
     fTrList(NULL), MinEnergyLimit(-1.), MaxEnergyLimit(-1.), PEnergy(-1.)
{
}
...
void MpdMCTracks::Exec(Option_t * /*option*/)
{
   ...
   for (Int_t i = 0; i < fTrackList->GetEntriesFast(); i++) {
      ...
      LOG(debug3) << "PEnergy " << PEnergy << " Min " << fEventManager->GetMinEnergy() << " Max "
                  << fEventManager->GetMaxEnergy();
