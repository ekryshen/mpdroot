...
TH2F *CreateHistogram2(const char *name, const char *xtitle, const char *ytitle, int nbinx, Float_t firstBinX,
                       Float_t lastBinX, int nbiny, Float_t firstBinY, Float_t lastBinY)
{
...
TH3F *CreateHistogram3(const char *name, const char *xtitle, const char *ytitle, const char *ztitle, int nbinx,
                       Float_t firstBinX, Float_t lastBinX, int nbiny, Float_t firstBinY, Float_t lastBinY, int nbinz,
                       Float_t firstBinZ, Float_t lastBinZ)
{
...
void toDirectory(const char *dir)
{
   ...
   for (UInt_t i = 0; i < splitedPath.size(); i++) {
      ...
      if (!nextDir) Error("QAHelpers::toDir", "Can't create/find directory");
      d = nextDir;
   }
   gFile->cd(dir);
}
...
std::vector<std::string> pathToList(const char *path)
{
   ...
   while (*index) {
      for (; *index != 0 && *index != '/'; index++)
         ;
      if (begin != index) result.push_back(std::string(begin, index - begin));
      for (; *index != 0 && *index == '/'; index++)
         ; // if several delimeters then skip them all
      begin = index;
   }
   return result;
}
...
TH1F *CreateHistogram1(const char *name, const std::string &suffix, const char *xtitle, int nbin, Float_t firstBin,
                       Float_t lastBin)
{
   std::string fullName = std::string(name) + suffix;
   return CreateHistogram1(fullName.c_str(), xtitle, nbin, firstBin, lastBin);
}
TH2F *CreateHistogram2(const char *name, const std::string &suffix, const char *xtitle, const char *ytitle, int nbinx,
                       Float_t firstBinX, Float_t lastBinX, int nbiny, Float_t firstBinY, Float_t lastBinY)
{
   std::string fullName = std::string(name) + suffix;
...
TH3F *CreateHistogram3(const char *name, const std::string &suffix, const char *xtitle, const char *ytitle,
                       const char *ztitle, int nbinx, Float_t firstBinX, Float_t lastBinX, int nbiny, Float_t firstBinY,
                       Float_t lastBinY, int nbinz, Float_t firstBinZ, Float_t lastBinZ)
{
   std::string fullName = std::string(name) + suffix;
   return CreateHistogram3(fullName.c_str(), xtitle, ytitle, ztitle, nbinx, firstBinX, lastBinX, nbiny, firstBinY,
                           lastBinY, nbinz, firstBinZ, lastBinZ);
}