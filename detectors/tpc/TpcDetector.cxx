...
TpcDetector::TpcDetector(const char *Name, Bool_t Active) : FairDetector(Name, Active)
{
...
void TpcDetector::Register()
{
   ...
   /** This will create a branch in the output tree called  TpcDetectorPoint, setting the last parameter to kFALSE
      means:
...
{
   if (iColl == 0)
      return fTpcPointCollection;
   else
      return nullptr;
}

TpcPoint *TpcDetector::AddHit(Int_t trackID, Int_t detID, TVector3 pos, TVector3 mom, Double_t time, Double_t length,
                              Double_t eLoss)
{
   TClonesArray &clref = *fTpcPointCollection;
   Int_t         size  = clref.GetEntriesFast();
   return new (clref[size]) TpcPoint(trackID, detID, pos, mom, time, length, eLoss);
}
