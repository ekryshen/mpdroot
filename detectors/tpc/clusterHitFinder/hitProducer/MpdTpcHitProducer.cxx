   ...
   : FairTask("TPC Hit Producer"), fModular(0), fPersistance(kFALSE)
{
   ...
   if (!fSecGeo) Fatal("MpdTpcHitProducer::MpdTpcHitProducer", " !!! Wrong geometry type !!! ");
...
void MpdTpcHitProducer::Exec(Option_t *opt)
{
   ...
   cout << "\n-I- MpdTpcHitProducer: Event No. " << FairRun::Instance()->GetEventHeader()->GetMCEntryNumber() << " "
        << ++eventNo << endl;
   ...
   if (first) {
      ...
      if (nSens) {
         ...
         if (sensVol0->getShape() == "PGON") {
            ...
            fSecGeo->SetNofRows(nLays);
            fSecGeo->SetPadHeight(dR);
            fSecGeo->SetMinY(rMin);
         }
         cout << " *** TPC sensitive volume: " << sensVol0->GetName() << " " << rMin << " " << rMax << " " << fZtpc
              << " " << nLays << " " << dR << endl;
      } else {
   ...
   for (Int_t i = layMax; i >= 0; --i) {
      ...
      for (Int_t j = 0; j < nHits; ++j) {
         ...
         // cout << i << " nMerge: " << nMerge << " " << hit0->GetRphi() << " " << hit0->GetZ() << " " << hit0->GetR()
         // << endl;
   ...
   Double_t dist, Int_t index) Double_t meas[2] = {hit->GetRphi(), hit->GetZ()}; Double_t err[2] = {hit->GetRphiErr(),
   hit->GetZerr()}; Double_t cossin[2] = {1., 0.}; MpdKalmanHit *hitK = new ((*fKHits)[nKh++])
       MpdKalmanHit(lay*1000000+nKh-1, 2, MpdKalmanHit::kFixedR, meas, err, cossin, hit->GetEdep()/hit->GetStep(),
   hit->GetR(), hit->GetRefIndex());
   ...
   */
   ...
   // Event summary
   cout << "-I- MpdTpcHitProducer: " << nPoints << " TpcPoints, " << fHitArray->GetEntriesFast() << " Hits created."
        << endl;
}
...
void MpdTpcHitProducer::ExecModular()
{
   ...
   for (Int_t j = 0; j < nPoints; ++j) {
      ...
      point->Position(p3);
      Int_t padID = fSecGeo->Global2Local(p3, p3local);
      // if (padID < 0) {cout << j << " " << p3.X() << " " << p3.Y() << " " << p3local.X() << " " << p3local.Y() <<
      // endl; continue; }// outside sector boundaries
      if (padID < 0) continue; // outside sector boundaries
      point->Momentum(pmom3);
      pmom3loc.SetXYZ(0., 0., 0.);
      // if (point->GetTrackID() <= 2 && point->GetTrackID() > 0) printf("%2d %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %6d
      // %4d %4d\n", point->GetTrackID(), p3.X(), p3.Y(), p3.Z(), p3local.X(), p3local.Y(), p3local.Z(), padID,
      // TpcPadID::numberToPadID(padID).row(), TpcPadID::numberToPadID(padID).sector());
      lay    = fSecGeo->PadRow(padID);
      layMax = TMath::Max(lay, layMax);
      // MpdTpcHit *hit = AddRawHit(j, point->GetDetectorID(), p3, p3err, j, point->GetTrackID());
   ...
   for (mit = midIndx.begin(); mit != midIndx.end(); ++mit) {
      ...
      for (Int_t i3 = 0; i3 < nh; ++i3) {
         ...
         Int_t sec = fSecGeo->Sector(h->GetDetectorID());
         if (sec != sec0 || i3 > lastIndx) {
            ...
            for (Int_t jj = i3; jj < nh; ++jj) {
               ...
               sec           = fSecGeo->Sector(h1->GetDetectorID());
         ...
         Double_t ypad = fSecGeo->LocalPadPosition(h->GetDetectorID()).Y(); // padrow position
         if (TMath::Abs(ypad - ypad0) < 0.1) {
            // The same padrow
            if (!ok) {
               fHitArray->RemoveAt(hindx[ord[i3]]);
               continue;
            }
            times[i3] = times[i3 - 1];
            ys[i3]    = ys[i3 - 1];
         ...
         /*cout << " np, ibeg, y, x, z: " << np << " " << ibeg << " " << yy[ibeg] << " " << xx[ibeg] << " " << zz[ibeg]
      << endl; cout << ys[i3] << " " << times[i3] << " " << zs[i3] << endl;*/
         ...
         if (dir > 0)
            h->SetLength(h->GetLength() + dl); // going outward
         else
            h->SetLength(h->GetLength() - dl); // inward
      ...
      for (Int_t i3 = 0; i3 < nh; ++i3) {
         ...
         h->SetLocalZ(zs[i3]);
         h->LocalPosition(p30);
         fSecGeo->Local2Global(fSecGeo->Sector(padID), p30, p3);
         h->SetPosition(p3);
      }
      delete[] times;
      delete[] ys;
      ...
      for (Int_t i3 = 0; i3 < nh; ++i3) {
         ...
         for (Int_t i31 = i3 + 1; i31 < nh; ++i31) {
            ...
            if (fSecGeo->PadRow(padID) != fSecGeo->PadRow(padID0)) break; // in different padrows
            if (fSecGeo->Sector(padID) != fSecGeo->Sector(padID0)) break; // in different sectors
            hit->LocalPosition(p3local);
            hit->Position(p3);
            if (TMath::Abs(p3extr.Z() - p3local.Z()) > 2.5) break;
            if (TMath::Abs(p3extr.X() - p3local.X()) > 2.5) break;
            // Merge hits
            // cout << " Merge: " << p3extr.X() << " " << p3extr.Y() << " " << p3extr.Z() << " " << p3local.X() << " "
            // << p3local.Y() << " " << p3local.Z() << endl;
            p3local0 += p3local;
            p30 += p3;
            leng += hit->GetLength();
...
//---------------------------------------------------------------------------
MpdTpcHit *MpdTpcHitProducer::AddRawHit(Int_t indx, Int_t detUID, const TVector3 &posHit, const TVector3 &posHitErr,
                                        Int_t pointIndx, Int_t trackIndx)
{
...
//---------------------------------------------------------------------------
Bool_t MpdTpcHitProducer::Interpolate(Int_t np, Int_t &ibeg, Double_t *yp, Double_t *xp, Double_t *zp, Double_t y0,
                                      Double_t &dir, Double_t &xhit, Double_t &zhit)
{
   ...
   for (Int_t j = ibeg; j < np; ++j) {
      ...
      if (dy0 * dy2 > 0.0) {
         ...
         if (TMath::Abs(dy2) < TMath::Abs(dy0) && j < np - 3) {
            ++ibeg;
            continue;
         }
         // ok = kFALSE;
         ...
         break;
      } else if ((yp3[1] - yp3[0]) * (yp3[2] - yp3[1]) < 0)
         ok = kFALSE; // not monotone
      break;
   }
   // Go to the left
   if (!ok) {
      for (Int_t j = ibeg; j > -1; --j) {
         ...
         if (dy0 * dy2 > 0.0) {
            // Both points on the same side from y0
            if (TMath::Abs(dy0) < TMath::Abs(dy2) && j > 2) {
               --ibeg;
               continue;
            }
   ...
   Double_t xEdge = (y0 + fSecGeo->GetMinY()) * TMath::Tan(fSecGeo->Dphi() / 2) + 0.2; // safety margin 0.2
...
void MpdTpcHitProducer::ExecNew()
{
   ...
   for (map<Int_t, map<Double_t, Int_t>>::iterator mit = idmap.begin(); mit != idmap.end(); ++mit) {
      ...
      for (map<Double_t, Int_t>::iterator mit1 = aaa.begin(); mit1 != aaa.end(); ++mit1) {
         ...
         point->Position(p3);
         Int_t padID = fSecGeo->Global2Local(p3, p3loc), isec = -1;
         if (padID < 0) continue; // outside sector boundaries
         lay = -1;
         if (padID >= 0) {
            lay  = fSecGeo->PadRow(padID);
            isec = fSecGeo->Sector(padID);
         }
         ...
         if (padID >= 0) {
            ...
            if (pmom3.Mag() > 1.e-6) {
               ...
               pmom3.SetMag(0.001);
               p3extr += pmom3;
               fSecGeo->Global2Local(p3extr, pmom3loc, isec);
               pmom3loc -= p3loc;
               if (pmom3loc[1] < -1.e-7)
                  idir = -1; // going inward
               else if (pmom3loc[1] > 1.e-7)
                  idir = 1;
            }
         }
         hit.SetFlag(idir);
      ...
      if (hitMap.size()) {
         MpdTpcHit htmp;
         htmp.SetDetectorID(fSecGeo->PadID(30, 0));
      ...
      for (map<Double_t, MpdTpcHit>::iterator mit1 = hitMap.begin(); mit1 != hitMap.end(); ++mit1) {
         ...
         if (isec != isec0 || idir != idir0) {
            ...
            if (xyzloct[0].size() < 2)
               mitb = mite = hitMap.lower_bound(xyzloct[3].front()); // Only one point
            else {
            ...
            for (; mitt != mitend; ++mitt) {
               ...
               if (mitb == mite) {
                  ...
                  p3loc[1] = fSecGeo->LocalPadPosition(padID).Y(); // padrow position
                  // if (p3loc[1] < ylocMinMax[0] || p3loc[1] > ylocMinMax[1]) continue; // curling track
                  if (p3loc[1] < ylocMinMax[0])
                     p3loc[1] = ylocMinMax[0];
                  else if (p3loc[1] > ylocMinMax[1])
                     p3loc[1] = ylocMinMax[1];

                  time = yt->Eval(p3loc[1]);
                  if ((idir0 == 0 && time > xyzloct[3].back()) || time < xyzloct[3].front() - dt ||
                      time > xyzloct[3].back() + dt)
                  ...
                  if (mithit != hitMap.end())
                     hitok = &mithit->second;
                  else
                     hitok = &hitMap.rbegin()->second;
               }
               lay0 = lay;
               fSecGeo->Local2Global(fSecGeo->Sector(padID), p3loc, p3);
               if (fSecGeo->Global2Local(p3, p30) < 0) continue; // cross-check
               ...
               if (hitp->GetStep() > maxStep) {
                  Double_t phi = fSecGeo->SectorAngle(isec0);
                  // Double_t phi = secGeo->SectorAngle(secGeo->Sector(padID));
                  TVector3  sec3(TMath::Cos(phi), TMath::Sin(phi), 0.0);
                  Double_t  padh  = (lay < fSecGeo->NofRowsReg(0)) ? fSecGeo->PadHeight(0) : fSecGeo->PadHeight(1);
