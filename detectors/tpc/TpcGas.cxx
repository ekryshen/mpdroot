...
TpcGas::TpcGas()
   : _E(0), _B(0), _T(293.), _p(1024.), _VDrift(0), _Dl(0), _Dt(0), _k(0), _W(0), _CSD(0), _CSDNorm(0), _CSDEpol(0)
{
}

TpcGas::TpcGas(double const E, double const B, double const T, double const p, double const VDrift, double const Dl,
               double const Dt, double const k, double const W, const std::vector<double> &CSD, double const CSDEpol)
   : _E(E), _B(B), _T(T), _p(p), _VDrift(VDrift), _Dl(Dl), _Dt(Dt), _k(k), _W(W), _CSD(CSD), _CSDNorm(0),
     _CSDEpol(CSDEpol)
{
}

TpcGas::TpcGas(const std::string &Filename, double const E) : _E(E)
{
   // Reading the GasFile
   std::ifstream infile(Filename.c_str(), std::fstream::in);
   if (!infile.good()) Fatal("TpcGas::TpcGas", "Input File is not found");
   ...
   if (nCSDFile < 2) Fatal("TpcGas::TpcGas", "Number of cluster sizes too small in File.\nExecution aborted.");
   ...
   if (!infile.good()) Fatal("TpcGas::TpcGas", "Could not read cluster size distribution in File.\nExecution aborted.");
   infile.close();
   // file is read
   ...
   if (inTable < 0 || inTable > noent - 1) Warning("TpcGas::TpcGas", "E field out of the range defined in input file");
...
int TpcGas::GetRandomCSUniform() const
{
   double r = gRandom->Uniform();
   while (r > _CSDNorm) r = gRandom->Uniform();

   return GetRandomCS(r);
}

// return wrong number if r > overall sum
int TpcGas::GetRandomCS(double const r) const
{
   UInt_t i   = 0;
   double sum = _CSD[0];
...
void TpcGas::SetCSD(const std::vector<double> &CSD)
{
   _CSD = CSD;
}
...
int TpcGas::ReadGasBegin(std::ifstream *const pinfile)
{
...
int TpcGas::ReadGasArrays(std::ifstream *const pinfile, int const noent, double *const e, double *const vdrift,
                          double *const dt, double *const dl, double *const k)
{
   int nCSDFile = 0; // Nr. of ClusterSizes
   for (int i = 0; i < noent; i++) (*pinfile) >> e[i];
   (*pinfile).ignore(256, ':');
   for (int i = 0; i < noent; i++) (*pinfile) >> vdrift[i];
   (*pinfile).ignore(256, ':');
   for (int i = 0; i < noent; i++) (*pinfile) >> dt[i];
   (*pinfile).ignore(256, ':');
   for (int i = 0; i < noent; i++) (*pinfile) >> dl[i];
   (*pinfile).ignore(256, ':');
   for (int i = 0; i < noent; i++) (*pinfile) >> k[i];
...
const double TpcGas::LinExpolation(double const inTable, const double *const table, int const nTable)
{
   if (nTable == 1)
      return (table[0]);
   else if (inTable <= 0)
      ...
      for (int i = 1; i < nTable; i++) {
         if (i >= inTable) return (table[i] - (i - inTable) * (table[i] - table[i - 1]));
      }
