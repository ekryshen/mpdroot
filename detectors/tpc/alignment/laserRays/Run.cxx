...
Run::Run(Alignment &inAlignment, Debug const &inDebug) : vAlignment(inAlignment), vDebug(inDebug) {}
Run::~Run() {}
...
void Run::Calibration(const char *TrackFilename)
{
   ...
   while (!feof(fp)) {
      PointsOnTrack = FileHelper::ReadPointsOnTrack(fp, TrackFilename, "Calibration");
      if (PointsOnTrack == 0) {
         break;
      }
      ...
      for (int k = 0; k < PointsOnTrack; k++) {
         for (int i = 0; i < 3; i++) {
            ...
            if (readNumber == 0) {
               const char *coordinate = (i == 0 ? "X" : (i == 1 ? "Y" : (i == 2 ? "Z" : "Unknown coordinate")));
               std::cout << TrackFilename << " : "
...
void Run::Measurement(const char *inputName, const char *outputName)
{
   ...
   while (!feof(inputStream)) {
      PointsOnTrack = FileHelper::ReadPointsOnTrack(inputStream, inputName, "Measurement");
      if (PointsOnTrack == 0) {
         break;
      }
      if (!FileHelper::WritePointsOnTrack(outputStream, PointsOnTrack, outputName, "Measurement")) {
         break;
      }
      ++TrackNumber;
      ...
      for (int Point = 0; Point < PointsOnTrack; Point++) {
         if (!FileHelper::ReadXYZ(v1[0], v1[1], v1[2], inputStream, inputName, "Measurement", TrackNumber, Point)) {
            break;
         }

         vDebug.Print("Point(x,y,z): (%f, %f, %f)\n", v1[0], v1[1], v1[2]);
         vAlignment.Correct(v1, v2);
         if (!FileHelper::WriteXYZ(v2[0], v2[1], v2[2], outputStream, outputName, "Measurement", TrackNumber, Point)) {
            break;
         }
      }
