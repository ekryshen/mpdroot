...
Alignment::Alignment(Debug const &inMode) : vDebug(inMode)
{
...
void Alignment::AddTrack(int n, double *v)
{
   ...
   //
   //  Building matrix of system of equations
   //
   int    lastSegmentID = -1; // just hack to have nicer debug message
...
int Alignment::Correct(double *v1, double *v2)
{
   ...
   // FIXME: Next 6 lines?
   w[0] = 80. * sin(k * 30. / 180. * M_PI);
   w[1] = 80. * cos(k * 30. / 180. * M_PI);
   w[2] = 0;
   for (int i = 0; i < 3; i++) {
      w[i] = 0.;
   }
...
{
   int k = static_cast<int>(floor(fi / M_PI * 6 + 12.5));
   k     = k % 12;
   if (z < 0.) {
      k = k + 12;
   }
