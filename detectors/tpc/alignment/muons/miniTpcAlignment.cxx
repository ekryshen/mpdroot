...
void miniTpcAlignment::Init(BaseTpcSectorGeo &tpcGeo, Int_t Event1, Int_t Event2, Int_t minHits, Int_t *Sectors,
                            const char *InDataFile, const char *aFile)
{
   ...
   if (!fTpcSecGeo) Fatal("miniTpcAlignment::Init", " !!! Wrong geometry type !!! ");
   ...
   t_fullChi2 = (TTree *)InputMCfile->Get("Chi2");
...
void miniTpcAlignment::alignment()
{
   if (printL > 0) {
   ...
   // if the 0-sector anchored to its theoretical position
   if (anchor) {
      for (Int_t i = 0; i < NLV; i++)
         for (Int_t j = NLV; j < N - Nanchor; j++) A[i][j] = A[i][j + Nanchor];
      // printf("[%d][%d]=[%d][%d]\n",i,j,i,j+Nanchor);}
      for (Int_t i = NLV; i < N - Nanchor; i++) {
         for (Int_t j = NLV; j < N - Nanchor; j++) A[i][j] = A[i + Nanchor][j + Nanchor];
         // printf("[%d][%d]=[%d][%d]\n",i,j,i+Nanchor,j+Nanchor);}
         B[i] = B[i + Nanchor];
      }
   }

   if (printL > 1) {
      cout << "==================== A&C ======== N=" << N - Nanchor << " ============" << endl;
      for (Int_t i = 0; i < NLV; i++) {
         //      for(Int_t m=0;m<N;m++) printf("%11.3e ",A[i][m]);
         //      printf("=%11.3e\n",B[i]);
         for (Int_t m = 0; m < N - Nanchor; m++) printf("%18.10e ", A[i][m]);
         printf("=%18.10e\n", B[i]);
      }
      for (Int_t i = NLV; i < N - Nanchor; i++) {
         Int_t sec;
         if ((i - NLV + Nanchor) % 6 == 0 || i == NLV) {
            sec = (i - NLV + Nanchor) / 6;
            printf("sector=%d   hits=%lld\n", sec, nhts[possec[(i - NLV) / 6]]);
         }
         for (Int_t m = 0; m < NLV; m++) printf("%18.10e ", A[i][m]);
         printf(" ");
         Int_t m1 = NLV + 6 * sec - Nanchor;
         Int_t m2 = m1 + 6;
         m1       = NLV;
         m2       = N - Nanchor;
         // for all matr    m1=NLV;m2=N-Nanchor;
   ...
   if (printL > 0) {
      ...
      for (Int_t is = 0; is < 24; is++) {
         if (sectors[is]) {
            Int_t ps = secpos[is];
            printf("sector=%2d  ", is);
            Int_t pi = NLV + 6 * ps - Nanchor;
            if (is == 0 && Nanchor > 0) {
               printf("dR0(%11.5e,%11.5e,%11.5e)  dA(%11.5e,%11.5e,%11.5e)\n", 0e0, 0e0, 0e0, X[pi + 3], X[pi + 4],
                      X[pi + 5]);
            } else {
               printf("dR0(%11.5e,%11.5e,%11.5e)  dA(%11.5e,%11.5e,%11.5e)\n", X[pi], X[pi + 1], X[pi + 2], X[pi + 3],
                      X[pi + 4], X[pi + 5]);
            }
            /* printf("input: R0(%11.5e,%11.5e,%11.5e)  abg(%11.5e,%11.5e,%11.5e)\n",
                    R0_A[0][is],R0_A[1][is],R0_A[2][is],alpha_A[is],beta_A[is],
                    gamma_A[is]);*/
         }
      }
...
// Calculate the shift of global parameters to the point with
// given derivatives values (der). The point is defined by hits
// (inputFile) and the alignment for this set of the input data (aFile)
void miniTpcAlignment::alignmentD(Double_t *der)
{
   Long64_t nhts[24];
   if (printL > 0) {
      cout << "miniTpcAlignment: InHitsFile ========  " << InHitsFile.Data() << endl;
   ...
   // if there no hits in a sector we put for it coefficients to get zeros
   // in the solutiom Ax=0 for this sector
   for (Int_t s = 0; s < 24; s++) {
      if (sectors[s]) {
         Int_t is = secpos[s];
         if (nhts[s])
            for (Int_t i = 0; i < 6; i++) {
               C[NLV + 6 * is + i] += der[NLV + 6 * is + i];
               B[NLV + 6 * is + i] = C[NLV + 6 * is + i];
            }
         else
            for (Int_t i = 0; i < 6; i++) {
               for (Int_t j = 0; j < NLV; j++) A[NLV + 6 * is + i][j] = 0;
   ...
   if (printL > 1) {
      ...
      for (Int_t i = NLV; i < N; i++) {
         if ((i - NLV) % 6 == 0) printf("sector=%d   hits=%lld\n", possec[(i - NLV) / 6], nhts[possec[(i - NLV) / 6]]);
         for (Int_t m = 0; m < N; m++) printf("%11.3e ", A[i][m]);
         printf("=%11.3e\n", B[i]);
      }
   }
   ...
   if (printL > 0) {
      ...
      for (Int_t is = 0; is < 24; is++) {
         if (sectors[is]) {
            Int_t ps = secpos[is];
            cout << "sector = " << is << endl;
            printf("delta: R0(%7.4f,%7.4f,%7.4f)  abg(%7.4f,%7.4f,%7.4f)\n", X[NLV + 6 * ps], X[NLV + 6 * ps + 1],
                   X[NLV + 6 * ps + 2], X[NLV + 6 * ps + 3], X[NLV + 6 * is + 4], X[NLV + 6 * is + 5]);
            printf("input: R0(%7.4f,%7.4f,%7.4f)  abg(%7.4f,%7.4f,%7.4f)\n", R0_A[0][is], R0_A[1][is], R0_A[2][is],
                   alpha_A[is], beta_A[is], gamma_A[is]);
         }
      }
   }
}
...
//
Int_t miniTpcAlignment::linsys(Int_t n, Double_t *x, Double_t a[NV][NV], Double_t *b)
{
...
void miniTpcAlignment::results(Double_t *R0x, Double_t *R0y, Double_t *R0z, Double_t *alpha, Double_t *beta,
                               Double_t *gamma)
{
   /*  for(Int_t i=0;i<nS;i++) {
        R0x[possec[i]]=X[NLV+6*i];
        R0y[possec[i]]=X[NLV+6*i+1];
        R0z[possec[i]]=X[NLV+6*i+2];
        alpha[possec[i]]=X[NLV+6*i+3];
        beta[possec[i]]=X[NLV+6*i+4];
        gamma[possec[i]]=X[NLV+6*i+5];
     } */
   if (Nanchor) {
      for (Int_t i = 143; i >= Nanchor; i--) X[i] = X[i - Nanchor];
      for (Int_t i = 0; i < Nanchor; i++) X[i] = 0;
   }
   for (Int_t s = 0; s < 24; s++)
      if (sectors[s]) {
...
void miniTpcAlignment::getDFDP(Double_t *der)
{
...
// A*deltaP = C (minimum conditon for the global chi2)
void miniTpcAlignment::Alignment(Double_t &F, Double_t *gradF)
{
   if (printL > 0) {
      cout << "miniTpcAlignment: InHitsFile ========  " << InHitsFile.Data() << endl;
      if (!aInFile.EqualTo(" ")) // read a special alignment
