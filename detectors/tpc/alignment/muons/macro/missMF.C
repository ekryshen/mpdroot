            ...
            const char *outFile = "0r5_0g5_80R130_dt08_evt10000")
{
   ...
   Int_t    Chambers    = 1;
   Double_t B_mf[3]     = {0, 0, 0.5}; // magnetic field, Tesla
   Int_t    MaxPcluster = 444;         // maximum pad number in cluster
   ...
   Double_t P_min      = 80.;          // min radio [cm]
   Double_t P_max      = 130.;         // min radio [cm]
   ...
   Double_t Sig_Z_pos    = 0.001;            // z-coordinate (gaz velocity) sigma, %
   ...
   mc->SetEvalCut(chi2cut);       // set maximum Xi^2 value at the event reconstruction
   mc->simulate(events);          // simulate the "events" number
                                  //  fTpcSecGeo->PrintSectorAlignment(0);
