#ifndef TPCALIGNMENT_H
#define TPCALIGNMENT_H

/// \ingroup rec
/// \class TpcAlignment
/// \brief find TPC alignment
///
...
class TpcAlignment : public TObject
/*
...
public:
   ///< Constructor
   TpcAlignment(BaseTpcSectorGeo &tpcGeo, Int_t Event1, Int_t Event2, Int_t minHits, Int_t *Sectors,
   ...
   ///< Constructor 2
   TpcAlignment() { ; }
   TpcAlignment(BaseTpcSectorGeo &tpcGeo, const char *InDataFile, const char *aFile)
   {
   ...
   ///< Destructor
   virtual ~TpcAlignment() { ; } ///< Destructor
...
private:
   ...
   // print level
   Int_t printL = 0;
   // Minimum number of hits per a track
   Int_t minNhits = 0;
   ...
   TString outDir;
   // File with alignment data (if it is not in InHitsFile)
   TFile *alignmentF;
