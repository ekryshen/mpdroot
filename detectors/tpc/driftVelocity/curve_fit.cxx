...
auto internal_solve_system(gsl_vector *initial_params, gsl_multifit_nlinear_fdf *fdf,
                           gsl_multifit_nlinear_parameters *params) -> std::tuple<std::vector<double>, int, int>
{
   ...
   int ok_status = gsl_multifit_nlinear_driver(max_iter, xtol, gtol, ftol, nullptr, nullptr, &info, work);
   // std::cout << "ok status " << info << " info " << info << std::endl;
   ...
   // "}"); std::cout << "curve fitted after " << niter << " iterations {nfev = " << nfev << "} {njev = " << njev << "}
   // {naev = " << naev << "}" << std::endl;

   gsl_multifit_nlinear_free(work);
   gsl_vector_free(initial_params);
   return std::make_tuple(result, ok_status, info);
}
