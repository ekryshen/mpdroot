...
namespace TpcAlignment {
	...
	string FileHelper::BuildFilePath(Solution solution, Direction direction, unsigned track, string filename, string innerPath)
	{
		string prefix, path, suffix;
		std::tie(prefix, path, suffix) = Path(solution, direction);
		...
		filepath.append(filename).append("_").append(to_string(track)). \
			append(suffix);
		return filepath;
	}

	string FileHelper::BuildFilePath(Solution solution, Direction direction, string filename, string innerPath, unsigned track)
	{
		string prefix, path, suffix;
		std::tie(prefix, path, suffix) = Path(solution, direction);
		string filepath{ string(prefix).append(path) };
	...
	string FileHelper::BuildFilePath(Solution solution, Direction direction, unsigned track, size_t point, string filename, string innerPath)
	{
		string prefix, path, suffix;
		std::tie(prefix, path, suffix) = Path(solution, direction);
		string filepath{ string(prefix).append(path) };
	...
	void FileHelper::SaveMR(string filename, SectorMatrix const& vM, SectorMatrix const& vR, int precision)
	{
		...
		for (int s = 0; s < SECTORS; s++) {
			for (size_t i = 0; i < L_; i++) {
				for (size_t j = 0; j < L_; j++) {
					fs << fixed << setprecision(precision) << vM.at(s).at(i + L_ * j) << " ";
				}
				fs << fixed << setprecision(precision) << vR.at(s).at(i) << " ";
			}
			fs << '\n';
		}
		fs.close();
	...
	void FileHelper::SaveCorrectionMatrix(std::string filename, SectorMatrix const& vCorrectionMatrix, int precision)
	{
		...
		if (!fs.is_open())
		{
			throw runtime_error(string("Can't open a file: ").append(filename).append(" to savel a CorrectionMatrix"));
		}

		for (size_t k = 0; k < SECTORS; k++) {
			for (size_t i = 0; i < 3; i++) {
				for (size_t j = 0; j < M_; j++) {
					fs << fixed << setprecision(precision) << vCorrectionMatrix[k][i + 3 * j] << " ";
				}
				fs << '\n';
			}
			fs << '\n';
	...
	void FileHelper::ReadCorrectionMatrix_obsolete(std::string filename, SectorMatrix& vCorrectionMatrix)
	{
		...
		if (fp == NULL) {
			throw runtime_error(string("Can't read the file: ").
				append(filename).
				append(". Correction matrix is not set!"));
		}
		int readN{ 0 };
		for (size_t k = 0; k < SECTORS; k++) {
			vCorrectionMatrix.at(k).resize(L_);
			for (size_t i = 0; i < 3u; i++) {
				for (size_t j = 0; j < M_; j++) {
					...
					if (readN == 1) {
						...
						{
							fclose(fp);
							throw runtime_error(string("Error readind file :").
						...
						fclose(fp);
						throw runtime_error(string("Error in file: ").append(filename).
							append("Line: ").append(to_string(k)).
							append(", Column: ").append(to_string(i * j)));
					}
				}
			}
		}
