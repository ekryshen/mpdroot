...
int FileHelper::ReadPointsOnTrack(FILE *fStream, const char *filename,
                                  const char *method) {
  int PointsOnTrack{0};
  int readNumber{0};
  if (!feof(fStream)) {
    ...
    if (readNumber == 0) {
      cout << method << ": " << filename
           << ": can't read Points On Track's value." << '\n';
      return 0;
    }
  } else {
    cout << method << ": " << filename << ": is empty" << '\n';
    return 0;
  }
  return PointsOnTrack;
}

bool FileHelper::WritePointsOnTrack(FILE *outputStream, int PointsOnTrack,
                                    const char *filename, const char *method) {
  int writtenNumber{0};
...
bool FileHelper::ReadCoordinate(double &coordinate, FILE *fStream,
                                const char *filename, const char *method,
                                const char *coordinateName, int Track,
                                int Point) {
  int readNumber{0};
  ...
  if (readNumber == 0) {
    cout << method << ": " << filename
         << ": ReadCoordinate: Error reading coordinate " << coordinateName
...
bool FileHelper::WriteCoordinate(double coordinate, FILE *fStream,
                                 const char *filename, const char *method,
                                 const char *coordinateName, int Track,
                                 int Point) {
  int writeNumber{0};
  ...
  if (writeNumber == 0) {
    cout << method << ": " << filename
         << ": WriteCoordinate: Error writing coordinate " << coordinateName
...
bool FileHelper::ReadXYZ(double &x, double &y, double &z, FILE *fStream,
                         const char *filename, const char *method, int Track,
                         int Point) {
...
bool FileHelper::WriteXYZ(double x, double y, double z, FILE *fStream,
                          const char *filename, const char *method, int Track,
                          int Point) {
