...
namespace TpcAlignment {
	...
	CorrectionMatrix::CorrectionMatrix(/*SectorMatrix& a,*/ Debug const& inMode) :
		vDebug(inMode)
	{
		vDebug.Print("testA start");
	}

	CorrectionMatrix::~CorrectionMatrix() { vDebug.Print("testA finished"); }
	...
	void CorrectionMatrix::LoadMatrix4File(std::string filename)
	{
		if (filename.empty())
		{
			throw runtime_error("Correction Matrix file path is not set.");
		}
	...
	double  CorrectionMatrix::CalculateCoordinatesNewValue(Point const& point,
	...
	{
		Point result { CalculateCoordinatesNewValue(inputPoint, 0u),
		CalculateCoordinatesNewValue(inputPoint, 1u),
