...
namespace TpcAlignment {
	...
	void SystemOfLinearEquations::AccumulateMR(const Track& track,
		const EigenCoefficients& eigenCoefficients)
	{
		for (size_t i=0; i < track.Get().size(); ++i)
		{		
			CalculateMR(track.Get().at(i).Sector(),
				CalculateG(eigenCoefficients.P, track.Get().at(i)),
				CalculateH(eigenCoefficients.P, eigenCoefficients.Q, track.Get().at(i)));
		}
	}

	void SystemOfLinearEquations::CalculateMR(int sector,
		vector<double> const & G, vector<double> const& H) {
	...
	void SystemOfLinearEquations::BuildMR(Rays const & tracks,
		CorrectionMatrix const & A)
	{
	...
	void SystemOfLinearEquations::BuildAndSolve(Rays& tracks,
		CorrectionMatrix & A)
	{
		BuildMR(tracks, A);
