...
namespace TpcAlignment {
	...
	FILE* Rays::GetFileStream(const char* RaysFilename, string mode)
	{
		...
		{
			throw runtime_error(string("Read4File: ").append(RaysFilename).
				append(" : can't open file. Error: ").
				append(ex.what()).c_str());
		}
		catch (...)
		{
	...
	void Rays::Read4File(const char* RaysFilename)
	{
		...
		{
			fclose(fp);
			throw runtime_error(string("Error while reading from the file: ").
				append(RaysFilename).append(": ").
				append(ex.what()).c_str());
		}
		catch (...)
		{
			fclose(fp);
			throw runtime_error(string("Unknown error while reading from file: ").
				append(RaysFilename).c_str());
		}
		fclose(fp);
	}
	...
	void Rays::Write2File(const char* RaysFilename)
	{
		...
		{
			throw runtime_error(string(RaysFilename).
				append(": can't open file. Error: ").
				append(ex.what()));
		...
		{
			errMsg.append("Error while saving Rays to the file: ").
				append(RaysFilename).append(": ").
				append(ex.what());
		}
		catch (...)
		{
			errMsg.append("Unknown error while saving Rays to the file: ").
				append(RaysFilename);
		}
	...
	void Rays::SaveRays(FILE* fp, const char* filename)
	{
		...
		{
			track.WriteTrack(fp,
				string("Save Rays to the file: ").append(filename));
		}
	}
}
