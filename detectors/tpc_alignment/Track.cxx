...
namespace TpcAlignment {
	Track::Track(Debug const& inDebug, 
		size_t TrackNumber):
	vDebug(inDebug),
	vTrackId(TrackNumber)
	{
	}
	...
	{
		int NumberOfPointsOnTrack =
			ReadNumberOfPointsOnTrack(fp, errorMsg);
		...
		if (readChar == 0 )
		{
			throw runtime_error(string("Can't get EOL or EOF"));
		}
	}
	...
	unsigned Track::ReadNumberOfPointsOnTrack(FILE* fStream, string errorMsg) const 
	{
		...
		if (!feof(fStream)) {
			...
			if (readNumber == 0) {
				throw invalid_argument(errorMsg.append("can't read Points On Track's value."));
			}
		}
		else {
			throw invalid_argument(errorMsg.append("File is empty"));
		}
