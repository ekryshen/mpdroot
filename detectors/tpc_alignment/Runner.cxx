...
namespace TpcAlignment {
	...
	void Runner::Calibrate(unsigned NumberOfCalibration)
	{
		for (int time = 0; time < NumberOfCalibration; ++time)
		{
			...
			{
				throw runtime_error(string("Calibrate on loop ").append(to_string(time)).
					append(" throws an exception: ").append(ex.what()));
			}
			catch (...)
			{
				throw runtime_error(string("Calibrate on loop ").append(to_string(time)).
					append(" throws an exception: ").append("Unknown exception"));
			}
		}
	}

	void Runner::SaveAMR2Files(string outputCorectionMatrix,
							   string outputMR, 
							   int precision)
	{
	...
	void Runner::CorrectTracks(string inputFileName,
