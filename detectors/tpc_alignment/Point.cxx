...
namespace TpcAlignment {
	...
	{
		if(vSectorId < 0) {
			throw runtime_error("Sector is not set.");
		}
		return vSectorId;
	}

	void Point::Read4File(FILE* fp,
		std::string errorMsg)
	...
	void Point::WriteCoordinate(double coordinate, FILE* fStream,
	                                 string errorMsg) {
		int writeNumber{ 0 };
	...
	unsigned Point::GetSectorNumberByCoordinate(double x, double y, double z)
	{
		...
		if (fi > 1.E+9)
		{
			throw runtime_error(string("fi > 1.E+9: ").append(to_string(fi)));
		}
		...
		if (segment_number < 0)
		{
			throw runtime_error(string("Sector couldn't be a negative number: "). \
				append(to_string(segment_number).append(" Check point: ("). \
					append(to_string(x)).append(", ").append(to_string(y)).append(", ").append(to_string(z)).append(")")));
		}
