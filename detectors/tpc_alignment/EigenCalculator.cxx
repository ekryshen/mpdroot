...
namespace TpcAlignment {
	...
	EigenCoefficients EigenCalculator::CalculateEigenValues(Track const& track,
		CorrectionMatrix const& vCorrectionMatrix)
	{
		if (track.Get().empty())
		{
			return { {}, {} };
	...
	Point EigenCalculator::CalculateQ(const Track& track,
	...
	Point EigenCalculator::CalculateP(Point const& frontPoint, 
		Point const& backPoint,
	...
	void EigenCalculator::NormilizeP(Point& P)
	{
		...
		if (fabs(r) < tolerance)
		{
			throw runtime_error("R is quite close to Zero");
		}
		P /= r;
	}

	void EigenCalculator::AccumulateQ(Point& Q,
		double TrackSize,
