...
{
   /** This will create a branch in the output tree called  BmdDetectorPoint, setting the last parameter to kFALSE
      means:
...
BmdPoint *BmdDetector::AddHit(Int_t trackID, Int_t detID, TVector3 pos, TVector3 mom, Double_t time, Double_t length,
                              Double_t eLoss)
{
   TClonesArray &clref = *fBmdPointCollection;
   Int_t         size  = clref.GetEntriesFast();
   return new (clref[size]) BmdPoint(trackID, detID, pos, mom, time, length, eLoss);
}
...
void BmdDetector::ConstructGeometry()
{
   ...
   if (fileName.EndsWith(".root")) {
      ...
      LOG(info) << "Geometry format of BMD file " << fileName.Data() << " not supported." << endl;
   }
}
...
Bool_t BmdDetector::CheckIfSensitive(std::string name)
{
   ...
   if (tsname.Contains("A1_") || tsname.Contains("C1_") || tsname.Contains("A2_") || tsname.Contains("C2_") ||
       tsname.Contains("A3_") || tsname.Contains("C3_") || tsname.Contains("A4_") || tsname.Contains("C4_") ||
       tsname.Contains("A5_") || tsname.Contains("C5_") || tsname.Contains("A6_") || tsname.Contains("C6_") ||
