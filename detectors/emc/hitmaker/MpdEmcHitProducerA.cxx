...
MpdEmcHitProducerA::MpdEmcHitProducerA()
   : FairTask("Ideal EMC hit Producer"), fGeoPar(0), fPointArray(0), fMcTrackArray(0), fDigiArray(0), fTested(kFALSE)
{
   fHgtheta0 = 0;
   fHgtheta1 = 0;
}
...
InitStatus MpdEmcHitProducerA::Init()
{
   ...
   if (!fPointArray) {
      cout << "-W- MpdEmcHitProducerA::Init: "
...
void MpdEmcHitProducerA::Exec(Option_t *opt)
{
   cout << "-I- MpdEmcHitProducerA::Exec: started with "
        << " tracks/points = " << fMcTrackArray->GetEntriesFast() << "/" << fPointArray->GetEntriesFast() << endl;
...
void MpdEmcHitProducerA::Test()
{
   ...
   cout << "-I- MpdEmcHitProducer::Test: Ntr [empty / total] = " << nempty << "/" << fMcTrackArray->GetEntriesFast()
        << endl;

   cout << "-I- MpdEmcHitProducer::Test: Ntr [in(daughts) / total(daughts)] = " << tt2 << "/" << tt1 << endl;

   cout << "-I- MpdEmcHitProducer::Test: Ntr [pi0 / total] = " << npi0 << "/" << fMcTrackArray->GetEntriesFast()
        << endl;

   cout << "-I- MpdEmcHitProducer::Test: Ntr [ndaughters / 2*pi0 / ngamma / ngamma_points] = " << ndaughts << "/"
        << npi0 * 2 << "/" << ngammas << "/" << ngp << endl;

   cout << "-I- MpdEmcHitProducer::Test: Ntr [ndaughters (in) / ndaughters (total)] " << ndaughts_in << "/" << ndaughts
        << endl;
   ...
   cout << "-I- MpdEmcHitProducer::Test: Points [total / mc-total / mc-ECAL] = " << fPointArray->GetEntriesFast() << "/"
        << np1 << "/" << np2 << endl;

   // cout << endl;

   cout << "-I- MpdEmcHitProducer::Test: Points:NGAMMA [tracks / tracks,gen = 2 / points,tracks] = " << gammas.size()
        << "/" << ngg << "/" << np << endl;

   cout << "-I- MpdEmcHitProducer::Test: Points:NELECT [tracks(electrons) / total] = " << electrons.size() << "/"
        << allpart.size() << endl;
