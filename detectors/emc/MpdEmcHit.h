...
public:
   ...
   Int_t GetSec() const { return fDetectorID / (1000 * 12); };

   Int_t GetMod() const { return fDetectorID - (fDetectorID / 1000) * 1000; };

   Int_t GetSupMod() const { return -1; };
   ...
   Float_t GetRhoCenter() const { return sqrt(fX * fX + fY * fY); }

   Float_t GetZCenter() const { return fZ; };

   Float_t GetPhiCenter() const { return fPhiCenter; };

   Float_t GetThetaCenter() const { return fThetaCenter; };
   ...
   void IncreaseEnergyTime(Float_t timeEnergy) { fTime += timeEnergy; };
   ...
   void SetZCenter(Float_t z) { fZ = z; };

   void SetThetaCenter(Float_t theta) { fThetaCenter = theta; };
