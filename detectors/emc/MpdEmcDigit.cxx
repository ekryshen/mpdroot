...
MpdEmcDigit::MpdEmcDigit()
   : FairHit(), fChanZId(-1), fChanPhiId(-1), fE(-1.0), fTrackID(-1), fFlag(0), fPDG(0), fNumTracks(0), fZCenter(0.0),
     fPhiCenter(0.0)
{
}
...
MpdEmcDigit::MpdEmcDigit(Int_t detID, TVector3 pos, TVector3 dpos, Int_t index) : FairHit(detID, pos, dpos, index) {}

// -----   Constructor without flag  ------------------------------------------

MpdEmcDigit::MpdEmcDigit(UInt_t side, UInt_t sector, UInt_t row, UInt_t channel)
   : FairHit(), fE(0), fTrackID(-1), fFlag(0), fPDG(0), fNumTracks(0)
{
...
void MpdEmcDigit::Print(const Option_t *opt) const
{
   ...
   // cout << "\tSec: " << fSecId << "   Row: " << fRowId << "   Super module: " << fSupModId << "   Module:" << fModId
   // << endl;
...
void MpdEmcDigit::IncreaseEnergy(Float_t e, Int_t trId)
{
   ...
   fE += e;
   if (fContrib.find(trId) == fContrib.end())
      fContrib[trId] = e;
   else
      fContrib[trId] += e;
