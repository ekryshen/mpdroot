...
{
   if (GetDetectorID() == rhs.GetDetectorID()) return GetTrackID() < rhs.GetTrackID();
...
{
   if (rhs.GetEnergyLoss() > GetEnergyLoss()) SetTime(rhs.GetTime());
...
MpdEmcPointKI MpdEmcPointKI::operator+(const MpdEmcPointKI &rhs) const
{
   ...
   if (rhs.GetEnergyLoss() > result.GetEnergyLoss()) result.SetTime(rhs.GetTime());
