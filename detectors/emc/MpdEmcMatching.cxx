...
MpdEmcMatching::MpdEmcMatching() : FairTask("TPC-EMC matching") {}
...
InitStatus MpdEmcMatching::Init()
{
   ...
   for (Int_t isec = 1; isec < nSec; ++isec) {
      if (isec == 2 || isec == 6)
         fSecRows0.insert(*fSecRows0.rbegin() + nSecRows / 2);
      else
         fSecRows0.insert(*fSecRows0.rbegin() + nSecRows);
   }

   // Get input array
   fPointArray = (TClonesArray *)ioman->GetObject("EmcPoint");
   if (!fPointArray) {
      cout << "-W- MpdEmcMatching::Init: "
...
void MpdEmcMatching::Exec(Option_t *opt)
{
   ...
   for (Int_t i = 0; i < ntpc; ++i) {
      // if (isec < nSec && (*tr->GetParamAtHit())(1,0) > 20 && (*tr->GetParamAtHit())(3,0) > 0) continue; // different
      // sides else if (isec >= nSec && (*tr->GetParamAtHit())(1,0) < -20 && (*tr->GetParamAtHit())(3,0) < 0) continue;
      // // different sides
      DoMatching(i);
   }
}
...
void MpdEmcMatching::DoMatching(Int_t itrack)
{
   ...
   if (TMath::Abs(dsecs) > 1 && TMath::Abs(dsecs) < nSec / 2)
      secs[1] = secs[0] + TMath::Sign(1, dsecs);
   else if (TMath::Abs(dsecs) > nSec / 2)
      secs[1] = secs[0] - TMath::Sign(1, dsecs);
   ...
   */
   ...
   for (Int_t io = 0; io < 2; ++io) {
      ...
      for (multimap<Double_t, Int_t>::iterator it = pits[io].first; it != pits[io].second; ++it) {
         ...
         MpdEmcMatch *match =
            new ((*fMatchArray)[imatch++]) MpdEmcMatch(itrack, it->second, distT, distL, recp->GetQ(), trLeng);
         // Electrons
         Int_t ireg = 0;
         if (distL >= -1.0 && distL <= trLeng + 1.0)
            ireg = 1;
         else if (distL > trLeng) {
            distL -= (trLeng + 1.0);
            ireg = 2;
         }
         ...
         if (ireg == 1)
            chi2 += (distL - depthPi) * (distL - depthPi) / sigLpi[ireg] / sigLpi[ireg];
         else
            chi2 += (distL * distL / sigLpi[ireg] / sigLpi[ireg]);
...
void MpdEmcMatching::GetTowerCoords(TVector3 &pos, Int_t io, Double_t &phiT, Double_t &theT)
{
   ...
   if (io == 0)
      theT = theSmin->Eval(theta);
   else
      theT = theSmax->Eval(theta);
}
//__________________________________________________________________________
