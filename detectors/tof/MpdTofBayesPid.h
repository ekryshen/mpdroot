...
// stage 2: use  auto tofPid = new MpdTofBayesPid("PDFflnm.root", iterationNmb, "Priors_prefix",
// "nextIter_Priors_prefix") to create next iteration Priors; 		 fRun->AddTask(tofPid) -- add task to chain For
// iterationNmb=1 is created default const Priors automatically. Usually 3-4 iterations are required for Priors
// convergence. Pid probabilities is copied to MpdTofMatchingData::fBayesPid[4] TRANSPARENT(don't saved to disk) public
// member data.
//------------------------------------------------------------------------------------------------------------------------
...
public:
   ...
   const TH1D *GetPrior(size_t pid) const
   {
      assert(pid < fNpdf);
      return fPriors[pid];
   }
...
class PidProbabilitiesMatrix {
   ...
   TH1D *fProb[fNpdf][fNpdf] =
      {}; // εPID[i][j] - the probability to identify a species i as a species j (clone of MpdBayesPriors::fPriorOrigin)
   TH1D *fA_true[fNpdf] =
      {}; // A_true - true abundances of each species (clone of MpdBayesPriors::fPriorOrigin, used for test of fProb)
   TH1D *fA_meas[fNpdf] = {}; // A_meas - measured abundances of each species (clone of MpdBayesPriors::fPriorOrigin,
                              // used for test of fProb)
   double fPidThres = -1.; // if negative -  max. probability method(1) used, else - used as probability threshold for
                           // fixed threshold method(2)
   TString fName, fFlnm;
   TList   fList;
...
public:
   ...
   MpdTofBayesPid(const char *PDFflnm, size_t iterNmb, const char *priorPrefix = nullptr,
                  const char *nextPriorPrefix = nullptr, const char *QAflnm = nullptr, const char *taskName = "TOF_pid",
                  Int_t verbose = 1, Bool_t useMCdata = true);
   ...
   template <typename T>
   void         Add(TList &, T *hist);
   static Int_t GetIndex(
