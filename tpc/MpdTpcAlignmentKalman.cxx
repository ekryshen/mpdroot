...
MpdTpcAlignmentKalman::StatBuffer &MpdTpcAlignmentKalman::StatBuffer::stats_compute(
   Eigen::Matrix<double, npars, 1> &mean, Eigen::Matrix<double, npars, 1> &disp)
{
   mean.setZero();
   disp.setZero();
   ...
   if (length) {
      for (size_t i = 0; i < length; ++i) mean += data[i];

      auto norm = 1. / length;
      mean *= norm;

      if (length > 1) {
         for (size_t i = 0; i < length; ++i) disp += ((data[i] - mean).array() * (data[i] - mean).array()).matrix();
...
InitStatus MpdTpcAlignmentKalman::Init()
{
   ...
   out_tree->Branch("st", ost.t,
                    "t[24]/D:ann[24]/D:nsg[24]/D:nht[24]/D:dx[24]/D:dy[24]/D:dz[24]/D:dp[24]/D:de[24]/D:dk[24]/"
                    "D:Cx[24]/D:Cy[24]/D:Cz[24]/D:Cp[24]/D:Ce[24]/D:Ck[24]/D"); //: Vx[24]/D:Vy[24]/D:Vz[24]/D");
   ...
   for (unsigned sid = 0; sid < nSectors; ++sid) {
      // st[sid] << 0., 0., reader->GetSectorsShift().fTpcSectorShift[sid].X(),
      // reader->GetSectorsShift().fTpcSectorShift[sid].Y();
...
{
   {
      ...
      for (unsigned sid = 0; sid < nSectors; ++sid) is_over = (is_finished[sid] && is_over);
...
Int_t MpdTpcAlignmentKalman::tracksFind()
{
   ...
   if ((!std::strcmp(tracks_type, "laser_mc")) || (!std::strcmp(tracks_type, "laser_emu"))) {
      ...
      auto laser_hits_collect = [&]() -> void {
         for (unsigned sid = 0; sid < nSectors; ++sid) sec_hit[sid] = 0;
      ...
      for (Int_t pln = -4; pln < 4; ++pln) // planes
      {
         ...
         for (Int_t beam = 0; beam < 4; ++beam) // beams
         {
            ...
            {
               auto phi =
                  (360. - 90. * (beam + 1) - bndl_ang + (refl < 0 ? (refl - 1) * mir_ang : (refl + 2) * mir_ang)) *
                  TMath::DegToRad();
               auto sn = std::sin(phi);
               auto cs = std::cos(phi);
...
bool MpdTpcAlignmentKalman::trackParsCompute(FittedTrack &tr)
{
   ...
   if (std::isfinite(mean[0]) && std::isfinite(mean[1]) && std::isfinite(mean[2])) {
      ...
      for (auto it = tr.hits.begin(); it != tr.hits.end(); ++it) {
         ...
         cov(0, 1) += dp[0] * dp[1];
         cov(0, 2) += dp[0] * dp[2];
         /*cov(1,0) += dp[1] * dp[0];*/ cov(1, 1) += dp[1] * dp[1];
         cov(1, 2) += dp[1] * dp[2];
      ...
      // std::cout << " rec org " << tr.org.transpose() << " dir " << tr.dir.transpose() << " hits " << tr.hits.size()
      // << std::endl;
      ...
      for (auto it = tr.hits.begin(); it != tr.hits.end(); ++it) {
         ...
         auto pos = tr.org + (tr.dir.adjoint() * (hit_pos - tr.org)) *
                                tr.dir; // fitted track's point closest to the hit, glob coord
         (*it)->fit = pos;
         ...
         tr_cov(0, 0) += dp[0] * dp[0];
         tr_cov(0, 1) += dp[0] * dp[1];
         tr_cov(0, 2) += dp[0] * dp[2];
         /*tr_cov(1,0) += dp[1] * dp[0];*/ tr_cov(1, 1) += dp[1] * dp[1];
         tr_cov(1, 2) += dp[1] * dp[2];
      ...
      for (unsigned sid = 0; sid < nSectors; ++sid) {
         if (tr.cov[sid]) {
            ...
            tr_cov(0, 1) *= invSize;
            tr_cov(0, 2) *= invSize;
            tr_cov(1, 0) = tr_cov(0, 1);
            tr_cov(1, 1) *= invSize;
            tr_cov(1, 2) *= invSize;
            tr_cov(2, 0) = tr_cov(0, 2);
            tr_cov(2, 1) = tr_cov(1, 2);
            tr_cov(2, 2) *= invSize;
   ...
   for (auto tr = tracks.begin(); tr != tracks.end(); ++tr) {
      ...
      for (++it; it != tr->hits.end(); ++it) {
         ...
         if (sid_n != sid_p) {
            ...
            if (sector_active) sector_state_revise(sid_p);
         ...
         if (sector_active) {
            ...
            // sn = A*sp;
            sn                       = sp;
            sn.block(0, 0, ndims, 1) = hit_n->fit.block(0, 0, ndims, 1);
            ...
            if (invertible) {
               ...
               /*
               std::cout << "+ dlprd,cm: " << (sn.block(0,0,ndims,1) - sp.block(0,0,ndims,1)).norm() << " vdt: "  <<
               c_vel*dt << std::endl; std::cout << "  line dir: " << (hit_n->nrm.block(0,0,ndims,1) -
               hit_p->nrm.block(0,0,ndims,1)).normalized().transpose() << " v dir: "  <<
               (sp.block(ndims,0,ndims,1)*dt).normalized().transpose() << std::endl; std::cout << std::endl << K << " K"
               << std::endl << std::endl; std::cout << "  mhit: " << hit_n->glb.block(0,0,ndims,1).transpose() << "
               model: "  << (H[sid_n]*sn).transpose() << std::endl; std::cout << "  res:  " << res.transpose() << "
               norm: "  << res.norm() << std::endl << K*res << " K*res" << std::endl; std::cout << "  sold: " <<
               sp.transpose() << std::endl << "  sprd: " << sn.transpose() << std::endl; */
               ...
               if (invertible) chi2[sid_n] += res.transpose() * Rinv * res;
            }
         }
...
bool MpdTpcAlignmentKalman::sector_state_init(const unsigned int &sid, const Eigen::Matrix<double, ndims, 1> &vel,
                                              const Eigen::Matrix<double, ndims, 1> &pos, const FittedTrack *tr)
{
   ...
   if (sector_active) {
      ...
      ann[sid]      = (track_segments[sid] < nAnnSecTracks
                          ? std::pow(ann0, static_cast<double>(nAnnSecTracks - track_segments[sid]) /
                                              static_cast<double>(nAnnSecTracks))
                          : 1.);
...
MpdTpcAlignmentKalman::Hit *MpdTpcAlignmentKalman::hit_(const MpdTpcHit *tpcHit) const
{
   ...
   hit->glb = /*track_segments[hit->sid] < nAnnSecTracks ? hit->raw :*/ align(
      locPos.second, locPos.first); // apply currently known alignment for hits to be used in fit

   // std::cout << "sector: " << (hit)->sid << " glob: " << (hit)->glb.transpose() << " loc: " << (hit)->loc.transpose()
   // << std::endl; std::cout << "sector: " << sid << " glob X: " << globPos.X() << " Y: " << globPos.Y() << " Z: " <<
   // globPos.Z() << " loc X: " << locPos.first.X() << " Y: " << locPos.first.Y() << " Z: " << locPos.first.Z() <<
   // std::endl;
...
std::pair<Eigen::Vector3d, unsigned> MpdTpcAlignmentKalman::glob2loc(const Eigen::Vector3d &globPos,
                                                                     const unsigned         sid2use) const
{
   unsigned        sid = sid2use;
   Eigen::Vector3d locPos;

   if (sid2use >= nSectors) sid = sid_(globPos);
...
Eigen::Vector3d MpdTpcAlignmentKalman::align(const unsigned &sid, const Eigen::Vector3d &arg,
                                             const Eigen::Matrix<double, npars, 1> *a) const
{
   Eigen::Vector3d                        val;
...
std::pair<Eigen::Vector3d, unsigned> MpdTpcAlignmentKalman::misalign(const Eigen::Vector3d                 &arg,
                                                                     const Eigen::Matrix<double, npars, 1> *a) const
{
...
void MpdTpcAlignmentKalman::FinishEvent()
{
   ...
   for (auto tr = tracks.begin(); tr != tracks.end(); ++tr) tr->clear();
}

void MpdTpcAlignmentKalman::Finish()
{
   auto time_tot = ((float)exec_time) / CLOCKS_PER_SEC;
   std::cout << "MpdTpcAlignmentKalman execution time: " << time_tot << " s, per event: " << time_tot / events_processed
             << " s" << std::endl;
...
void MpdTpcAlignmentKalman::event_log_flush()
{
   ...
   for (unsigned sid = 0; sid < nSectors; ++sid) nshit[sid] = nsseg[sid] = 0;
   ...
   if (!(events_processed % printout_period)) {
      std::cout << "Event #" << events_processed << ": sec0 ann " << ann[0] << " Cxx " << C[0](p_dx, p_dx) << " Cyy "
                << C[0](p_dy, p_dy) << " Czz " << C[0](p_dz, p_dz);
      std::cout << std::endl
                << "                   diff-diff_prev                diff/true                     true                "
                   "  current                       sigma    tracks"
                << std::endl;
   }
   ...
   for (unsigned sid = 0; sid < nSectors; ++sid) {
      ...
      if (is_active[sid] && !(events_processed % printout_period))
         std::printf("sect %02d:     % .0e % .0e % .0e     % .3f % .3f % .3f     % .3f % .3f % .3f     % .3f % .3f % "
                     ".3f     %.1e %.1e %.1e     %5d\n",
                     sid, (diffx - s0[sid](p_dx, 0)), (diffy - s0[sid](p_dy, 0)), (diffz - s0[sid](p_dz, 0)), qx, qy,
                     qz, st[sid](p_dx, 0), st[sid](p_dy, 0), st[sid](p_dz, 0), s[sid](p_dx, 0), s[sid](p_dy, 0),
                     s[sid](p_dz, 0), std::sqrt(disp(p_dx, 0)), std::sqrt(disp(p_dy, 0)), std::sqrt(disp(p_dz, 0)),
                     track_segments[sid]);
