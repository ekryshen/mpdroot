...
InitStatus MpdTpcTrackEmulator::Init()
{
   if (!(mfpath * stddev * hitres)) {
      Error("MpdTpcTrackEmulator::Init",
            "mean free path / scattering standart deviation / hits resolution HAVE TO BE SET explicitly");
      return kFATAL;
   }
   ...
   if (!std::strcmp(tracks_type, "laser")) {
      ...
      for (Int_t trk = 0; trk < ntrk_per_ev; ++trk) {
         ...
         do {
            ...
            auto phi = ang * k;
            auto ksi = rnd_flat(0, 1) > 0.5 ? ang * (rnd_flat(0, 1) > 0.5 ? k + f * 0.25 : k - f * 0.25)
            ...
            // std::cout << " emu org " << org.transpose() << " dir " << dir.transpose() << " hits " << nHits <<
            // std::endl;
         } while (!nHits);
      }
   } // muons
...
unsigned MpdTpcTrackEmulator::track_pass(const Eigen::Vector3d &org, const Eigen::Vector3d &dir)
{
   ...
   do {
      ...
      while (dl.norm() < hitres) dl = rnd_exp(mfpath) * dir;
