...
public:
    ...
    virtual void                                                Finish() override                             { if (rand_gen) delete rand_gen; }

    MpdTpcTrackEmulator&                                        stddev_set(const Double_t val)                { stddev = val; return *this; }
    MpdTpcTrackEmulator&                                        mean_free_path_set(const Double_t val)        { mfpath = val; return *this; }
    MpdTpcTrackEmulator&                                        hits_resolution_set(const Double_t val)       { hitres = val; return *this; }
    MpdTpcTrackEmulator&                                        velocity_set(const Double_t val)              { velocity = val*1.E-9; return *this; } // cm/s -> cm/ns
    MpdTpcTrackEmulator&                                        tracks_per_event_set(const Int_t val)         { ntrk_per_ev = val; return *this; }
