...
private:
    ...
    //static constexpr Int_t                                           nwmod = 3; // number of alignment pars need to be corrected for weak mods
    ...
    struct FittedTrack
    {
        ...
        void                                                            reset()   { hits.clear(); for (unsigned i = 0; i < nSectors; ++i) cov[i] = nullptr; }
        void                                                            clear()   { hits.clear(); for (unsigned i = 0; i < nSectors; ++i) if (cov[i]) { delete cov[i]; cov[i] = nullptr; } }
    ...
    public:
        ...
        ~StatBuffer()                                                                                                         { if (data) delete[] data; }
        ...
        StatBuffer&                                                     stats_compute(Eigen::Matrix<double, npars, 1>& mean,
                                                                                      Eigen::Matrix<double, npars, 1>& disp);
    };
    ...
    // algo subroutines
    Eigen::Vector3d                                                  align(const unsigned& sid, const Eigen::Vector3d& p_loc, const Eigen::Matrix<double, npars, 1>* a = nullptr) const;  // loc->glob, alignment applied
    std::pair< Eigen::Vector3d, unsigned>                            misalign(const Eigen::Vector3d& p_glb, const Eigen::Matrix<double, npars, 1>* a =  nullptr) const; //  glob->loc, misalignment applied
    void                                                             matrixHInit(const unsigned& sid);

    bool                                                             sector_status_(const unsigned& sid);
    bool                                                             sector_state_init(const unsigned int &sid, const Eigen::Matrix<double, ndims, 1>& vel, const Eigen::Matrix<double, ndims, 1>& pos, const FittedTrack* tr);
    ...
    //void                                                             hit2vector(const Hit* hit, TVector3& globXYZ, TVector3& locXYZ) const;
    //std::pair<Eigen::Vector3d, unsigned>                             glob2loc(const TVector3& globXYZ) const;
    //Eigen::Vector3d                                                  loc2glob(const TVector3& locXYZ, const unsigned& sid) const;
    //TVector3                                                         toTV3(const Eigen::Vector3d& v) const { return TVector3(v(0,0), v(1,0), v(2,0)); }

//public:
    //void                                                             SetAlignParamsFile(std::string path) { if(reader) reader->SetAlignParamsFile(path); }
#endif
