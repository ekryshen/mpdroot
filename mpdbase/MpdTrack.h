...
private:
   ...
   Float_t fLastPointZ;     //
   Short_t fHelixQ;         //
   Bool_t  fEdgeCut;  // kTRUE if number of hits closer to boundaries than 1.5 cm divided by nHits is larger than 50%
                      // (else: kFALSE)
...
public:
   ...
   {
      fPidTOFProbElectron = n1;
      fPidTOFProbPion     = n2;
      fPidTOFProbKaon     = n3;
      fPidTOFProbProton   = n4;
      SetTofFlag(flag);
   }
   void SetTPCpidProb(Float_t n1, Float_t n2, Float_t n3, Float_t n4, Int_t flag)
   {
      fPidTPCProbElectron = n1;
      fPidTPCProbPion     = n2;
      fPidTPCProbKaon     = n3;
      fPidTPCProbProton   = n4;
      SetTofFlag(flag);
   }
   ...
   MpdHelix  GetHelix(Double_t fieldBz = 0.0) const;
   ClassDef(MpdTrack, 4)
};
