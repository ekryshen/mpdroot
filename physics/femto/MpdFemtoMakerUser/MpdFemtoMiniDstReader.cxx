...
//_________________
MpdFemtoMiniDstReader::MpdFemtoMiniDstReader() : MpdFemtoBaseEventReader(), mMiniDstReader(nullptr), mHbtEvent(nullptr)
{
   // Default Constructor
...
MpdFemtoMiniDstReader::MpdFemtoMiniDstReader(MpdMiniDstReader *miniDstReader, int debug)
   : MpdFemtoBaseEventReader(), mMiniDstReader(miniDstReader), mHbtEvent(nullptr), mEventsPassed(0)
{
...
MpdFemtoMiniDstReader::MpdFemtoMiniDstReader(const MpdFemtoMiniDstReader &copy)
   : MpdFemtoBaseEventReader(), mMiniDstReader(copy.mMiniDstReader), mHbtEvent(nullptr), mEventsPassed(0)
{
   // Copy constructor
...
MpdFemtoEvent *MpdFemtoMiniDstReader::returnHbtEvent()
{
   ...
   if (!mMiniDstReader) {
      std::cout << "[ERROR] MpdFemtoEvent* MpdFemtoMiniDstReader::returnHbtEvent() - no MpdMiniDstReader is provided"
                << std::endl;
      mReaderStatus = 1;
      return mHbtEvent;
   }
   ...
   if (!miniDst) {
      std::cout << "[ERROR] MpdFemtoEvent* MpdFemtoMiniDstReader::returnHbtEvent() - no miniDst is in MpdMiniDstReader"
                << std::endl;
      mReaderStatus = 1;
      return mHbtEvent;
   }
   ...
   if (!miniDst->event() ||
       !miniDst->numberOfTracks()) { // CAN'T UNDERSTAND WHY THERE IS NO "!" BEFORE SECOND CONDITION or <=0
      std::cout << "[WARNING] MpdFemtoEvent* MpdFemtoMiniDstReader::returnHbtEvent()"
                << " - MpdMiniEvent does not exist or particles missing" << std::endl;
      mReaderStatus = 1;
   } else {
      ...
      for (unsigned int iTrk = 0; iTrk < miniDst->numberOfTracks(); iTrk++) {
         ...
         if (hiddenInfo) hbtTrack->setHiddenInfo(hiddenInfo);
