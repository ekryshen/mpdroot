...
MpdFemtoMcDstReader::MpdFemtoMcDstReader()
   : MpdFemtoBaseEventReader(), mMcDstReader(nullptr), mMagField(-5.), mHbtEvent(nullptr), mDoRotate(false),
     mEventPlaneResolution(1.), mRefMult(0), mRefMultPos(0), mRefMult2(0), mRefMult2Pos(0), mSphericity(-1.),
     mSphericity2(-1.), mEventsPassed(0), mMatrix(nullptr), mMatrix2(nullptr)
{
   // Default constructor
   mReaderStatus = 0;
...
MpdFemtoMcDstReader::MpdFemtoMcDstReader(McDstReader *reader, int debug)
   : MpdFemtoBaseEventReader(), mMcDstReader(reader), mMagField(-5.), mHbtEvent(nullptr), mDoRotate(false),
     mEventPlaneResolution(1.), mRefMult(0), mRefMultPos(0), mRefMult2(0), mRefMult2Pos(0), mSphericity(-1.),
     mSphericity2(-1.), mEventsPassed(0), mMatrix(nullptr), mMatrix2(nullptr)
{
...
MpdFemtoMcDstReader::MpdFemtoMcDstReader(const MpdFemtoMcDstReader &copy)
   : MpdFemtoBaseEventReader(), mMcDstReader(copy.mMcDstReader), mMagField(copy.mMagField), mHbtEvent(nullptr),
     mDoRotate(copy.mDoRotate), mEventPlaneResolution(copy.mEventPlaneResolution), mRefMult(0), mRefMultPos(0),
     mRefMult2(0), mRefMult2Pos(0), mSphericity(-1.), mSphericity2(-1.), mEventsPassed(0), mMatrix(nullptr),
     mMatrix2(nullptr)
{
   // Copy constructor
   if (mRandom) {
...
MpdFemtoEvent *MpdFemtoMcDstReader::returnHbtEvent()
{
   ...
   if (!mMcDstReader) {
      std::cout << "[ERROR] MpdFemtoEvent* MpdFemtoMcDstReader::returnHbtEvent() - no McDstReader is provided"
                << std::endl;
      mReaderStatus = 1;
      return mHbtEvent;
   }
   ...
   if (!mcDst) {
      std::cout << "[ERROR] MpdFemtoEvent* MpdFemtoMcDstReader::returnHbtEvent() - no McDst is in McDstReader"
                << std::endl;
      mReaderStatus = 1;
      return mHbtEvent;
   }
   ...
   for (unsigned int iTrk = 0; iTrk < mcDst->numberOfParticles(); iTrk++) {
      ...
      hbtTrack->setDca(particle->x() * 1e-13, particle->y() * 1e-13, particle->x() * 1e-13);
      ...
      hiddenInfo->setTrueMomentum(hbtTrack->p().X(), hbtTrack->p().Y(), pz);

      // Check if rotate event plane angle (IS IT NEEDED??)
      if (mDoRotate) {
         double radialPos = TMath::Sqrt(particle->x() * particle->x() + particle->y() * particle->y());
         azimuthalAngle   = TMath::ATan2(particle->y(), particle->x());
         azimuthalAngle += phi;
         hiddenInfo->setEmissionPoint(radialPos * TMath::Cos(azimuthalAngle), radialPos * TMath::Sin(azimuthalAngle),
                                      particle->z(), particle->t());
      } else {
         hiddenInfo->setEmissionPoint(particle->x(), particle->y(), particle->z(), particle->t());
      }
      ...
      if (TMath::Abs(particle->eta()) > 1. || pt < 0.15) continue;
...
double MpdFemtoMcDstReader::dEdxMean(Double_t mass, Double_t momentum)
{
   ...
   if (beta > 0) {
      dedxMean = tpcDedxGain / (beta * beta) * (0.5 * TMath::Log(rise) - beta * beta - tpcDedxOffset);
   } else {
      dedxMean = 1000.;
   }
   return dedxMean;
