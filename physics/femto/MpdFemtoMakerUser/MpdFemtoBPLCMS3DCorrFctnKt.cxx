...
//_________________
MpdFemtoBPLCMS3DCorrFctnKt::MpdFemtoBPLCMS3DCorrFctnKt(const char *title, const int &nBins, const double &qLo,
                                                       const double &qHi, const int &ktBins, const double &ktLo,
                                                       const double &ktHi)
   : MpdFemtoBaseCorrFctn()
{
   ...
   for (int iKtBin = 0; iKtBin < mNKtBins; iKtBin++) {
      ...
      baseTitle += appendix;
      mNumerator[iKtBin] = new TH3F(baseName.Data(), baseTitle.Data(), mNQbins, mQRange[0], mQRange[1], mNQbins,
                                    mQRange[0], mQRange[1], mNQbins, mQRange[0], mQRange[1]);
      ...
      baseTitle += roundf(mKtRange[0] + mKtStep * (iKtBin + 1)) / 100;
      baseTitle += appendix;
      mDenominator[iKtBin] = new TH3F(baseName.Data(), baseTitle.Data(), mNQbins, mQRange[0], mQRange[1], mNQbins,
                                      mQRange[0], mQRange[1], mNQbins, mQRange[0], mQRange[1]);
      ...
      baseTitle += roundf(mKtRange[0] + mKtStep * (iKtBin + 1)) / 100;
      baseTitle += appendix;
      mQinvHisto[iKtBin] = new TH3F(baseName.Data(), baseTitle.Data(), mNQbins, mQRange[0], mQRange[1], mNQbins,
                                    mQRange[0], mQRange[1], mNQbins, mQRange[0], mQRange[1]);
...
MpdFemtoBPLCMS3DCorrFctnKt::MpdFemtoBPLCMS3DCorrFctnKt(const MpdFemtoBPLCMS3DCorrFctnKt &copy)
   : MpdFemtoBaseCorrFctn(copy), mNQbins(copy.mNQbins), mNKtBins(copy.mNKtBins), mKtStep(copy.mKtStep)
{
   // Copy constructor
   for (int i = 0; i < 2; i++) {
...
void MpdFemtoBPLCMS3DCorrFctnKt::addRealPair(MpdFemtoPair *pair)
{
   ...
   if ((mIndexKt >= 0) && (mIndexKt < mNKtBins)) {
      ...
      if (mNumerator[mIndexKt]) {
         mNumerator[mIndexKt]->Fill(pair->qOutCMS(), pair->qSideCMS(), pair->qLongCMS(), 1.);
      }
   } // if ( (mIndexKt>=0) && (mIndexKt<mNKtBins) )
}
...
void MpdFemtoBPLCMS3DCorrFctnKt::addMixedPair(MpdFemtoPair *pair)
{
   ...
   if ((mIndexKt >= 0) && (mIndexKt < mNKtBins)) {
      ...
      if (mDenominator[mIndexKt]) {
         mDenominator[mIndexKt]->Fill(pair->qOutCMS(), pair->qSideCMS(), pair->qLongCMS(), 1.);
      }

      // Check that histrogram exists
      if (mQinvHisto[mIndexKt]) {
         mQinvHisto[mIndexKt]->Fill(pair->qOutCMS(), pair->qSideCMS(), pair->qLongCMS(), TMath::Abs(pair->qInv()));
      }
   } // if ( (mIndexKt>=0) && (mIndexKt<mNKtBins) )
}
