...
MpdFemtoBasicTrackCut::MpdFemtoBasicTrackCut()
   : MpdFemtoBaseTrackCut(), mType(true), mCharge(1), mNHitsRat(0), mDetSelection(3), mPidSelection(HbtPID::Pion),
     mNTracksPassed(0), mNTracksFailed(0)
{
   // Default constructor
   mNHits[0]    = 10;
   mNHits[1]    = 100;
...
MpdFemtoBasicTrackCut::MpdFemtoBasicTrackCut(const MpdFemtoBasicTrackCut &c)
   : MpdFemtoBaseTrackCut(c), mType(c.mType), mCharge(c.mCharge), mNHitsRat(c.mNHitsRat),
     mDetSelection(c.mDetSelection), mPidSelection(c.mPidSelection), mNTracksPassed(0), mNTracksFailed(0)
{
   // Copy constructor
...
bool MpdFemtoBasicTrackCut::pass(const MpdFemtoTrack *t)
{
   ...
   if (mDetSelection == 0) { // TPC selection
      ...
      std::cout << "[ERROR] MpdFemtoBasicTrackCut: Wrong particle identification scheme " << mDetSelection << std::endl;
      return false;
   }
...
MpdFemtoString MpdFemtoBasicTrackCut::report()
{
   ...
   report +=
      TString::Format("Particle TPC+TOF nSigma for electron:\t%E - %E\n", mTnTNSigmaElectron[0], mTnTNSigmaElectron[1]);
   ...
   report +=
      TString::Format("Particle TPC+TOF nSigmat for proton:\t%E - %E\n", mTnTNSigmaProton[0], mTnTNSigmaProton[1]);
   ...
   report +=
      TString::Format("Number of tracks which passed:\t%u  Number which failed:\t%u\n", mNTracksPassed, mNTracksFailed);
...
TList *MpdFemtoBasicTrackCut::listSettings()
{
   ...
   settings_list->AddVector(
      ...
      new TObjString(TString::Format("MpdFemtoBasicTrackCut.hbtpid=%d", mPidSelection)), NULL);
...
void MpdFemtoBasicTrackCut::setNHits(const short &lo, const short &hi)
{
   ...
   if (lo > hi) {
      std::cout << "[WARNING] MpdFemtoBasicTrackCut::setNHits() -- bad nhits limits (low,high): (" << low << "," << hi
                << ")" << std::endl;
   } else {
      low  = lo;
      high = hi;
   ...
   if (low <= 0) {
      ...
      mNHits[0] = ((low > std::numeric_limits<unsigned char>::max()) ? std::numeric_limits<unsigned char>::max()
                                                                     : (unsigned char)low);
   }
   ...
   if (high <= 0) {
      ...
      mNHits[1] = ((high > std::numeric_limits<unsigned char>::max()) ? std::numeric_limits<unsigned char>::max()
                                                                      : (unsigned char)high);
   }
}
...
void MpdFemtoBasicTrackCut::setCharge(const short &charge)
{
   ...
   if (charge == 0) {
      ...
      mCharge = ((charge < std::numeric_limits<char>::min()) ? -1 : (char)charge);
   } else {
      mCharge = ((charge > std::numeric_limits<char>::max()) ? +1 : (char)charge);
   }
}
...
bool MpdFemtoBasicTrackCut::isGoodCleanTpcPid(const MpdFemtoTrack *t, TLorentzVector vec)
{
   ...
   if (mPidSelection == HbtPID::Electron) { // Electron
      ...
      std::cout << "[ERROR] MpdFemtoBasicTrackCut::isGoodCleanTpcPid -- Wrong HbtPID selection" << mPidSelection
                << std::endl;
   }
   ...
   if (mVerbose) {
      std::cout
         ...
         << Form("nSigma(electron)   : %3.2f <= %3.2f <= %3.2f \t %s \n", mNSigmaElectron[0], t->nSigmaElectron(),
                 mNSigmaElectron[1],
                 (mNSigmaElectron[0] <= t->nSigmaElectron() && t->nSigmaElectron() <= mNSigmaElectron[1]) ? "true"
                                                                                                          : "false")
         ...
         << Form("nSigmaOther(pion)  : %3.2f <= %3.2f || %3.2f >= %3.2f \t %s \n", t->nSigmaPion(), mNSigmaOther[0],
                 t->nSigmaPion(), mNSigmaOther[0],
                 ((t->nSigmaPion() <= mNSigmaOther[0]) || (t->nSigmaPion() >= mNSigmaOther[1])) ? "true" : "false")
         << Form("nSigmaOther(kaon)  : %3.2f <= %3.2f || %3.2f >= %3.2f \t %s \n", t->nSigmaKaon(), mNSigmaOther[0],
                 t->nSigmaKaon(), mNSigmaOther[0],
                 ((t->nSigmaKaon() <= mNSigmaOther[0]) || (t->nSigmaKaon() >= mNSigmaOther[1])) ? "true" : "false")
         << Form("nSigmaOther(proton): %3.2f <= %3.2f || %3.2f >= %3.2f \t %s \n", t->nSigmaProton(), mNSigmaOther[0],
                 t->nSigmaProton(), mNSigmaOther[0],
                 ((t->nSigmaProton() <= mNSigmaOther[0]) || (t->nSigmaProton() >= mNSigmaOther[1])) ? "true" : "false");
   } // if ( mVerbose )

   goodTpcClean = (mNSigmaElectron[0] <= t->nSigmaElectron() && t->nSigmaElectron() <= mNSigmaElectron[1] &&
...
bool MpdFemtoBasicTrackCut::isTpcCleanPion(const MpdFemtoTrack *t, TLorentzVector vec)
{
   ...
   if (mVerbose) {
      std::cout
         ...
         << Form("nSigma(pion)       : %3.2f <= %3.2f <= %3.2f \t %s \n", mNSigmaPion[0], t->nSigmaPion(),
                 mNSigmaPion[1],
                 (mNSigmaPion[0] <= t->nSigmaPion() && t->nSigmaPion() <= mNSigmaPion[1]) ? "true" : "false")
         << Form("p                  : %3.2f <= %3.2f <= %3.2f \t %s \n", mTpcMom[0], vec.P(), mTpcMom[1],
                 (mTpcMom[0] <= vec.P() && vec.P() <= mTpcMom[1]) ? "true" : "false")
         << Form("nSigmaOther(electron)  : %3.2f <= %3.2f || %3.2f >= %3.2f \t %s \n", t->nSigmaElectron(),
                 mNSigmaOther[0], t->nSigmaElectron(), mNSigmaOther[0],
                 ((t->nSigmaElectron() <= mNSigmaOther[0]) || (t->nSigmaElectron() >= mNSigmaOther[1])) ? "true"
                                                                                                        : "false")
         << Form("nSigmaOther(kaon)  : %3.2f <= %3.2f || %3.2f >= %3.2f \t %s \n", t->nSigmaKaon(), mNSigmaOther[0],
                 t->nSigmaKaon(), mNSigmaOther[0],
                 ((t->nSigmaKaon() <= mNSigmaOther[0]) || (t->nSigmaKaon() >= mNSigmaOther[1])) ? "true" : "false")
   ...
   goodTpcClean = (mNSigmaPion[0] <= t->nSigmaPion() && t->nSigmaPion() <= mNSigmaPion[1] && (mTpcMom[0] <= vec.P()) &&
                   (vec.P() <= mTpcMom[1]) &&
...
bool MpdFemtoBasicTrackCut::isTpcCleanKaon(const MpdFemtoTrack *t, TLorentzVector vec)
{
   ...
   if (mVerbose) {
      std::cout
         ...
         << Form("nSigma(kaon)         : %3.2f <= %3.2f <= %3.2f \t %s \n", mNSigmaKaon[0], t->nSigmaKaon(),
                 mNSigmaKaon[1],
                 (mNSigmaKaon[0] <= t->nSigmaKaon() && t->nSigmaKaon() <= mNSigmaKaon[1]) ? "true" : "false")
         << Form("p                    : %3.2f <= %3.2f <= %3.2f \t %s \n", mTpcMom[0], vec.P(), mTpcMom[1],
                 (mTpcMom[0] <= vec.P() && vec.P() <= mTpcMom[1]) ? "true" : "false")
         << Form("nSigmaOther(electron): %3.2f <= %3.2f || %3.2f >= %3.2f \t %s \n", t->nSigmaElectron(),
                 mNSigmaOther[0], t->nSigmaElectron(), mNSigmaOther[0],
   ...
   goodTpcClean = (mNSigmaKaon[0] <= t->nSigmaKaon() && t->nSigmaKaon() <= mNSigmaKaon[1] && (mTpcMom[0] <= vec.P()) &&
                   (vec.P() <= mTpcMom[1]) &&
                   ((t->nSigmaElectron() <= mNSigmaOther[0]) || (t->nSigmaElectron() >= mNSigmaOther[1])) &&
                   ((t->nSigmaPion() <= mNSigmaOther[0]) || (t->nSigmaPion() >= mNSigmaOther[1])) &&
                   ((t->nSigmaProton() <= mNSigmaOther[0]) || (t->nSigmaProton() >= mNSigmaOther[1])));
...
bool MpdFemtoBasicTrackCut::isTpcCleanProton(const MpdFemtoTrack *t, TLorentzVector vec)
{
   ...
   if (mVerbose) {
      std::cout
         ...
         << Form("nSigma(proton)        : %3.2f <= %3.2f <= %3.2f \t %s \n", mNSigmaProton[0], t->nSigmaProton(),
                 mNSigmaProton[1],
                 (mNSigmaProton[0] <= t->nSigmaProton() && t->nSigmaProton() <= mNSigmaProton[1]) ? "true" : "false")
         << Form("p                     : %3.2f <= %3.2f <= %3.2f \t %s \n", mTpcMom[0], vec.P(), mTpcMom[1],
                 (mTpcMom[0] <= vec.P() && vec.P() <= mTpcMom[1]) ? "true" : "false")
         << Form("nSigmaOther(electron) : %3.2f <= %3.2f || %3.2f >= %3.2f \t %s \n", t->nSigmaElectron(),
                 mNSigmaOther[0], t->nSigmaElectron(), mNSigmaOther[0],
   ...
   goodTpcClean = (mNSigmaProton[0] <= t->nSigmaProton() && t->nSigmaProton() <= mNSigmaProton[1] &&
                   (mTpcMom[0] <= vec.P()) && (vec.P() <= mTpcMom[1]) &&
                   ((t->nSigmaElectron() <= mNSigmaOther[0]) || (t->nSigmaElectron() >= mNSigmaOther[1])) &&
                   ((t->nSigmaPion() <= mNSigmaOther[0]) || (t->nSigmaPion() >= mNSigmaOther[1])) &&
                   ((t->nSigmaKaon() <= mNSigmaOther[0]) || (t->nSigmaKaon() >= mNSigmaOther[1])));
...
bool MpdFemtoBasicTrackCut::isGoodTofPid(const MpdFemtoTrack *t)
{
   ...
   if (t->isPrimary() && t->isTofTrack()) {
      ...
      if (mPidSelection == HbtPID::Electron) {
         ...
         std::cout << "[WARNING] MpdFemtoBasicTrackCut::isGoodTofPid -- Wrong PID type selection: " << mPidSelection
                   << std::endl;
      }

      goodTOF =
         ((mTofMassSqr[0] <= t->massSqr()) && (t->massSqr() <= mTofMassSqr[1]) && (mInvBetaDiff[0] <= invBetaDiff) &&
          (invBetaDiff <= mInvBetaDiff[1]) && (mTofMom[0] <= t->pMom().Mag()) && (t->pMom().Mag() <= mTofMom[1]));

      if (mVerbose) {
         std::cout << Form("Mass^2                 : %3.2f <= %3.2f <= %3.2f \t %s \n", mTofMassSqr[0], t->massSqr(),
                           mTofMassSqr[1],
                           (mTofMassSqr[0] <= t->massSqr() && t->massSqr() <= mTofMassSqr[1]) ? "true" : "false")
                   << Form("1/beta-1/beta_expected : %4.3f <= %4.3f <= %4.3f \t %s \n", mInvBetaDiff[0], invBetaDiff,
                           mInvBetaDiff[0],
                           (mInvBetaDiff[0] <= invBetaDiff && invBetaDiff <= mInvBetaDiff[1]) ? "true" : "false")
                   << Form("p                      : %3.2f <= %3.2f <= %3.2f \t %s \n", mTofMom[0], t->pMom().Mag(),
                           mTofMom[0],
...
bool MpdFemtoBasicTrackCut::isTpcElectron(const MpdFemtoTrack *t)
{
   if (mVerbose) {
      std::cout << "isTpcElectron: \n"
                << Form("nSigma(electron) : %3.2f <= %3.2f <= %3.2f \t %s \n", mTnTNSigmaElectron[0],
                        t->nSigmaElectron(), mTnTNSigmaElectron[1],
                        (mTnTNSigmaElectron[0] <= t->nSigmaElectron() && t->nSigmaElectron() <= mTnTNSigmaElectron[1])
                           ? "true"
                           : "false");
   } // if ( mVerbose )

   return ((mTnTNSigmaElectron[0] <= t->nSigmaElectron()) && (t->nSigmaElectron() <= mTnTNSigmaElectron[1]));
}
   ...
   if (mVerbose) {
      std::cout << "isTpcPion: \n"
                << Form("nSigma(pion) : %3.2f <= %3.2f <= %3.2f \t %s \n", mTnTNSigmaPion[0], t->nSigmaPion(),
                        mTnTNSigmaPion[1],
                        (mTnTNSigmaPion[0] <= t->nSigmaPion() && t->nSigmaPion() <= mTnTNSigmaPion[1]) ? "true"
                                                                                                       : "false");
   } // if ( mVerbose )

   return ((mTnTNSigmaPion[0] <= t->nSigmaPion()) && (t->nSigmaPion() <= mTnTNSigmaPion[1]));
}
   ...
   if (mVerbose) {
      std::cout << "isTpcKaon: \n"
                << Form("nSigma(kaon) : %3.2f <= %3.2f <= %3.2f \t %s \n", mTnTNSigmaKaon[0], t->nSigmaKaon(),
                        mTnTNSigmaKaon[1],
                        (mTnTNSigmaKaon[0] <= t->nSigmaKaon() && t->nSigmaKaon() <= mTnTNSigmaKaon[1]) ? "true"
                                                                                                       : "false");
   } // if ( mVerbose )

   return ((mTnTNSigmaKaon[0] <= t->nSigmaKaon()) && (t->nSigmaKaon() <= mTnTNSigmaKaon[1]));
}
   ...
   if (mVerbose) {
      std::cout << "isTpcProton: \n"
                << Form("nSigma(proton) : %3.2f <= %3.2f <= %3.2f \t %s \n", mTnTNSigmaProton[0], t->nSigmaProton(),
                        mTnTNSigmaProton[1],
                        (mTnTNSigmaProton[0] <= t->nSigmaProton() && t->nSigmaProton() <= mTnTNSigmaProton[1])
                           ? "true"
                           : "false");
   } // if ( mVerbose )

   return ((mTnTNSigmaProton[0] <= t->nSigmaProton()) && (t->nSigmaProton() <= mTnTNSigmaProton[1]));
}
...
bool MpdFemtoBasicTrackCut::isGoodKine(const MpdFemtoTrack *t, TLorentzVector vec)
{
   ...
   return ((mNHits[0] <= t->nHits()) && (t->nHits() <= mNHits[1]) && (t->nHitsFit2PossRatio() >= mNHitsRat) &&
           (mPt[0] <= vec.Pt()) && (vec.Pt() <= mPt[1]) && (mP[0] <= vec.P()) && (vec.P() <= mP[1]) &&
           (mRapidity[0] <= vec.Rapidity()) && (vec.Rapidity() <= mRapidity[1]) && (mEta[0] <= vec.Eta()) &&
           (vec.Eta() <= mEta[1]) && (mDCA[0] <= t->gDCA().Mag()) && (t->gDCA().Mag() <= mDCA[1]));
}
