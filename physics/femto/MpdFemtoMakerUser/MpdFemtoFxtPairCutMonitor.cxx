...
MpdFemtoFxtPairCutMonitor::MpdFemtoFxtPairCutMonitor(const char *name)
   : MpdFemtoBaseCutMonitor(), mKt(nullptr), mPt1Pt2Qinv(nullptr), mFMRvsQinv(nullptr), mSLvsQinv(nullptr),
     mAveSepVsQinv(nullptr), mRValueVsQinv(nullptr), mDEtaDPhi(nullptr), mMinv(nullptr), mPtVsEta(nullptr)
{
   // Constructor
   string s("MpdFemtoFxtPairCutMonitor");
   string n(name);
   mKt         = new TH1F((s + n + "mKt").c_str(), "Kt;k_{T} (GeV/c)", 75, 0., 1.5);
   mPt1Pt2Qinv = new TH3F((s + n + "mPt1Pt2Qinv").c_str(),
                          "p_{T1} vs. p_{T2} vs. q_{inv};p_{T1} (GeV/c);p_{T2} (GeV/c);q_{inv} (GeV/c)", 32, 0., 1.6,
                          32, 0., 1.6, 400, 0., 0.8);
   mFMRvsQinv = new TH2F((s + n + "mFMRvsQinv").c_str(), "FMR vs. q_{inv};q_{Inv} (GeV/c);Fraction of merged rows", 200,
                         0., 0.8, 100, 0., 1.);
   mSLvsQinv  = new TH2F((s + n + "mSLvsQinv").c_str(), "SL vs. q_{inv};q_{inv} (GeV/c);Splitting level", 200, 0., 0.8,
                         32, -0.6, 1.);
   mAveSepVsQinv =
      new TH2F((s + n + "mAveSepVsQinv").c_str(),
               "AveSep vs. q_{inv};q_{inv} (GeV/c); Average two-track separation (cm)", 200, 0., 0.8, 100, 0., 50.);
   mRValueVsQinv = new TH2F((s + n + "mRValueVsQinv").c_str(),
                            "R=#sqrt{#Delta#eta+#Delta#phi} vs. q_{inv};q_{inv} (GeV/c);R", 200, 0., 0.8, 200, 0., 2.);
   mDEtaDPhi = new TH2F((s + n + "mDEtaDPhi").c_str(), "#Delta#eta vs. #Delta#phi;#Delta#phi;#Delta#eta", 400, -2., 2.,
                        300, -1.5, 1.5);
   mMinv     = new TH1F((s + n + "mMinv").c_str(), "M_{inv};M_{inv} (GeV/c^{2});Entries", 200, 0., 2.);
   mPtVsEta  = new TH2F((s + n + "mPtVsEta").c_str(), "p_{T} vs. #eta;#eta;p_{T} (GeV/c)", 60, -1.5, 1.5, 40, 0., 2.);
   mDEtaVsDPhiStarMin =
      new TH2F((s + n + "mDEtaVsDPhiStarMin").c_str(), "#Delta#eta vs. #Delta#phi*min;#Delta#phi*min (rad);#Delta#eta",
...
void MpdFemtoFxtPairCutMonitor::fill(const MpdFemtoPair *pair)
{
   ...
   mPt1Pt2Qinv->Fill(pair->track1()->track()->pt(), pair->track2()->track()->pt(), pair->qInv());
