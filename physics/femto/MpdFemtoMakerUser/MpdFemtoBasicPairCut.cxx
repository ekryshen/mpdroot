...
bool MpdFemtoBasicPairCut::pass(const MpdFemtoPair *pair)
{
   ...
   float dPhiStarMin = MpdFemtoPair::calculateDPhiStarMin(pair->track1()->momentum(), pair->track1()->track()->charge(),
                                                          pair->track2()->momentum(), pair->track2()->track()->charge(),
                                                          0.1, 0.6, 2.,                           /* in meters */
   ...
   Bool_t   isInside =
      (dEta >= mDEta[0] && dEta <= mDEta[1] && dPhiStarMin >= mDPhiStarMin[0] && dPhiStarMin <= mDPhiStarMin[1]);

   bool mGoodPair =
      ((pair->quality() >= mQuality[0]) && (pair->quality() <= mQuality[1]) && (pair->kT() >= mKt[0]) &&
       (pair->kT() <= mKt[1]) && (pair->pT() >= mPt[0]) && (pair->pT() <= mPt[1]) &&
       (pair->openingAngle() >= mOpeningAngle[0]) && (pair->openingAngle() <= mOpeningAngle[1]) &&
       (pair->rapidity() >= mRapidity[0]) && (pair->rapidity() <= mRapidity[1]) && (pair->eta() >= mEta[0]) &&
       (pair->eta() <= mEta[1]) && (pair->qInv() >= mQinv[0]) && (pair->qInv() <= mQinv[1]) &&
       (pair->mInv() >= mMinv[0]) && (pair->mInv() <= mMinv[1]) &&
       (pair->emissionAngle() >= mAngleToPrimaryVertex[0]) && (pair->emissionAngle() <= mAngleToPrimaryVertex[1]) &&
       (pair->nominalTpcEntranceSeparation() >= mEntranceSeparation[0]) &&
       (pair->nominalTpcEntranceSeparation() <= mEntranceSeparation[1]) &&
       (pair->fractionOfMergedRow() >= mFracOfMergedRow[0]) && (pair->fractionOfMergedRow() <= mFracOfMergedRow[1]) &&
       (pair->closestRowAtDCA() >= mClosestRowAtDCA[0]) && (pair->closestRowAtDCA() <= mClosestRowAtDCA[1]) &&
       (pair->weightedAvSep() >= mWeightedAvSep[0]) && (pair->weightedAvSep() <= mWeightedAvSep[1]) &&
       (pair->nominalTpcAverageSeparation() >= mAverageSeparation[0]) &&
       (pair->nominalTpcAverageSeparation() <= mAverageSeparation[1]) && (pair->rValue() >= mRValueLo) && !isInside);
   ...
   if (mVerbose) {
      std::cout
         ...
         << Form("Opening angle  : %4.2f <= %4.2f <= %4.2f \t pass: %s \n", mOpeningAngle[0], pair->openingAngle(),
                 mOpeningAngle[1],
                 (pair->openingAngle() >= mOpeningAngle[0] && pair->openingAngle() <= mOpeningAngle[1]) ? "true"
                                                                                                        : "false")
         << Form("Rapidity       : %4.3f <= %4.3f <= %4.3f \t pass: %s \n", mRapidity[0], pair->rapidity(),
                 mRapidity[1],
         ...
         << Form("ClosesRowAtDCA : %4.3f <= %4.3f <= %4.3f \t pass: %s \n", mClosestRowAtDCA[0],
                 ...
                 (pair->closestRowAtDCA() >= mClosestRowAtDCA[0] && pair->closestRowAtDCA() <= mClosestRowAtDCA[1])
                    ? "true"
                    : "false")
         << Form("WeightedAverSep: %5.2f <= %5.2f <= %5.2f \t pass: %s \n", mWeightedAvSep[0], pair->weightedAvSep(),
                 mWeightedAvSep[1],
                 (pair->weightedAvSep() >= mWeightedAvSep[0] && pair->weightedAvSep() <= mWeightedAvSep[1]) ? "true"
                                                                                                            : "false")
         << Form("AverageSep     : %5.2f <= %5.2f <= %5.2f \t pass: %s \n", mAverageSeparation[0],
                 pair->nominalTpcAverageSeparation(), mAverageSeparation[1],
                 (pair->nominalTpcAverageSeparation() >= mAverageSeparation[0] &&
                  pair->nominalTpcAverageSeparation() <= mAverageSeparation[1])
                    ? "true"
                    : "false")
         << Form("R value        : %4.2f <= %4.2f \t pass: %s \n", mRValueLo, pair->rValue(),
                 (pair->rValue() >= mRValueLo) ? "true" : "false")
         << Form("dPhi* min      : %4.3f <= %4.3f <= %4.3f \t pass: %s \n", mDPhiStarMin[0], dPhiStarMin,
                 mDPhiStarMin[1], (dPhiStarMin >= mDPhiStarMin[0] && dPhiStarMin <= mDPhiStarMin[1]) ? "true" : "false")
         << std::endl
         << std::endl;
   } // if ( mVerbose )
...
#define NEWOBJ(var)                                                                 \
   new TObjString(TString::Format("MpdFemtoBasicPairCut." #var ".min=%f", var[0])), \
      new TObjString(TString::Format("MpdFemtoBasicPairCut." #var ".max=%f", var[1]))

   settings_list->AddVector(NEWOBJ(mQuality), NEWOBJ(mKt), NEWOBJ(mPt), NEWOBJ(mOpeningAngle), NEWOBJ(mRapidity),
                            NEWOBJ(mEta), NEWOBJ(mQinv), NEWOBJ(mMinv), NEWOBJ(mEntranceSeparation),
                            NEWOBJ(mAverageSeparation),
                            new TObjString(TString::Format("MpdFemtoBasicPairCut.mRValueLo=%f", mRValueLo)), NULL);
