...
//_________________
MpdFemtoModelBPLCMS3DCorrFctnKt::MpdFemtoModelBPLCMS3DCorrFctnKt(const char *title, const int &nBins, const double &qLo,
                                                                 const double &qHi, const int &ktBins,
                                                                 const double &ktLo, const double &ktHi,
                                                                 const bool isUseDenominator)
   : MpdFemtoBaseCorrFctn(), mManager(nullptr)
{
   ...
   for (int iKtBin = 0; iKtBin < mNKtBins; iKtBin++) {
      ...
      baseTitle += appendix;
      TH3F *histoNum = new TH3F(baseName.Data(), baseTitle.Data(), nBins, qLo, qHi, nBins, qLo, qHi, nBins, qLo, qHi);
      histoNum->Sumw2();
      ...
      baseTitle += Form("%3.2f", (mKtRange[0] + mKtStep * (iKtBin + 1)));
      baseTitle += appendix;
      TH3F *histoNumW = new TH3F(baseName.Data(), baseTitle.Data(), nBins, qLo, qHi, nBins, qLo, qHi, nBins, qLo, qHi);
      histoNumW->Sumw2();
      ...
      baseTitle += appendix;
      TH3F *histoNumQinvW =
         new TH3F(baseName.Data(), baseTitle.Data(), nBins, qLo, qHi, nBins, qLo, qHi, nBins, qLo, qHi);
      histoNumQinvW->Sumw2();
...
MpdFemtoModelBPLCMS3DCorrFctnKt::MpdFemtoModelBPLCMS3DCorrFctnKt(const MpdFemtoModelBPLCMS3DCorrFctnKt &copy)
   : MpdFemtoBaseCorrFctn(copy), mManager(copy.mManager), mNKtBins(copy.mNKtBins), mKtStep(copy.mKtStep),
     mIsUseDenominator(copy.mIsUseDenominator)
{
   ...
   for (int iCF = 0; iCF < mNKtBins; iCF++) {
      mNumerator.push_back((copy.mNumerator.at(iCF)) ? new TH3F(*copy.mNumerator.at(iCF)) : nullptr);
      mNumeratorWeighted.push_back((copy.mNumeratorWeighted.at(iCF)) ? new TH3F(*copy.mNumeratorWeighted.at(iCF))
                                                                     : nullptr);
      mNumeratorQinvWeighted.push_back(
         (copy.mNumeratorQinvWeighted.at(iCF)) ? new TH3F(*copy.mNumeratorQinvWeighted.at(iCF)) : nullptr);

      if (mIsUseDenominator) {
         mDenominator.push_back((copy.mDenominator.at(iCF)) ? new TH3F(*copy.mDenominator.at(iCF)) : nullptr);
         mDenominatorWeighted.push_back(
            (copy.mDenominatorWeighted.at(iCF)) ? new TH3F(*copy.mDenominatorWeighted.at(iCF)) : nullptr);
         mDenominatorQinvWeighted.push_back(
            (copy.mDenominatorQinvWeighted.at(iCF)) ? new TH3F(*copy.mDenominatorQinvWeighted.at(iCF)) : nullptr);
      } // if ( mIsUseDenominator )
   }    // for ( int iCF=0; iCF<mNKtBins; iCF++ )
}
...
void MpdFemtoModelBPLCMS3DCorrFctnKt::addRealPair(MpdFemtoPair *pair)
{
   ...
   if ((mIndexKt >= 0) && (mIndexKt < mNKtBins)) {
      ...
      if (mNumerator.at(mIndexKt)) {
         mNumerator.at(mIndexKt)->Fill(pair->qOutCMS(), pair->qSideCMS(), pair->qLongCMS(), 1.);
      } // if ( mNumerator[mIndexKt] )

      // Check that histrogram exists
      if (mNumeratorWeighted.at(mIndexKt)) {
         double weight = mManager->weight(pair);
         mNumeratorWeighted.at(mIndexKt)->Fill(pair->qOutCMS(), pair->qSideCMS(), pair->qLongCMS(), weight);
      } // if ( mNumeratorWeighted[mIndexKt] )

      // Check that histogram exists
      if (mNumeratorQinvWeighted.at(mIndexKt)) {
         mNumeratorQinvWeighted.at(mIndexKt)->Fill(pair->qOutCMS(), pair->qSideCMS(), pair->qLongCMS(), pair->qInv());
      }
   } // if ( (mIndexKt>=0) && (mIndexKt<mNKtBins) )
}
...
void MpdFemtoModelBPLCMS3DCorrFctnKt::addMixedPair(MpdFemtoPair *pair)
{
   ...
   if ((mIndexKt >= 0) && (mIndexKt < mNKtBins)) {
      ...
      if (mDenominator.at(mIndexKt)) {
         mDenominator.at(mIndexKt)->Fill(pair->qOutCMS(), pair->qSideCMS(), pair->qLongCMS(), 1.);
      } // if ( mDenominator[mIndexKt] )

      // Check that histrogram exists
      if (mDenominatorWeighted.at(mIndexKt)) {
         double weight = mManager->weight(pair);
         mDenominatorWeighted.at(mIndexKt)->Fill(pair->qOutCMS(), pair->qSideCMS(), pair->qLongCMS(), weight);
      } // if ( mDenominatorWeighted[mIndexKt] )

      // Check that histrogram exists
      if (mDenominatorQinvWeighted.at(mIndexKt)) {
         mDenominatorQinvWeighted.at(mIndexKt)->Fill(pair->qOutCMS(), pair->qSideCMS(), pair->qLongCMS(), pair->qInv());
