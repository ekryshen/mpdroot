...
MpdFemtoSmearPair::MpdFemtoSmearPair()
   : mSmearedPair(), mParticle1(), mParticle2(), mFracPtRes(0), mPhi_a(0), mPhi_b(0), mPhi_alpha(0), mTheta_a(0),
     mTheta_b(0), mTheta_alpha(0), mHbtRandom(nullptr)
{
   // Default constructor
   setup();
   if (mHbtRandom) {
...
TLorentzVector MpdFemtoSmearPair::smearedMomentum(TLorentzVector fourmom)
{
   ...
   fourmom.SetE(TMath::Sqrt(mass2 + fourmom.X() * fourmom.X() + fourmom.Y() * fourmom.Y() + fourmom.Z() * fourmom.Z()));
   return fourmom;
}
