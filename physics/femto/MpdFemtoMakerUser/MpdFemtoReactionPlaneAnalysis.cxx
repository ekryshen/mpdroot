...
//_________________
MpdFemtoReactionPlaneAnalysis::MpdFemtoReactionPlaneAnalysis(unsigned int binsVertex, double minVertex,
                                                             double maxVertex, unsigned int binsMult, double minMult,
                                                             double maxMult, unsigned short binsRP, double minRP,
                                                             double maxRP)
   : MpdFemtoAnalysis(), mVertexZBins(binsVertex), mOverFlowVertexZ(0), mUnderFlowVertexZ(0), mMultBins(binsMult),
     mOverFlowMult(0), mUnderFlowMult(0), mRPBins(binsRP), mCurrentRP(0)
{
   // Constructor
   ...
   if (minVertex >= maxVertex) {
      std::cout << "[WARNING] MpdFemtoVertexMultAnalysis - wrong z-vertex positions (" << minVertex
                << " >= " << maxVertex << "). No events are expected to pass." << std::endl;
   }

   if (minMult >= maxMult) {
      std::cout << "[WARNING] MpdFemtoVertexMultAnalysis - wrong multiplicity intervals (" << minMult
                << " >= " << maxMult << "). No events are expected to pass." << std::endl;
   }
   ...
   mPicoEventCollectionVectorHideAway = new MpdFemtoPicoEventCollectionVectorHideAway(
      mVertexZBins, mVertexZ[0], mVertexZ[1], mMultBins, mMult[0], mMult[1], mRPBins, mRP[0], mRP[1]);
};

//_________________
MpdFemtoReactionPlaneAnalysis::MpdFemtoReactionPlaneAnalysis(const MpdFemtoReactionPlaneAnalysis &a)
   : MpdFemtoAnalysis(), mVertexZBins(a.mVertexZBins), mOverFlowVertexZ(0), mUnderFlowVertexZ(0),
     mMultBins(a.mMultBins), mOverFlowMult(0), mUnderFlowMult(0), mRPBins(a.mRPBins), mCurrentRP(0)
{
   // Copy constructor
