...
//_________________
MpdFemtoFxtTrackCutMonitor::MpdFemtoFxtTrackCutMonitor(const char *name, const double ParticleMass)
   : MpdFemtoBaseCutMonitor(), mDCAGlobal(nullptr), mNhits(nullptr), mP(nullptr), mPt(nullptr),
     mPtVsNsigmaPion(nullptr), mPtVsNsigmaKaon(nullptr), mPtVsNsigmaProton(nullptr), mPvsDedx(nullptr),
     mRapidity(nullptr), mPseudoRapidity(nullptr), mPvsMassSqr(nullptr), mPvsInvBeta(nullptr), mPtVsEta(nullptr),
     mInvBetaDiffPionVsP(nullptr), mInvBetaDiffKaonVsP(nullptr), mInvBetaDiffProtonVsP(nullptr), monMass(ParticleMass)
{
   // Constructor
   string s("MpdFemtoFxtTrackCutMonitor");
   string n(name);
   mDCAGlobal      = new TH1F((s + n + "mDCAGlobal").c_str(), "DCA Global; DCA Global (cm)", 100, 0., 5.);
   mNhits          = new TH1F((s + n + "mNhits").c_str(), "nHits;nHits", 80, -0.5, 79.5);
   mP              = new TH1F((s + n + "mP").c_str(), "Momentum;p (GeV/c);Entries", 76, 0., 1.9);
   mPt             = new TH1F((s + n + "mPt").c_str(), "Transverse momentum;p_{T} (GeV/c);Entries", 76, 0., 1.9);
   mPtVsNsigmaPion = new TH2F((s + n + "mPtVsNsigmaPion").c_str(), "n#sigma(#pi) vs. p_{T};p_{T} (GeV/c);n#sigma(#pi)",
                              76, 0., 1.9, 100, -10., 10.);
   mPtVsNsigmaKaon = new TH2F((s + n + "mPtVsNsigmaKaon").c_str(), "n#sigma(K) vs. p_{T};p_{T} (GeV/c);n#sigma(K)", 76,
                              0., 1.9, 100, -10., 10.);
   mPtVsNsigmaProton = new TH2F((s + n + "mPtVsNsigmaProton").c_str(), "n#sigma(p) vs. p_{T};p_{T} (GeV/c);n#sigma(p)",
                                76, 0., 1.9, 100, -10., 10.);
   mPvsDedx  = new TH2F((s + n + "mPvsDedx").c_str(), "dE/dx vs. momentum;p (GeV/c);dE/dx (keV/cm)", 76, 0.0, 1.9, 100,
                        0., 15000.);
   mRapidity = new TH1F((s + n + "mRapidity").c_str(), "Rapidity;y;Entries", 56, -1.2, 1.2);
   mPseudoRapidity = new TH1F((s + n + "mPseudoRapidity").c_str(), "Pseudorapidity;#eta;Entries", 56, -1.2, 1.2);
   mPvsMassSqr = new TH2F((s + n + "mPvsMassSqr").c_str(), "m^{2} vs. p_{T};p_{T} (GeV/c);m^{2} (GeV/c^{2})^{2}", 76,
                          0., 1.9, 120, -0.1, 1.1);
   mPvsInvBeta = new TH2F((s + n + "mPvsInvBeta").c_str(), "1/#beta vs. p;p (GeV/c);1/#beta", 76, 0., 1.9, 60, 0.8, 2.);
   mPtVsEta = new TH2F((s + n + "mPtVsEta").c_str(), "#eta vs. p_{T};#eta;p_{T} (GeV/c)", 44, -1.1, 1.1, 76, 0., 1.9);
   mInvBetaDiffPionVsP =
      new TH2F((s + n + "mInvBetaDiffPionVsP").c_str(), "1/#beta-1/#beta(#pi) vs. p;p (GeV/c);1/#beta-1/#beta(#pi)", 76,
               0., 1.9, 200, -0.1, 0.1);
   mInvBetaDiffKaonVsP = new TH2F((s + n + "mInvBetaDiffKaonVsP").c_str(),
                                  "1/#beta-1/#beta(K) vs. p;p (GeV/c);1/#beta-1/#beta(K)", 76, 0., 1.9, 200, -0.1, 0.1);
   mInvBetaDiffProtonVsP =
      new TH2F((s + n + "mInvBetaDiffProtonVsP").c_str(), "1/#beta-1/#beta(p) vs. p;p (GeV/c);1/#beta-1/#beta(p)", 76,
               0., 1.9, 200, -0.1, 0.1);
}
...
void MpdFemtoFxtTrackCutMonitor::fill(const MpdFemtoTrack *track)
{
   ...
   */
   ...
   mInvBetaDiffPionVsP->Fill(
      track->ptot(), track->invBeta() - TMath::Sqrt(M_PION_PLUS * M_PION_PLUS + track->p().Mag2()) / track->ptot());
   mInvBetaDiffKaonVsP->Fill(
      track->ptot(), track->invBeta() - TMath::Sqrt(M_KAON_PLUS * M_KAON_PLUS + track->p().Mag2()) / track->ptot());
   mInvBetaDiffProtonVsP->Fill(track->ptot(),
                               track->invBeta() - TMath::Sqrt(M_PROTON * M_PROTON + track->p().Mag2()) / track->ptot());
}
