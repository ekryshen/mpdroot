...
//_________________
MpdFemtoDeltaEtaDeltaPhiStarMinKt::MpdFemtoDeltaEtaDeltaPhiStarMinKt(const char *title, const int &nBinsEta,
                                                                     const double &etaLo, const double &etaHi,
                                                                     const int &nBinsPhi, const double &phiLo,
                                                                     const double &phiHi, const int &ktBins,
   ...
   : MpdFemtoBaseCorrFctn()
{
   ...
   for (int iKtBin = 0; iKtBin < mNKtBins; iKtBin++) {
      ...
      baseTitle += appendix;
      TH2F *hNumer = new TH2F(baseName.Data(), baseTitle.Data(), mPhiBins, mPhiRange[0], mPhiRange[1], mEtaBins,
                              mEtaRange[0], mEtaRange[1]);
      hNumer->Sumw2();
      ...
      baseTitle += Form("%3.2f", (mKtRange[0] + mKtStep * (iKtBin + 1)));
      baseTitle += appendix;
      TH2F *hDenom = new TH2F(baseName.Data(), baseTitle.Data(), mPhiBins, mPhiRange[0], mPhiRange[1], mEtaBins,
                              mEtaRange[0], mEtaRange[1]);
      hDenom->Sumw2();
...
{
   // Copy constructor
   setHistoParameters(copy.mEtaBins, copy.mEtaRange[0], copy.mEtaRange[1], copy.mPhiBins, copy.mPhiRange[0],
                      copy.mPhiRange[1]);
...
MpdFemtoDeltaEtaDeltaPhiStarMinKt &MpdFemtoDeltaEtaDeltaPhiStarMinKt::operator=(
   const MpdFemtoDeltaEtaDeltaPhiStarMinKt &copy)
{
   // Assignment operator
   if (this != &copy) {
...
//_________________
void MpdFemtoDeltaEtaDeltaPhiStarMinKt::setRadiiParameters(const int &npoints, const double &radiusLo,
...
void MpdFemtoDeltaEtaDeltaPhiStarMinKt::addRealPair(MpdFemtoPair *pair)
{
   ...
   if ((mIndexKt >= 0) && (mIndexKt < mNKtBins) && pair->qInv() < 0.15) {
      ...
      dPhiStarMin = MpdFemtoPair::calculateDPhiStarMin(mMomentum1, mQ1, mMomentum2, mQ2, mRadiusStep, mRadii[0],
                                                       mRadii[1], mBField);
...
//_________________
void MpdFemtoDeltaEtaDeltaPhiStarMinKt::setHistoParameters(const int &nBinsEta, const double &etaLo,
                                                           const double &etaHi, const int &nBinsPhi,
                                                           const double &phiLo, const double &phiHi)
{
   if (etaLo > etaHi) {
