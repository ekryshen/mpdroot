...
//_________________
MpdFemtoModelQinvCorrFctnKt::MpdFemtoModelQinvCorrFctnKt(const char *title, const int &nQbins, const double &qLo,
                                                         const double &qHi, const int &nKtBins, const double &ktLo,
                                                         const double &ktHi, const bool useDenominator)
   : MpdFemtoBaseCorrFctn(), mManager(nullptr)
{
   ...
   for (int iKtBin = 0; iKtBin < mNKtBins; iKtBin++) {
      ...
      baseTitle += appendix;
      TH1D *histoNum = new TH1D(baseName.Data(), baseTitle.Data(), nQbins, qLo, qHi);
      histoNum->Sumw2();
      ...
      baseTitle += Form("%3.2f", (mKtRange[0] + mKtStep * (iKtBin + 1)));
      baseTitle += appendix;
      TH1D *histoNumW = new TH1D(baseName.Data(), baseTitle.Data(), nQbins, qLo, qHi);
      histoNumW->Sumw2();
      ...
      if (mIsUseDenominator) {
         ...
         baseTitle += Form("%3.2f", (mKtRange[0] + mKtStep * (iKtBin + 1)));
         baseTitle += appendix;
         TH1D *histoDen = new TH1D(baseName.Data(), baseTitle.Data(), nQbins, qLo, qHi);
         histoDen->Sumw2();
         ...
         baseTitle += Form("%3.2f", (mKtRange[0] + mKtStep * (iKtBin + 1)));
         baseTitle += appendix;
         TH1D *histoDenW = new TH1D(baseName.Data(), baseTitle.Data(), nQbins, qLo, qHi);
         histoDenW->Sumw2();
...
MpdFemtoModelQinvCorrFctnKt::MpdFemtoModelQinvCorrFctnKt(const MpdFemtoModelQinvCorrFctnKt &c)
   : MpdFemtoBaseCorrFctn(c), mManager(c.mManager), mNKtBins(c.mNKtBins), mKtStep(c.mKtStep),
     mIsUseDenominator(c.mIsUseDenominator)
{
   ...
   for (int iCF = 0; iCF < mNKtBins; iCF++) {
      ...
      if (mIsUseDenominator) {
         ...
         mDenominatorWeighted.push_back((c.mDenominatorWeighted.at(iCF)) ? new TH1D(*c.mDenominatorWeighted.at(iCF))
                                                                         : nullptr);
      }
...
MpdFemtoModelQinvCorrFctnKt &MpdFemtoModelQinvCorrFctnKt::operator=(const MpdFemtoModelQinvCorrFctnKt &c)
{
   ...
   if (this != &c) {
      ...
      for (int iCF = 0; iCF < mNKtBins; iCF++) {
         mNumerator.push_back((c.mNumerator.at(iCF)) ? new TH1D(*c.mNumerator.at(iCF)) : nullptr);
         mNumeratorWeighted.push_back((c.mNumeratorWeighted.at(iCF)) ? new TH1D(*c.mNumeratorWeighted.at(iCF))
                                                                     : nullptr);
         if (mIsUseDenominator) {
            mDenominator.push_back((c.mDenominator.at(iCF)) ? new TH1D(*c.mDenominator.at(iCF)) : nullptr);
            mDenominatorWeighted.push_back((c.mDenominatorWeighted.at(iCF)) ? new TH1D(*c.mDenominatorWeighted.at(iCF))
   ...
   if ((mIndexKt >= 0) && (mIndexKt < mNKtBins)) {
      if (mNumerator.at(mIndexKt)) {
         ...
         std::cout << "[ERROR] void MpdFemtoModelQinvCorrFctnKt::addRealPair(MpdFemtoPair* pair) - "
                   << "No correlation function with index: " << mIndexKt << " was found" << std::endl;
      }
      if (mNumeratorWeighted.at(mIndexKt)) {
         ...
         std::cout << "[ERROR] void MpdFemtoModelQinvCorrFctnKt::addRealPair(MpdFemtoPair* pair) - "
                   << "No weighted correlation function with index: " << mIndexKt << " was found" << std::endl;
      }
