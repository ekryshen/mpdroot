...
public:
   ...
   TVector3 decayVectorXi() const { return (mBachelor) ? (decayVertexXi() - primaryVertex()) : TVector3(0., 0., 0.); }
   ...
   float massXi() const { return TMath::Sqrt(TMath::Power((eBacPion() + eLambda()), 2) + ptot2Xi()); }
   /// Mass assuming (anti) Omega hypothesis
   float massOmega() const { return TMath::Sqrt(TMath::Power((eBacKaon() + eLambda()), 2) + ptot2Xi()); }
   ...
   {
      return ((mBachelor) ? TVector3(mMomBacAtDca2DecayPointX, mMomBacAtDca2DecayPointY, mMomBacAtDca2DecayPointX)
                          : TVector3(0, 0, 0));
   }
   ...
   unsigned long trackTopologyMapBac() { return (mBachelor) ? mBachelor->topologyMap() : 0; }
   ...
   float eBacKaon() const { return (mBachelor) ? TMath::Sqrt(ptot2Bac() + M_KAON_PLUS * M_KAON_PLUS) : 0; }

   float eBacPion() const { return (mBachelor) ? TMath::Sqrt(ptot2Bac() + M_PION_PLUS * M_PION_PLUS) : 0; }
