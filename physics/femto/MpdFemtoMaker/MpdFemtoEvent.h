...
public:
   ...
   MpdFemtoEvent();
   /// Copy constructor
   MpdFemtoEvent(const MpdFemtoEvent &ev, MpdFemtoBaseTrackCut *track = nullptr, MpdFemtoBaseV0Cut *v0 = nullptr,
                 MpdFemtoBaseXiCut *xi = nullptr, MpdFemtoBaseKinkCut *kink = nullptr);
   /// Assignment operator
   ...
   {
      mRefMult =
         ((mult > 0) ? ((mult > std::numeric_limits<unsigned short>::max()) ? std::numeric_limits<unsigned short>::max()
                                                                            : (unsigned short)mult)
                     : 0);
   }
   /// Set reference multiplicity
   ...
   {
      mZdcSumAdcEast =
         ((adc > 0) ? ((adc > std::numeric_limits<unsigned short>::max()) ? std::numeric_limits<unsigned short>::max()
                                                                          : (unsigned short)adc)
                    : 0);
   }
   /// Set ADC of east ZDC
