...
MpdFemtoModelHiddenInfo::MpdFemtoModelHiddenInfo()
   : mTrueMomentumX(nullptr), mTrueMomentumY(nullptr), mTrueMomentumZ(nullptr), mMotherMomentumX(nullptr),
     mMotherMomentumY(nullptr), mMotherMomentumZ(nullptr), mEmissionPointX(nullptr), mEmissionPointY(nullptr),
     mEmissionPointZ(nullptr), mEmissionPointT(nullptr), mPDGPid(0), mMotherPdg(0), mTrueMomentumPosX(nullptr),
     mTrueMomentumPosY(nullptr), mTrueMomentumPosZ(nullptr), mEmissionPointPosX(nullptr), mEmissionPointPosY(nullptr),
     mEmissionPointPosZ(nullptr), mEmissionPointPosT(nullptr), mPDGPidPos(0), mTrueMomentumNegX(nullptr),
     mTrueMomentumNegY(nullptr), mTrueMomentumNegZ(nullptr), mEmissionPointNegX(nullptr), mEmissionPointNegY(nullptr),
     mEmissionPointNegZ(nullptr), mEmissionPointNegT(nullptr), mPDGPidNeg(0), mOrigin(0)
{
   // Default constructor
}
...
MpdFemtoModelHiddenInfo::MpdFemtoModelHiddenInfo(const MpdFemtoModelHiddenInfo &aInfo)
   : MpdFemtoHiddenInfo(aInfo), mTrueMomentumX(nullptr), mTrueMomentumY(nullptr), mTrueMomentumZ(nullptr),
     mMotherMomentumX(nullptr), mMotherMomentumY(nullptr), mMotherMomentumZ(nullptr), mEmissionPointX(nullptr),
     mEmissionPointY(nullptr), mEmissionPointZ(nullptr), mEmissionPointT(nullptr), mPDGPid(aInfo.mPDGPid),
     mMotherPdg(aInfo.mMotherPdg), mTrueMomentumPosX(nullptr), mTrueMomentumPosY(nullptr), mTrueMomentumPosZ(nullptr),
     mEmissionPointPosX(nullptr), mEmissionPointPosY(nullptr), mEmissionPointPosZ(nullptr), mEmissionPointPosT(nullptr),
     mPDGPidPos(aInfo.mPDGPidPos), mTrueMomentumNegX(nullptr), mTrueMomentumNegY(nullptr), mTrueMomentumNegZ(nullptr),
     mEmissionPointNegX(nullptr), mEmissionPointNegY(nullptr), mEmissionPointNegZ(nullptr), mEmissionPointNegT(nullptr),
     mPDGPidNeg(aInfo.mPDGPidNeg), mOrigin(aInfo.mOrigin)
{
   // Copy constructor
...
//_________________
void MpdFemtoModelHiddenInfo::setTrueMomentum(const double &aPx, const double &aPy, const double &aPz)
{
...
//_________________
void MpdFemtoModelHiddenInfo::setMotherMomentum(const double &aPx, const double &aPy, const double &aPz)
{
   // Set momentum from components
   if (mMotherMomentumX == nullptr) mMotherMomentumX = new float;
   if (mMotherMomentumY == nullptr) mMotherMomentumY = new float;
...
//_________________
void MpdFemtoModelHiddenInfo::setEmissionPoint(const double &aRx, const double &aRy, const double &aRz,
                                               const double &aT)
{
...
//_________________
void MpdFemtoModelHiddenInfo::setTrueMomentumPos(const double &aPx, const double &aPy, const double &aPz)
{
   // Set momentum from components
   if (!mTrueMomentumPosX) mTrueMomentumPosX = new float;
   if (!mTrueMomentumPosY) mTrueMomentumPosY = new float;
...
//_________________
void MpdFemtoModelHiddenInfo::setEmissionPointPos(const double &aRx, const double &aRy, const double &aRz,
                                                  const double &aT)
{
   // Set position from components
   if (!mEmissionPointPosX) mEmissionPointPosX = new float;
...
//_________________
void MpdFemtoModelHiddenInfo::setTrueMomentumNeg(const double &aPx, const double &aPy, const double &aPz)
{
   // Set momentum from components
   if (!mTrueMomentumNegX) mTrueMomentumNegX = new float;
   if (!mTrueMomentumNegY) mTrueMomentumNegY = new float;
...
//_________________
void MpdFemtoModelHiddenInfo::setEmissionPointNeg(const double &aRx, const double &aRy, const double &aRz,
                                                  const double &aT)
{
   // Set position from components
   if (!mEmissionPointNegX) mEmissionPointNegX = new float;
