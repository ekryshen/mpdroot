...
public:
   ...
   MpdFemtoTrack *track() const { return mTrack; }
   /// Return pointer to V0
   MpdFemtoV0 *v0() const { return mV0; }
   /// Return pointer to kink
   MpdFemtoKink *kink() const { return mKink; }
   /// Return pointer to xi
   MpdFemtoXi *xi() const { return mXi; }
   ...
   /// Helix of the track
   MpdFemtoPhysicalHelix helix() const { return mTrack ? mTrack->helix() : MpdFemtoPhysicalHelix(); }
   /// Topology map of the track
   unsigned long topologyMap() const { return mTrack ? mTrack->topologyMap() : 0; }
   ...
   /// Decay point of V0
   TVector3 secondaryVertex() const { return TVector3(secondaryVertexX(), secondaryVertexY(), secondaryVertexZ()); }
   ...
   void calculateTpcExitAndEntrancePoints(MpdFemtoPhysicalHelix *tHelix, TVector3 *PrimVert, TVector3 *SecVert,
                                          TVector3 *tmpTpcEntrancePoint, TVector3 *tmpTpcExitPoint,
                                          TVector3 *tmpPosSample, float *tmpZ, float *tmpU, int *tmpSect);
   ...
   /// Set decay point
   void setDecayPoint(MpdFemtoV0 *v0) { setSecondaryVertex(v0); }
   ...
   MpdFemtoHiddenInfo *hiddenInfo() const { return mHiddenInfo; }
   /// Retrieve hidden information
   MpdFemtoHiddenInfo *getHiddenInfo() const { return hiddenInfo(); }
   /// Check if hidden information is valid
   bool validHiddenInfo() const { return (mHiddenInfo) ? true : false; }
   /// Set hidden information
   void setHiddenInfo(MpdFemtoHiddenInfo *aHiddenInfo) { mHiddenInfo = aHiddenInfo->clone(); }
