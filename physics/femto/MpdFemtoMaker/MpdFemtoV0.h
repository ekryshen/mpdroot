...
public:
   ...
   float v0AndgleBetweenMomentumAndDecayVector() const { return TMath::Cos(momV0().Angle(v0DecayVector())); }
   /// Angle between momentum and decay vector
   float V0AndgleBetweenMomentumAndDecayVector() const { return v0AndgleBetweenMomentumAndDecayVector(); }
   /// V0 momentum
   ...
   float massK0Short() const { return TMath::Sqrt((ePosPion() + eNegPion()) * (ePosPion() + ePosPion()) + ptot2V0()); }
   /// Mass of the V0 assuming K_s^0 -> pi^+ + pi^- decay
   float MassK0Short() const { return massK0Short(); }
   /// Mass of the V0 assuming phi^0 -> K^+ + K^- decay
   float massPhi() const { return TMath::Sqrt((ePosKaon() + eNegKaon()) * (ePosKaon() + eNegKaon()) + ptot2V0()); }
   /// Mass of the V0 assuming phi^0 -> K^+ + K^- decay
   ...
   /// Set primary vertex position (x,y,z)
   void SetPrimaryVertex(const float &x, const float &y, const float &z) { setPrimaryVertex(x, y, z); }
   ...
   /// Set decay point (x,y,z)
   void SetDecayPoint(const float &x, const float &y, const float &z) { setDecayPoint(x, y, z); }
   ...
   /// Set V0 momentum (x,y,z)
   void SetV0Mom(const float &px, const float &py, const float &pz) { setV0Mom(px, py, pz); }
   ...
   float massSqrPos() const { return (isTofTrackPos()) ? (ptot2Pos() * (inverseBeta2Pos() - 1.)) : -999.f; }
   /// Squared mass of the positive daughter (by TOF)
   ...
   /// Set momentum of the positive daughter (x,y,z)
   void SetMomPos(const float &px, const float &py, const float &pz) { setMomPos(px, py, pz); }
   ...
   /// Set DCA of the positive daughter to primary vertex (x,y,z)
   void SetDcaGlobalPos(const float &x, const float &y, const float &z) { setDcaGlobalPos(x, y, z); }
   ...
   {
      mPosChi2 = ((chi2 * 10) > std::numeric_limits<unsigned char>::max() ? std::numeric_limits<unsigned char>::max()
   ...
   void setDedxPos(const double &dEdx)
   {
      if (dEdx <= 0) {
         ...
         mPosDedx =
            ((dEdx * 1e9) > std::numeric_limits<unsigned short>::max() ? std::numeric_limits<unsigned short>::max()
   ...
   void setDedxPosInKeV(const double &dEdx)
   {
      if (dEdx <= 0) {
         ...
         mPosDedx =
            ((dEdx * 1e3) > std::numeric_limits<unsigned short>::max() ? std::numeric_limits<unsigned short>::max()
   ...
   float massSqrNeg() const { return (isTofTrackNeg()) ? (ptot2Neg() * (inverseBeta2Neg() - 1.)) : -999.f; }
   /// Squared mass of the negative daughter (by TOF)
   ...
   /// Set momentum of the negative daughter (x,y,z)
   void SetMomNeg(const float &px, const float &py, const float &pz) { setMomNeg(px, py, pz); }
   ...
   /// Set DCA of the negative daughter to primary vertex (x,y,z)
   void SetDcaGlobalNeg(const float &x, const float &y, const float &z) { setDcaGlobalNeg(x, y, z); }
   ...
   {
      mNegChi2 = ((chi2 * 10) > std::numeric_limits<unsigned char>::max() ? std::numeric_limits<unsigned char>::max()
                                                                          : (unsigned char)TMath::Nint(chi2 * 10.f));
   }
   /// Set chi2 of reconstruction of the negative daughter
