...
MpdFemtoEvent::MpdFemtoEvent()
   : mEventNumber(0), mRunNumber(0), mMagneticField(0), mRefMult(0), mRefMultPos(0), mRefMultCorr(0),
     mRefMultCorrWeight(0), mRefMult2(0), mRefMult2Pos(0), mGRefMult(0), mGRefMultPos(0), mBTofTrayMultiplicity(0),
     mNBTOFMatch(0), mNBEMCMatch(0), mNumberOfPrimaryTracks(0), mNumberOfGlobalTracks(0), mZdcSumAdcEast(0),
     mZdcSumAdcWest(0), mZdcCoincidenceRate(0), mBbcCoincidenceRate(0), mSphericity(-1), mSphericity2(-1),
     mEventPlaneAngle(0), mEventPlaneResolution(0), mCent16(-1), mPrimaryVertexPositionX(-999),
     mPrimaryVertexPositionY(-999), mPrimaryVertexPositionZ(-999), mVpdVz(0), mRanking(-1e5), mImpactPar(0)
{
...
//___________________
MpdFemtoEvent::MpdFemtoEvent(const MpdFemtoEvent &ev, MpdFemtoBaseTrackCut *tCut, MpdFemtoBaseV0Cut *vCut,
                             MpdFemtoBaseXiCut *xCut, MpdFemtoBaseKinkCut *kCut)
{
   ...
   for (MpdFemtoTrackIterator tIter = ev.mTrackCollection->begin(); tIter != ev.mTrackCollection->end(); tIter++) {
   ...
   for (MpdFemtoV0Iterator vIter = ev.mV0Collection->begin(); vIter != ev.mV0Collection->end(); vIter++) {
   ...
   for (MpdFemtoXiIterator xIter = ev.mXiCollection->begin(); xIter != ev.mXiCollection->end(); xIter++) {
   ...
   for (MpdFemtoKinkIterator kIter = ev.mKinkCollection->begin(); kIter != ev.mKinkCollection->end(); kIter++) {
...
MpdFemtoEvent &MpdFemtoEvent::operator=(const MpdFemtoEvent &ev)
{
   ...
   if (this != &ev) {
      ...
      if (mTrackCollection) {
         for (MpdFemtoTrackIterator iter = mTrackCollection->begin(); iter != mTrackCollection->end(); iter++) {
            delete *iter;
         }
      ...
      if (mV0Collection) {
         for (MpdFemtoV0Iterator iter = mV0Collection->begin(); iter != mV0Collection->end(); iter++) {
            delete *iter;
         }
         mV0Collection->clear();
      } else {
      ...
      if (mKinkCollection) {
         for (MpdFemtoKinkIterator iter = mKinkCollection->begin(); iter != mKinkCollection->end(); iter++) {
            delete *iter;
         }
      ...
      if (mXiCollection) {
         for (MpdFemtoXiIterator iter = mXiCollection->begin(); iter != mXiCollection->end(); iter++) {
            delete *iter;
         }
         mXiCollection->clear();
      } else {
      ...
      // Copy collections
      for (MpdFemtoTrackIterator iIter = ev.mTrackCollection->begin(); iIter != ev.mTrackCollection->end(); iIter++) {
         MpdFemtoTrack *trackCopy = new MpdFemtoTrack(**iIter);
         mTrackCollection->push_back(trackCopy);
      }

      for (MpdFemtoV0Iterator iIter = ev.mV0Collection->begin(); iIter != ev.mV0Collection->end(); iIter++) {
         MpdFemtoV0 *v0Copy = new MpdFemtoV0(**iIter);
         mV0Collection->push_back(v0Copy);
      }

      for (MpdFemtoKinkIterator iIter = ev.mKinkCollection->begin(); iIter != ev.mKinkCollection->end(); iIter++) {
         MpdFemtoKink *kinkCopy = new MpdFemtoKink(**iIter);
         mKinkCollection->push_back(kinkCopy);
      }

      for (MpdFemtoXiIterator iIter = ev.mXiCollection->begin(); iIter != ev.mXiCollection->end(); iIter++) {
...
MpdFemtoEvent::~MpdFemtoEvent()
{
   ...
   for (MpdFemtoV0Iterator V0iter = mV0Collection->begin(); V0iter != mV0Collection->end(); V0iter++) {
      delete *V0iter;
   }
   mV0Collection->clear();
   ...
   for (MpdFemtoXiIterator XiIter = mXiCollection->begin(); XiIter != mXiCollection->end(); XiIter++) {
      delete *XiIter;
   }
   mXiCollection->clear();
   ...
   for (MpdFemtoKinkIterator kinkIter = mKinkCollection->begin(); kinkIter != mKinkCollection->end(); kinkIter++) {
      delete *kinkIter;
   }
   mKinkCollection->clear();
...
void MpdFemtoEvent::rotateZ(const double &angle)
{
   ...
   std::cout << " MpdFemtoEvent::rotateZ(const double angle) - angle=" << angle << " rad    ";
...
void MpdFemtoEvent::addTriggerId(const unsigned int &id)
{
   ...
   if (std::find(mTriggerIds.begin(), mTriggerIds.end(), id) == mTriggerIds.end()) {
