...
MpdFemtoXi::MpdFemtoXi()
   : MpdFemtoV0(), mCharge(false), mDecayVertexXiX(0), mDecayVertexXiY(0), mDecayVertexXiZ(0), mDcaXiDaughters(999),
     mDcaXiToPrimVertex(999), mMomXiX(0), mMomXiY(0), mMomXiZ(0), mChi2Xi(std::numeric_limits<unsigned char>::max()),
     mClXi(0), mBachelor(nullptr), mMomBacAtDca2DecayPointX(0), mMomBacAtDca2DecayPointY(0), mMomBacAtDca2DecayPointZ(0)
{
   // Default constructor
   if (!mBachelor) {
...
MpdFemtoXi::MpdFemtoXi(const MpdFemtoXi &xi)
   : MpdFemtoV0(xi), mCharge(xi.mCharge), mDecayVertexXiX(xi.mDecayVertexXiX), mDecayVertexXiY(xi.mDecayVertexXiY),
     mDecayVertexXiZ(xi.mDecayVertexXiZ), mDcaXiDaughters(xi.mDcaXiDaughters),
     mDcaXiToPrimVertex(xi.mDcaXiToPrimVertex), mMomXiX(xi.mMomXiX), mMomXiY(xi.mMomXiY), mMomXiZ(xi.mMomXiZ),
     mChi2Xi(xi.mChi2Xi), mBachelor(nullptr), mMomBacAtDca2DecayPointX(xi.mMomBacAtDca2DecayPointX),
     mMomBacAtDca2DecayPointY(xi.mMomBacAtDca2DecayPointY), mMomBacAtDca2DecayPointZ(xi.mMomBacAtDca2DecayPointZ)
{
   // Copy bachelor info
   if (!mBachelor) {
...
void MpdFemtoXi::setChi2Xi(const float &chi2)
{
   if (chi2 < 0) {
      ...
      mChi2Xi = ((chi2 * 10) > std::numeric_limits<unsigned char>::max() ? std::numeric_limits<unsigned char>::max()
