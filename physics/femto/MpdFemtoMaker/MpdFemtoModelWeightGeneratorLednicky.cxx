...
MpdFemtoModelWeightGeneratorLednicky::MpdFemtoModelWeightGeneratorLednicky()
   : MpdFemtoBaseModelWeightGenerator(), mWei(0), mWein(0), mWeif(0), mWeightDen(0), mItest(0), mIch(1), mIqs(1),
     mIsi(1), mI3c(0), mNuclMass(1.), mNuclCharge(0.), mSphereApp(false), mT0App(false), mLL(0), mNuclChargeSign(1),
     mSwap(0), mLLMax(30), mLLName(0), mNumProcessPair(0), mNumbNonId(0), mKpKmModel(14), mPhi_OffOn(1)
{
...
//_________________
MpdFemtoModelWeightGeneratorLednicky::MpdFemtoModelWeightGeneratorLednicky(
   const MpdFemtoModelWeightGeneratorLednicky &aWeight)
   : MpdFemtoBaseModelWeightGenerator(), mWei(0), mWein(0), mWeif(0), mWeightDen(0), mItest(0), mIch(1), mIqs(1),
     mIsi(1), mI3c(0), mNuclMass(1.), mNuclCharge(0.), mSphereApp(false), mT0App(false), mLL(0), mNuclChargeSign(1),
     mSwap(0), mLLMax(30), mLLName(0), mNumProcessPair(0), mNumbNonId(0), mKpKmModel(14), mPhi_OffOn(1)
{
...
//_________________
MpdFemtoModelWeightGeneratorLednicky &MpdFemtoModelWeightGeneratorLednicky::operator=(
   const MpdFemtoModelWeightGeneratorLednicky &aWeight)
{
   // Assignment operator
   if (this != &aWeight) {
...
double MpdFemtoModelWeightGeneratorLednicky::generateWeight(MpdFemtoPair *aPair)
{
   ...
   double tE1 = TMath::Sqrt(tM1 * tM1 + ((MpdFemtoModelHiddenInfo *)inf1->hiddenInfo())->trueMomentum().Mag2());
   double tE2 = TMath::Sqrt(tM2 * tM2 + ((MpdFemtoModelHiddenInfo *)inf2->hiddenInfo())->trueMomentum().Mag2());
   ...
   */
   ...
   if (!setPid(((MpdFemtoModelHiddenInfo *)inf1->hiddenInfo())->pdgPid(),
      ...
      mWeightDen = 1.;
      //  std::cout<<" bad PID weight generator pdg1
      //  "<<((MpdFemtoModelHiddenInfo*)inf1->GetHiddenInfo())->GetPDGPid()<<" pdg2
      //  "<<((MpdFemtoModelHiddenInfo*)inf1->GetHiddenInfo())->GetPDGPid() << std::endl;
      return 1; // non-correlated
   } else {     // Good Pid

      // std::cout<<" good PID weight generator pdg1
      // "<<((MpdFemtoModelHiddenInfo*)inf1->GetHiddenInfo())->GetPDGPid()<<" pdg2
      // "<<((MpdFemtoModelHiddenInfo*)inf1->GetHiddenInfo())->GetPDGPid() << std::endl;
...
MpdFemtoString MpdFemtoModelWeightGeneratorLednicky::report()
{
   ...
   int i;
   for (i = 1; i <= mLLMax; i++) {
      if (mNumProcessPair[i]) tStr << "         " << mNumProcessPair[i] << " " << mLLName[i] << std::endl;
   }
   if (mNumbNonId) tStr << "         " << mNumbNonId << " Non Identified" << std::endl;
...
void MpdFemtoModelWeightGeneratorLednicky::fsiSetLL()
{
   ...
   // std::cout<<"*********************** MpdFemtoModelWeightGeneratorLednicky::FsiSetLL()
   // *********************"<<std::endl;
