...
MpdFemtoV0::MpdFemtoV0()
   : mPrimaryVertexX(0), mPrimaryVertexY(0), mPrimaryVertexZ(0), mV0DecayPointX(0), mV0DecayPointY(0),
     mV0DecayPointZ(0), mV0MomX(0), mV0MomY(0), mV0MomZ(0), mV0PtotPos(0), mV0PtotNeg(0), mV0DcaDaughters(0),
     mV0DcaToPrimVertex(0), mChi2V0(0), mClV0(0), mAlphaV0(0), mPtArmV0(0), mPosId(0), mPosMomX(0), mPosMomY(0),
     mPosMomZ(0), mPosDca2PrimVertexX(0), mPosDca2PrimVertexY(0), mPosDca2PrimVertexZ(0), mPosNHits(0),
     mPosNHitsPoss(0), mPosNHitsDedx(0), mPosTopologyMap{}, mPosChi2(0), mPosDedx(0), mPosNSigmaElectron(0),
     mPosNSigmaPion(0), mPosNSigmaKaon(0), mPosNSigmaProton(0), mPosTofBeta(0), mNegId(0), mNegMomX(0), mNegMomY(0),
     mNegMomZ(0), mNegDca2PrimVertexX(0), mNegDca2PrimVertexY(0), mNegDca2PrimVertexZ(0), mNegNHits(0),
     mNegNHitsPoss(0), mNegNHitsDedx(0), mNegTopologyMap{}, mNegChi2(0), mNegDedx(0), mNegNSigmaElectron(0),
     mNegNSigmaPion(0), mNegNSigmaKaon(0), mNegNSigmaProton(0), mNegTofBeta(0), mHiddenInfo(nullptr)
{
   // Default constructor
   /* no-op */
}
...
{
   return MpdFemtoPhysicalHelix(momPos(), originPos(), bField() * kilogauss, static_cast<float>(1));
}

//_________________
void MpdFemtoV0::setNSigmaElectronPos(const float &ns)
{
   mPosNSigmaElectron = (TMath::Abs(ns * 1000.f) > std::numeric_limits<short>::max() ? std::numeric_limits<short>::max()
                                                                                     : (short)(ns * 1000.f));
}
...
{
   mPosNSigmaPion = (TMath::Abs(ns * 1000.f) > std::numeric_limits<short>::max() ? std::numeric_limits<short>::max()
                                                                                 : (short)(ns * 1000.f));
}
...
{
   mPosNSigmaKaon = (TMath::Abs(ns * 1000.f) > std::numeric_limits<short>::max() ? std::numeric_limits<short>::max()
                                                                                 : (short)(ns * 1000.f));
}
...
{
   mPosNSigmaProton = (TMath::Abs(ns * 1000.f) > std::numeric_limits<short>::max() ? std::numeric_limits<short>::max()
                                                                                   : (short)(ns * 1000.f));
}
...
{
   return MpdFemtoPhysicalHelix(momPos(), originNeg(), bField() * kilogauss, static_cast<float>(-1));
}

//_________________
void MpdFemtoV0::setNSigmaElectronNeg(const float &ns)
{
   mNegNSigmaElectron = (TMath::Abs(ns * 1000.f) > std::numeric_limits<short>::max() ? std::numeric_limits<short>::max()
                                                                                     : (short)(ns * 1000.f));
}
...
{
   mNegNSigmaPion = (TMath::Abs(ns * 1000.f) > std::numeric_limits<short>::max() ? std::numeric_limits<short>::max()
                                                                                 : (short)(ns * 1000.f));
}
...
{
   mNegNSigmaKaon = (TMath::Abs(ns * 1000.f) > std::numeric_limits<short>::max() ? std::numeric_limits<short>::max()
                                                                                 : (short)(ns * 1000.f));
}
...
{
   mNegNSigmaProton = (TMath::Abs(ns * 1000.f) > std::numeric_limits<short>::max() ? std::numeric_limits<short>::max()
                                                                                   : (short)(ns * 1000.f));
}
...
void MpdFemtoV0::setTofBetaNeg(const float &beta)
{
   if (beta < 0) {
      ...
      mNegTofBeta =
         ((beta * 20000.f) > std::numeric_limits<unsigned short>::max() ? std::numeric_limits<unsigned short>::max()
                                                                        : (unsigned short)(beta * 20000.f));
   }
}
...
void MpdFemtoV0::setChi2V0(const float &chi2)
{
   if (chi2 < 0) {
      ...
      mChi2V0 = ((chi2 * 10.f) > std::numeric_limits<unsigned char>::max() ? std::numeric_limits<unsigned char>::max()
