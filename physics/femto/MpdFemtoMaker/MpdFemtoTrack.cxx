...
MpdFemtoTrack::MpdFemtoTrack()
   : mId(0), mFlag(0), mNHits(0), mNHitsPoss(0), mNHitsDedx(0), mChi2(0), mDedx(0), mNSigmaElectron(-30),
     mNSigmaPion(-30), mNSigmaKaon(-30), mNSigmaProton(-30), mPidProbElectron(0), mPidProbPion(0), mPidProbKaon(0),
     mPidProbProton(0), mMap(0), mTofBeta(0), mPrimaryPx(0), mPrimaryPy(0), mPrimaryPz(0), mGlobalPx(0), mGlobalPy(0),
     mGlobalPz(0), mDcaX(-999), mDcaY(-999), mDcaZ(-999), mPrimaryVertexX(0), mPrimaryVertexY(0), mPrimaryVertexZ(0),
     mBField(0), mHiddenInfo(nullptr)
{
   // Default constructor
}
...
void MpdFemtoTrack::setChi2(const float &x)
{
   ...
   if (x < 0) {
      ...
      mChi2 = ((x * 1000.) > std::numeric_limits<unsigned short>::max() ? std::numeric_limits<unsigned short>::max()
...
void MpdFemtoTrack::setPidProbElectron(const float &prob)
{
   ...
   if (prob < 0) {
      ...
      mPidProbElectron =
         ((prob * 10000.) > std::numeric_limits<unsigned short>::max() ? std::numeric_limits<unsigned short>::max()
...
void MpdFemtoTrack::setDedx(const double &dEdx)
{
   ...
   if (dEdx < 0) {
      ...
      mDedx = ((dEdx * 1e9) > std::numeric_limits<unsigned short>::max() ? std::numeric_limits<unsigned short>::max()
...
MpdFemtoPhysicalHelix MpdFemtoTrack::helix() const
{
   ...
   return MpdFemtoPhysicalHelix(pMom(), primaryVertex(), mBField * kilogauss, static_cast<float>(charge()));
}
...
MpdFemtoPhysicalHelix MpdFemtoTrack::gHelix() const
{
   ...
   return MpdFemtoPhysicalHelix(gMom(), origin(), mBField * kilogauss, static_cast<float>(charge()));
}
