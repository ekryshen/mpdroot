...
MpdFemtoManager::MpdFemtoManager()
   : mAnalysisCollection(nullptr), mEventReader(nullptr), mEventWriterCollection(nullptr)
{
   // Constructor
...
MpdFemtoManager::MpdFemtoManager(const MpdFemtoManager &copy)
   : mAnalysisCollection(new MpdFemtoAnalysisCollection), mEventReader(copy.mEventReader),
     mEventWriterCollection(new MpdFemtoEventWriterCollection)
{
   // Copy constructor
   MpdFemtoAnalysisIterator AnalysisIter;
   for (AnalysisIter = copy.mAnalysisCollection->begin(); AnalysisIter != copy.mAnalysisCollection->end();
        AnalysisIter++) {
      mAnalysisCollection->push_back(*AnalysisIter);
   }

   MpdFemtoEventWriterIterator EventWriterIter;
   for (EventWriterIter = copy.mEventWriterCollection->begin(); EventWriterIter != copy.mEventWriterCollection->end();
        EventWriterIter++) {
...
MpdFemtoManager &MpdFemtoManager::operator=(const MpdFemtoManager &man)
{
   ...
   if (this != &man) {
      ...
      if (mEventWriterCollection) {
         for (writerIter = mEventWriterCollection->begin(); writerIter != mEventWriterCollection->end(); writerIter++) {
            delete *writerIter;
            *writerIter = nullptr;
         }
      } // if(mEventWriterCollection)
      ...
      // Copy collections
      mAnalysisCollection = new MpdFemtoAnalysisCollection;
      for (analysisIter = man.mAnalysisCollection->begin(); analysisIter != man.mAnalysisCollection->end();
           analysisIter++) {
         mAnalysisCollection->push_back(*analysisIter);
      }

      mEventWriterCollection = new MpdFemtoEventWriterCollection;
      for (writerIter = man.mEventWriterCollection->begin(); writerIter != man.mEventWriterCollection->end();
           writerIter++) {
