...
MpdFemtoHelix::MpdFemtoHelix()
   : mSingularity(false), mOrigin(0, 0, 0), mDipAngle(0), mCurvature(0), mPhase(0), mH(0), mCosDipAngle(0),
     mSinDipAngle(0), mCosPhase(0), mSinPhase(0)
{
   /*no-op*/
}

//_________________
MpdFemtoHelix::MpdFemtoHelix(double c, double d, double phase, const TVector3 &o, int h)
{
...
//_________________
void MpdFemtoHelix::setParameters(double c, double dip, double phase, const TVector3 &o, int h)
{
   //
...
//________________
double MpdFemtoHelix::pathLength(const TVector3 &r, const TVector3 &n) const
{
   //
...
//_________________
pair<double, double> MpdFemtoHelix::pathLength(const MpdFemtoHelix &h, double minStepSize, double minRange) const
{
   //
   ...
   if (mSingularity) {
      ...
      TVector3 b(-h.mCosDipAngle * h.mSinPhase, h.mCosDipAngle * h.mCosPhase, h.mSinDipAngle);
      double   ab = a * b;
      double   g  = dv * a;
      double   k  = dv * b;
      s2          = (k - ab * g) / (ab * ab - 1.);
