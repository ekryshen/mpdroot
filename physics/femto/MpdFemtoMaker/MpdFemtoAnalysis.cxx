...
void doFillParticleCollection(TrackCutType *cut, TrackCollectionType *track_collection,
                              MpdFemtoParticleCollection *output, char doReshuffle)
{
   ...
   if (initPartCollection && doReshuffle == 2) {
      ...
      //      std::cout<<"[ERROR]: Size of particles collection and size of iterators for this collection are
      //      different"<<
...
void fillHbtParticleCollection(MpdFemtoBaseParticleCut *partCut, MpdFemtoEvent *hbtEvent,
                               MpdFemtoParticleCollection *partCollection, char doReshuffle)
{
   ...
   case hbtTrack:
      ...
      doFillParticleCollection((MpdFemtoBaseTrackCut *)partCut, hbtEvent->trackCollection(), partCollection,
                               doReshuffle);
      break;

   case hbtV0:
      // Cut is cutting on V0s
      doFillParticleCollection((MpdFemtoBaseV0Cut *)partCut, hbtEvent->v0Collection(), partCollection, doReshuffle);
      break;

   case hbtXi:
      // Cut is cutting on Xis
      doFillParticleCollection((MpdFemtoBaseXiCut *)partCut, hbtEvent->xiCollection(), partCollection, doReshuffle);
      break;

   case hbtKink:
      // Cut is cutting on Kinks
      doFillParticleCollection((MpdFemtoBaseKinkCut *)partCut, hbtEvent->kinkCollection(), partCollection, doReshuffle);
      break;
   default:
...
MpdFemtoAnalysis::MpdFemtoAnalysis()
   : mPicoEventCollectionVectorHideAway(nullptr), mPairCut(nullptr), mCorrFctnCollection(nullptr), mEventCut(nullptr),
     mFirstParticleCut(nullptr), mSecondParticleCut(nullptr), mMixingBuffer(nullptr), mPicoEvent(nullptr),
     mNumEventsToMix(0), mNeventsProcessed(0), mMinSizePartCollection(0), mReshuffle(1), mVerbose(false)
{
   // Default constructor
...
MpdFemtoAnalysis::MpdFemtoAnalysis(const MpdFemtoAnalysis &a)
   : MpdFemtoBaseAnalysis(), mPicoEventCollectionVectorHideAway(nullptr), mPairCut(nullptr),
     mCorrFctnCollection(nullptr), mEventCut(nullptr), mFirstParticleCut(nullptr), mSecondParticleCut(nullptr),
     mMixingBuffer(nullptr), mPicoEvent(nullptr), mNumEventsToMix(a.mNumEventsToMix), mNeventsProcessed(0),
     mMinSizePartCollection(a.mMinSizePartCollection), mReshuffle(a.mReshuffle), mVerbose(a.mVerbose)
{
   // Copy constructor
   ...
   mSecondParticleCut = ((a.mFirstParticleCut == a.mSecondParticleCut) ? mFirstParticleCut : a.mSecondParticleCut);
...
MpdFemtoAnalysis &MpdFemtoAnalysis::operator=(const MpdFemtoAnalysis &ana)
{
   ...
   if (this != &ana) {
      ...
      if (mCorrFctnCollection) {
         for (MpdFemtoCorrFctnIterator iIter = mCorrFctnCollection->begin(); iIter != mCorrFctnCollection->end();
              iIter++) {
            delete *iIter;
         }
      ...
      // Clear mixing buffer
      if (mMixingBuffer) {
         for (MpdFemtoPicoEventIterator iIter = mMixingBuffer->begin(); iIter != mMixingBuffer->end(); iIter++) {
            delete *iIter;
         }
         mMixingBuffer->clear();
      } // if (mMixingBuffer)
      ...
      mSecondParticleCut =
         ((ana.mFirstParticleCut == ana.mSecondParticleCut) ? mFirstParticleCut : ana.mSecondParticleCut->clone());
...
MpdFemtoString MpdFemtoAnalysis::reshReport()
{
   ...
   default: temp += "[ERROR] MpdFemtoAnalsysis::mReshuffle has invalid value\n";
   }
...
MpdFemtoString MpdFemtoAnalysis::report()
{
   ...
   std::cout << "MpdFemtoAnalysis - constructing Report..." << std::endl;
   string temp = "-----------\nHbt Analysis Report:\n";
   temp += reshReport();
   temp += "\nEvent Cuts:\n";
   ...
   if (mCorrFctnCollection->size() == 0) {
      std::cout << "MpdFemtoAnalysis-Warning : no correlations functions in this analysis " << std::endl;
   }
   for (iter = mCorrFctnCollection->begin(); iter != mCorrFctnCollection->end(); iter++) {
      temp += (*iter)->report();
      temp += "\n";
...
void MpdFemtoAnalysis::processEvent(const MpdFemtoEvent *hbtEvent)
{
   ...
   if (collection1 == nullptr || collection2 == nullptr) {
      std::cout << "MpdFemtoAnalysis::processEvent - new PicoEvent is missing particle collections!" << std::endl;
      eventEnd(hbtEvent);
      delete mPicoEvent;
      return;
   }
   ...
   fillHbtParticleCollection(mFirstParticleCut, (MpdFemtoEvent *)hbtEvent, mPicoEvent->firstParticleCollection(),
                             mReshuffle);
   // In case of non-identical particles
   if (!(analyzeIdenticalParticles())) {
      fillHbtParticleCollection(mSecondParticleCut, (MpdFemtoEvent *)hbtEvent, mPicoEvent->secondParticleCollection(),
                                mReshuffle);
   }
...
//_________________________
void MpdFemtoAnalysis::makePairs(const char *typeIn, MpdFemtoParticleCollection *partCollection1,
...
void MpdFemtoAnalysis::eventBegin(const MpdFemtoEvent *ev)
{
   ...
   for (MpdFemtoCorrFctnIterator iter = mCorrFctnCollection->begin(); iter != mCorrFctnCollection->end(); iter++) {
      (*iter)->eventBegin(ev);
   }
}
