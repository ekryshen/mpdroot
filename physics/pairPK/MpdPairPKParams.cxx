...
ClassImp(MpdPairPKParams);

void MpdPairPKParams::ReadFromFile(std::string fname)
{
...
void MpdPairPKParams::Print() const
{
   cout << "#-------Parameters used for PairPK analysis------" << endl;
...
void MpdPairPKParams::read(std::string name, bool &b)
{
...
void MpdPairPKParams::read(std::string name, int &b)
{
   auto search = mMap.find(name);
   if (search != mMap.end()) {
...
void MpdPairPKParams::read(std::string name, float &b)
{
   auto search = mMap.find(name);
   if (search != mMap.end()) {
...
void MpdPairPKParams::read(std::string name, std::string &b)
{
   auto search = mMap.find(name);
   if (search != mMap.end()) {
      b = search->second;
