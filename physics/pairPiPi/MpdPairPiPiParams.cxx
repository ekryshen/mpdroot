...
ClassImp(MpdPairPiPiParams);

void MpdPairPiPiParams::ReadFromFile(std::string fname)
{
...
void MpdPairPiPiParams::Print() const
{
   cout << "#-------Parameters used for PairPiPi analysis------" << endl;
...
void MpdPairPiPiParams::read(std::string name, bool &b)
{
...
void MpdPairPiPiParams::read(std::string name, int &b)
{
   auto search = mMap.find(name);
   if (search != mMap.end()) {
...
void MpdPairPiPiParams::read(std::string name, float &b)
{
   auto search = mMap.find(name);
   if (search != mMap.end()) {
...
void MpdPairPiPiParams::read(std::string name, std::string &b)
{
   auto search = mMap.find(name);
   if (search != mMap.end()) {
      b = search->second;
