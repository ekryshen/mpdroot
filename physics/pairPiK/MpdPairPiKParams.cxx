...
ClassImp(MpdPairPiKParams);

void MpdPairPiKParams::ReadFromFile(std::string fname)
{
...
void MpdPairPiKParams::Print() const
{
   cout << "#-------Parameters used for PairPiK analysis------" << endl;
...
void MpdPairPiKParams::read(std::string name, bool &b)
{
...
void MpdPairPiKParams::read(std::string name, int &b)
{
   auto search = mMap.find(name);
   if (search != mMap.end()) {
...
void MpdPairPiKParams::read(std::string name, float &b)
{
   auto search = mMap.find(name);
   if (search != mMap.end()) {
...
void MpdPairPiKParams::read(std::string name, std::string &b)
{
   auto search = mMap.find(name);
   if (search != mMap.end()) {
      b = search->second;
