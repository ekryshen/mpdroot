...
void NicaMpdMcEventInterface::Clear(Option_t* opt) { fMcracks->GetArray()->Clear(opt); }
...
void NicaMpdMcEventInterface::ConnectToTree() {
  ...
  if (fEvent == nullptr) fEvent = (FairMCEventHeader*) manager->GetObject("EventHeader.");
  fMcracks->GetFromTree();
}

void NicaMpdMcEventInterface::Compress(Int_t* map, Int_t map_size) { fMcracks->Compress(map, map_size); }
...
void NicaMpdMcEventInterface::CopyAndCompress(NicaEventInterface* s, Int_t* map, Int_t map_size) {
  FairMCEventHeader* header = (FairMCEventHeader*) ((NicaMpdMcEventInterface*) s)->fEvent;
  ...
  fEvent->SetB(header->GetB());
  fEvent->SetNPrim(header->GetNPrim());
  fMcracks->CopyCompress<MpdMCTrack>(((NicaMpdMcEventInterface*) s)->fMcracks->GetArray(), map, map_size);
}
...
NicaTrackInterface* NicaMpdMcEventInterface::GetTrackInterface() const { return new NicaMpdMcTrackInterface(); }

void NicaMpdMcEventInterface::FillTrackInterface(NicaTrackInterface* track, Int_t index) {
