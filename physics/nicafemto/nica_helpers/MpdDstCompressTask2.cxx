...
MpdDstCompressTask2::MpdDstCompressTask2()
   : fUseMC(kFALSE), fUseTpcKalmans(kFALSE), fUseTpcHits(kFALSE), fUseHeader(kFALSE), fMCCompression(kFALSE),
     fFixMom(kFALSE), fMpdEvent(nullptr), fMCTracks(nullptr), fTpcKalmans(nullptr), fTpcHits(nullptr),
     fEventHeader(nullptr), fMCMapSize(0), fMCIndexMap(nullptr), fPID(nullptr)
{
...
InitStatus MpdDstCompressTask2::CheckBranches()
{
   ...
   if (fTpcKalmans == nullptr) fFixMom = kFALSE;
   if (fUseTpcHits) {
...
void MpdDstCompressTask2::FixPID(MpdTrack *track)
{
   ...
   if (TMath::IsNaN(sigma_el)) sigma_el = -1E+2;
   if (TMath::IsNaN(sigma_pi)) sigma_pi = -1E+2;
   if (TMath::IsNaN(sigma_ka)) sigma_ka = -1E+2;
   if (TMath::IsNaN(sigma_pr)) sigma_pr = -1E+2;
...
void MpdDstCompressTask2::FixMom(MpdTrack *track, Int_t index, TVector3 &vertex)
{
   ...
   TVector3   poz(kftrack->GetPosNew() * TMath::Cos(phi), kftrack->GetPosNew() * TMath::Sin(phi), kftrack->GetParam(1));
   TVector3   mom(track->GetPx(), track->GetPy(), track->GetPz());
