...
void MpdHitsOnTheFly::GetHitMaps(Int_t particle_index, ULong64_t& layerHit, ULong64_t& sharedHit) {
  ...
  if (khits)
    for (int j = 0; j < khits->GetEntriesFast(); j++) {
      ...
      if (layer >= 0) {
        if (fHitMap[id] > 1) SETBIT(sharedHit, layer);
...
MpdHitsOnTheFly::~MpdHitsOnTheFly() {
  if (fHitMapSize > 0) delete[] fHitMap;
}
