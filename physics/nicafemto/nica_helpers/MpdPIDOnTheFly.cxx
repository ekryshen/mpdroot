...
InitStatus MpdPIDOnTheFly::Init() {
  ...
  if (fEvent == nullptr) { fMiniTrack = (TClonesArray*) manager->GetObject("Track"); }
...
void MpdPIDOnTheFly::Exec(Option_t* opt) {
  if (fEvent) {
    ...
    fEventVector->SetXYZ(fEvent->GetPrimaryVerticesX(), fEvent->GetPrimaryVerticesY(), fEvent->GetPrimaryVerticesZ());
...
void MpdPIDOnTheFly::FillTrackPID(MpdTrack* track) {
  ...
  if (track->GetTofMass2() == 0) { track->SetTofMass2(-5); }
}

void MpdPIDOnTheFly::FillTrackDCA(MpdTrack* track, TVector3* recoVertex, TVector3* mcVertex) {
