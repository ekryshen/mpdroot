...
NicaMpdEventInterface::NicaMpdEventInterface() : fEvent(NULL), fMode(kAllTracks) { fEvent = new MpdEvent(); }
...
void NicaMpdEventInterface::CopyAndCompress(NicaEventInterface* s, Int_t* map, Int_t map_size) {
...
void NicaMpdEventInterface::FillTrackInterface(NicaTrackInterface* track, Int_t index) {
  ...
  switch (fMode) {
    ...
    case kPrimaryTracks: tr->SetRawTrack(fEvent->GetPrimaryTracks()->UncheckedAt(index)); break;
    case kGlobalTracks: tr->SetRawTrack(fEvent->GetGlobalTracks()->UncheckedAt(index)); break;
  }
}
...
void NicaMpdEventInterface::SetRunInfoId(Int_t i) { fEvent->SetRunInfoRunId(i); }

void NicaMpdEventInterface::CopyData(NicaEventInterface* s) { *fEvent = *((NicaMpdEventInterface*) s)->fEvent; }

void NicaMpdEventInterface::SetMagneticField(TVector3 mag) const { fEvent->SetRunInfoMagneticFieldZ(mag.Z()); }
...
Int_t NicaMpdEventInterface::GetTotalTrackNo() const {
  switch (fMode) {
    ...
    case kGlobalTracks: return fEvent->GetGlobalTracks()->GetEntriesFast(); break;
    case kPrimaryTracks: return fEvent->GetPrimaryTracks()->GetEntriesFast(); break;
  }
  return 0;
}

Int_t NicaMpdEventInterface::GetRunInfoId() const { return fEvent->GetRunInfoRunId(); }
...
NicaTrackInterface* NicaMpdEventInterface::GetTrackInterface() const { return new NicaMpdTrackInterface(); }
...
TObject* NicaMpdEventInterface::GetRawTrackPointer(Int_t index) const {
  switch (fMode) {
    ...
    case kPrimaryTracks: return fEvent->GetPrimaryTracks()->UncheckedAt(index); break;
    case kGlobalTracks: return fEvent->GetGlobalTracks()->UncheckedAt(index); break;
