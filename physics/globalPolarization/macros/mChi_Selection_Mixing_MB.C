...
void FitFile(int NITER, TString fullpath, double xmin, double nsig, double &max_value_file, int &max_value_iter_V0_file, int &max_value_iter_path_file, int &max_value_iter_angle_file);
...
/**
 * @brief Analysis macro for RECO GlobalPol wagon to obtain optimal chi selection values using event mixing for background subtraction
 * 
 ...
 */
void mChi_Selection_Mixing_MB(TString inFileDir = "Directory_Path", TString configname = "pGlobalPolRECO.txt", TString inname = "Selections_Full.root", TString outFile_values = "ChiSelection_values_MB_3sigma.txt", TString outFile_iterations = "ChiSelection_iterations_MB_3sigma.txt", int NITER = 10, double xmin = 1.09, double nsig = 3.0)
{	
	...
	for(int iter_pi = 0; iter_pi < NITER; iter_pi++)
	{
		for(int iter_p = 0; iter_p < NITER; iter_p++)
		{
			...
			FitFile(NITER,fullpath,xmin,nsig,max_value_file,max_value_iter_V0_file,max_value_iter_path_file,max_value_iter_angle_file);	
...
{	
	...
	TF1 *backFcn[NITER][NITER][NITER], *signalFcn[NITER][NITER][NITER];
	...
	for(int iter_V0 = 0; iter_V0 < NITER; iter_V0++)
	{
		for(int iter_path = 0; iter_path < NITER; iter_path++)
		{
			for(int iter_angle = 0; iter_angle < NITER; iter_angle++)
			{
				...
				hm0_bckg[iter_V0][iter_path][iter_angle] = (TH1D*) myFile_data->Get(Form("hm0_mixed_V%d_path%d_angle%d",iter_V0,iter_path,iter_angle));
				hm0_bckg[iter_V0][iter_path][iter_angle]->Scale(1./5.);
	...
	for(int iter_V0 = 0; iter_V0 < NITER; iter_V0++)
	{
		for(int iter_path = 0; iter_path < NITER; iter_path++)
		{
			for(int iter_angle = 0; iter_angle < NITER; iter_angle++)
			{
				int bin_left_scale = hm0_bckg[iter_V0][iter_path][iter_angle]->FindBin(1.14);
				int bin_right_scale = hm0_bckg[iter_V0][iter_path][iter_angle]->FindBin(1.16);
				double sum_hm0_scale = 0.;
				double sum_hm0_mixed_scale = 0.;
				for (int ib = bin_left_scale; ib <= bin_right_scale; ++ib) 
				{
				sum_hm0_scale += hm0[iter_V0][iter_path][iter_angle]->GetBinContent(ib); 
				sum_hm0_mixed_scale += hm0_bckg[iter_V0][iter_path][iter_angle]->GetBinContent(ib);
				}
				hm0_bckg[iter_V0][iter_path][iter_angle]->Scale(sum_hm0_scale/sum_hm0_mixed_scale);	
				
				backFcn[iter_V0][iter_path][iter_angle] = new TF1(Form("int_backFcn_V%d_path%d_angle%d",iter_V0,iter_path,iter_angle),background,xmin,xmax[iter_V0][iter_path][iter_angle],5);
				hm0_bckg[iter_V0][iter_path][iter_angle]->Fit(Form("int_backFcn_V%d_path%d_angle%d",iter_V0,iter_path,iter_angle),"q0","same",xmin,xmax[iter_V0][iter_path][iter_angle]);
				...
				double mass = signalFcn[iter_V0][iter_path][iter_angle]->GetParameter(1);
				double sigma = signalFcn[iter_V0][iter_path][iter_angle]->GetParameter(2);
