/**
 * @file mOmega2_Selection_SidebandsFit_MB.C
...
void FitFile(int NITER, TString inFile, double xmin, double nsig_bckg, double nsig, double &max_value_sig, int &max_value_iter);
...
/**
 * @brief Analysis macro for RECO GlobalPol wagon to obtain optimal omega_2 selection values using sidebands method for background subtraction
 * 
 ...
 */
void mOmega2_Selection_SidebandsFit_MB(TString inFile = "Output_GlopalPolRECO_omega2_selection.root", TString configname = "pGlobalPolRECO.txt", TString outFile = "Omega2Selection_values_MB_3sigma.txt", int NITER = 30, double xmin = 1.086, double nsig_bckg = 7.0, double nsig = 4.0)
{
	...
	int npar = fitting_fnc_final->GetNumberFreeParameters();
	for (int ip = 0; ip < npar; ++ip) fitting_fnc_final->SetParameter(ip,0);	

	fitting_fnc_final->SetParameter(0,hm0_final->GetMaximum());
	fitting_fnc_final->SetParameter(1,massL0);
	...
	double mass = fitting_fnc_final->GetParameter(npar-7);
	double sigma = TMath::Abs(fitting_fnc_final->GetParameter(npar-6));

	double xmin_cut = mass - nsig_bckg * sigma;
	double xmax_cut = mass + nsig_bckg * sigma;

	int imin = hm0_final->FindBin(xmin_cut);
	int imax = hm0_final->FindBin(xmax_cut);
	...
	cout << "mass - nsig * sigma = " << mass - nsig * sigma << "; mass + nsig * sigma = " << mass + nsig * sigma << endl;
...
void FitFile(int NITER, TString inFile, double xmin, double nsig_bckg, double nsig, double &max_value_sig, int &max_value_iter)
{
	...
	for(int iter = 0; iter < NITER; iter++)
	{
		...
		fitting_fnc_full[iter]->SetLineColor(2); 
		fitting_fnc_full[iter]->SetParNames("Strength","Mean","Sigma","pol1","pol2","pol3","pol4");
		int npar = fitting_fnc_full[iter]->GetNumberFreeParameters();
		for (int ip = 0; ip < npar; ++ip) fitting_fnc_full[iter]->SetParameter(ip,0);
		fitting_fnc_full[iter]->SetParameter(0,hm0_full[iter]->GetMaximum());
		fitting_fnc_full[iter]->SetParameter(1,massL0);
		...
		double mass = fitting_fnc_full[iter]->GetParameter(npar-7);
		double sigma = TMath::Abs(fitting_fnc_full[iter]->GetParameter(npar-6));
		double xmin_cut = mass - nsig_bckg * sigma;
		double xmax_cut = mass + nsig_bckg * sigma;
		int imin = hm0_full[iter]->FindBin(xmin_cut);
		int imax = hm0_full[iter]->FindBin(xmax_cut);
		...
		double errs[200] = {0};
		int nbins = hm0_full[iter]->GetNbinsX();
		for (int ib = 1; ib <= nbins; ++ib) 
		{
		...
		hm0_signal_full[iter]->Fit(int_signalFcn_full,"","same",xmin,xmax_full[iter]);
		...
		TLine *line_left = new TLine(left,0,left,hm0_signal_full[iter]->GetMaximum());
		line_left->SetLineColor(kBlack);
		line_left->SetLineWidth(2);
		line_left->SetLineStyle(2);
		line_left->Draw("same");
		TLine *line_right = new TLine(right,0,right,hm0_signal_full[iter]->GetMaximum());
		line_right->SetLineColor(kBlack);
