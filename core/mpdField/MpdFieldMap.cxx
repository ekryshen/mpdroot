...
MpdFieldMap::MpdFieldMap(const char *mapName, const char *fileType) : FairField(mapName)
{
   fPosX = fPosY = fPosZ = 0.;
   fXmin = fYmin = fZmin = 0.;
   fXmax = fYmax = fZmax = 0.;
...
MpdFieldMap::MpdFieldMap(MpdFieldPar *fieldPar)
{
   ...
   fBx = fBy = fBz = NULL;
   if (!fieldPar) {
      cerr << "-W- MpdConstField::MpdConstField: empty parameter container!" << endl;
      fName     = "";
      fFileName = "";
      fType     = -1;
   } else {
...
void MpdFieldMap::Init()
{
   if (fFileName.EndsWith(".root"))
      ...
      cerr << "-E- MpdFieldMap::Init: No proper file name defined! (" << fFileName << ")" << endl;
...
Bool_t MpdFieldMap::IsInside(Double_t x, Double_t y, Double_t z)
{
   ...
   /// Checking a point to be inside the TPC (B-field Map) (r1 - delta_r = 40.3 cm - 2 cm, r2 + delta_r = 120.3 cm + 2
   /// cm)
   else if (TMath::Power(x, 2) + TMath::Power(y, 2) >= TMath::Power(r1 - delta_r, 2) &&
            TMath::Power(x, 2) + TMath::Power(y, 2) <= TMath::Power(r2 + delta_r, 2))
      return kTRUE;
...
void MpdFieldMap::WriteAsciiFile(const char *fileName)
{
   ...
   // Open file
   cout << "-I- MpdFieldMap: Writing field map to ASCII file " << fileName << endl;
   ...
   cout << "-I- MpdFieldMap: " << fNx * fNy * fNz << " entries to write... " << setw(3) << 0 << " % ";
   Int_t index = 0;
   div_t modul;
...
void MpdFieldMap::WriteRootFile(const char *fileName, const char *mapName)
{
...
void MpdFieldMap::Print()
{
   ...
   cout << "----" << endl;
   cout << "----  Field at origin is ( " << setw(6) << bx << ", " << setw(6) << by << ", " << setw(6) << bz << ") kG"
        << endl;
   cout << "======================================================" << endl;
}
...
void MpdFieldMap::ReadAsciiFile(const char *fileName)
{
   ...
   // Open file
   cout << "-I- MpdFieldMap: Reading field map from ASCII file " << fileName << endl;
   ...
   if (type == "nosym")
      ...
      iType = 4;
   if (fType != iType) {
      cout << "-E- MpdFieldMap::ReadAsciiFile: Incompatible map types!" << endl;
   ...
   cout << right;
   Int_t nTot = fNx * fNy * fNz;
   cout << "-I- MpdFieldMap: " << nTot << " entries to read... " << setw(3) << 0 << " % ";
   Int_t index = 0;
   div_t modul;
   Int_t iDiv = TMath::Nint(nTot / 100.) + 1;
   for (Int_t ix = 0; ix < fNx; ix++) {
      for (Int_t iy = 0; iy < fNy; iy++) {
         for (Int_t iz = 0; iz < fNz; iz++) {
            if (!mapFile.good())
               cerr << "-E- MpdFieldMap::ReadAsciiFile: "
            ...
            if (mapFile.eof()) {
               cerr << endl
                    << "-E- MpdFieldMap::ReadAsciiFile: EOF"
...
void MpdFieldMap::ReadRootFile(const char *fileName, const char *mapName)
{
...
void MpdFieldMap::SetField(const MpdFieldMapData *data)
{
   ...
   if (data->GetType() != fType) {
      cout << "-E- MpdFieldMap::SetField: Incompatible map types!" << endl;
...
Double_t MpdFieldMap::MPDCalcBr(Double_t x, Double_t y)
{
   ...
   Double_t br_coeff[25] = {
      3.55683796461738e-005, -1.45190262790289e-005, -4.22589216944936e-005, -3.7825589770834e-006,
...
Double_t MpdFieldMap::MPDCalcBz(Double_t x, Double_t y)
{
   ...
   Double_t bz_coeff[25] = {
      0.500047838143456,     0.00044540880088384,    -5.35189259780466e-005, -0.000247723183378179,
