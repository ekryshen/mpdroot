...
FairParSet *MpdFieldContFact::createContainer(FairContainer *container)
{
   ...
   if (strcmp(name, "MpdFieldPar") == 0) {
      set = new MpdFieldPar(container->getConcatName().Data(), container->GetTitle(), container->getContext());
   }

   else if (strcmp(name, "MpdConstPar") == 0) {
      set = new MpdConstPar(container->getConcatName().Data(), container->GetTitle(), container->getContext());
   }

   else if (strcmp(name, "MpdMultiFieldPar") == 0) {
      set = new MpdMultiFieldPar(container->getConcatName().Data(), container->GetTitle(), container->getContext());
   }
   return set;
}
...
{
   FairContainer *con1 = new FairContainer("MpdFieldPar", "Field parameter container", "Default field");
   containers->Add(con1);

   FairContainer *con2 = new FairContainer("MpdConstPar", "Const Field parameter container", "Default field");
   containers->Add(con2);

   FairContainer *con3 = new FairContainer("MpdMultiFieldPar", "Multiple Field parameter container", "Default field");
   containers->Add(con3);
}
