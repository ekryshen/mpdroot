...
//_________________
MpdMiniHelix::MpdMiniHelix()
   : fSingularity(false), fOrigin(0, 0, 0), fDipAngle(0), fCurvature(0), fPhase(0), fH(0), fCosDipAngle(0),
     fSinDipAngle(0), fCosPhase(0), fSinPhase(0)
{
   /*no-op*/
}

//_________________
MpdMiniHelix::MpdMiniHelix(Double_t c, Double_t d, Double_t phase, const TVector3 &o, Int_t h)
{
...
//_________________
void MpdMiniHelix::setParameters(Double_t c, Double_t dip, Double_t phase, const TVector3 &o, Int_t h)
{
...
Double_t MpdMiniHelix::fudgePathLength(const TVector3 &p) const
{
   ...
   if (fSingularity) {
      ...
      s = atan2(dy * fCosPhase - dx * fSinPhase, 1 / fCurvature + dx * fCosPhase + dy * fSinPhase) /
   ...
   if (fSingularity) {
...
#ifndef ST_NO_NUMERIC_LIMITS
      {
         ...
         for (Int_t i = 0; i < MaxIterations; i++) {
            ...
            s -=
               (t11 * t12 * fH * fCosDipAngle - t19 * t7 * fH * fCosDipAngle - (dz - s * fSinDipAngle) * fSinDipAngle) /
               (t12 * t12 * fCosDipAngle * fCosDipAngle + t11 * t7 * t34 + t7 * t7 * fCosDipAngle * fCosDipAngle +
                t19 * t12 * t34 + t41);
...
pair<Double_t, Double_t> MpdMiniHelix::pathLength(Double_t r) const
{
   ...
   if (fSingularity) {
      ...
      Double_t t20 = -fCosDipAngle * fCosDipAngle *
                     (2.0 * fOrigin.x() * fSinPhase * fOrigin.y() * fCosPhase + t12 - t12 * t13 - t15 + t13 * t16);
      if (t20 < 0.) {
         return VALUE;
      }
      t20          = ::sqrt(t20);
      ...
      Double_t t35 = t14 * t3;
      Double_t t38 = 8.0 * t4 * t6 - 4.0 * t1 * t2 * t8 - 4.0 * t11 * fCurvature * t6 + 4.0 * t15 * t6 + t17 * t3 +
                     t19 * t3 + 2.0 * t21 * t8 + 4.0 * t8 * t23 - 4.0 * t8 * fOrigin.x() * fCurvature * t5 -
                     4.0 * t11 * t23 - 4.0 * t11 * fOrigin.y() * fCurvature * t2 + 4.0 * t11 - 4.0 * t14 + t32 * t3 +
...
//________________
Double_t MpdMiniHelix::pathLength(const TVector3 &r, const TVector3 &n) const
{
   //
   ...
   if (fSingularity) {
      Double_t t = n.z() * fSinDipAngle + n.y() * fCosDipAngle * fCosPhase - n.x() * fCosDipAngle * fSinPhase;
      if (t == 0) {
         s = NoSolution;
      } else {
         s = ((r - fOrigin) * n) / t;
      ...
      Double_t A = fCurvature * ((fOrigin - r) * n) - n.x() * fCosPhase - n.y() * fSinPhase;
...
//_________________
pair<Double_t, Double_t> MpdMiniHelix::pathLengths(const MpdMiniHelix &h, Double_t minStepSize, Double_t minRange) const
{
   //
   ...
   if (fSingularity) {
      ...
      TVector3 a(-fCosDipAngle * fSinPhase, fCosDipAngle * fCosPhase, fSinDipAngle);
      TVector3 b(-h.fCosDipAngle * h.fSinPhase, h.fCosDipAngle * h.fCosPhase, h.fSinDipAngle);
      Double_t ab = a * b;
      Double_t g  = dv * a;
      Double_t k  = dv * b;
      s2          = (k - ab * g) / (ab * ab - 1.);
...
void MpdMiniHelix::moveOrigin(Double_t s)
{
   ...
   if (fSingularity) {
      ...
      Double_t newPhase  = atan2(newOrigin.y() - ycenter(), newOrigin.x() - xcenter());
      fOrigin            = newOrigin;
      setPhase(newPhase);
   }
}
...
Int_t operator==(const MpdMiniHelix &a, const MpdMiniHelix &b)
{
   ...
   //
   return (a.origin() == b.origin() && a.dipAngle() == b.dipAngle() && a.curvature() == b.curvature() &&
           a.phase() == b.phase() && a.h() == b.h());
}
...
{
   return os << '(' << "curvature = " << h.curvature() << ", "
