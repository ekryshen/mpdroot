...
MpdHelix::MpdHelix()
   : mSingularity(false), mOrigin(), mDipAngle(0), mCurvature(0), mPhase(0), mH(0), mCosDipAngle(0), mSinDipAngle(0),
     mCosPhase(0), mSinPhase(0)
{ /*noop*/
}

MpdHelix::MpdHelix(double c, double d, double Phase, const TVector3 o, int hh)
{
   setParameters(c, d, Phase, o, hh);
}

MpdHelix::MpdHelix(TVector3 mom, TVector3 o, Double_t charge, Double_t Bz)
{
...
void MpdHelix::setParameters(double c, double dip, double Phase, const TVector3 o, int hh)
{
   //
...
void MpdHelix::setPhase(double val)
{
   ...
   if (fabs(mPhase) > M_PI) mPhase = atan2(mSinPhase, mCosPhase); // force range [-pi,pi]
}
   ...
   if (mSingularity) {
            ...
            // #ifndef NO_NAMESPACES
      {
         ...
         for (int i = 0; i < MaxIterations; i++) {
            ...
            s -=
               (t11 * t12 * mH * mCosDipAngle - t19 * t7 * mH * mCosDipAngle - (dz - s * mSinDipAngle) * mSinDipAngle) /
               (t12 * t12 * mCosDipAngle * mCosDipAngle + t11 * t7 * t34 + t7 * t7 * mCosDipAngle * mCosDipAngle +
                t19 * t12 * t34 + t41);
...
pair<double, double> MpdHelix::pathLength(double r) const
{
   ...
   if (mSingularity) {
      ...
      double t20 = -mCosDipAngle * mCosDipAngle *
      ...
      double t32 = t14 * t14;
      double t35 = t14 * t3;
      double t38 = 8.0 * t4 * t6 - 4.0 * t1 * t2 * t8 - 4.0 * t11 * mCurvature * t6 + 4.0 * t15 * t6 + t17 * t3 +
                   t19 * t3 + 2.0 * t21 * t8 + 4.0 * t8 * t23 - 4.0 * t8 * mOrigin.X() * mCurvature * t5 -
                   4.0 * t11 * t23 - 4.0 * t11 * mOrigin.Y() * mCurvature * t2 + 4.0 * t11 - 4.0 * t14 + t32 * t3 +
...
double MpdHelix::pathLength(const TVector3 r, const TVector3 n) const
{
   //
...
pair<double, double> MpdHelix::pathLengths(const MpdHelix &hh) const
{
   ...
   //
   if (mSingularity != hh.mSingularity) return pair<double, double>(NoSolution, NoSolution);
   ...
   if (mSingularity) {
      ...
      TVector3 a(-mCosDipAngle * mSinPhase, mCosDipAngle * mCosPhase, mSinDipAngle);
      TVector3 b(-hh.mCosDipAngle * hh.mSinPhase, hh.mCosDipAngle * hh.mCosPhase, hh.mSinDipAngle);
      double   ab = a * b;
      double   g  = dv * a;
      double   k  = dv * b;
      s2          = (k - ab * g) / (ab * ab - 1.);
