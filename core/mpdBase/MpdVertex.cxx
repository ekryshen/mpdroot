...
MpdVertex::MpdVertex(const char *name, const char *title) : TNamed(name, title)
{
   fX = fY = fZ = fChi2 = 0.;
   fNDF = fNTracks = 0;
   for (Int_t i = 0; i < 6; i++) fCovMatrix[i] = 0;
...
MpdVertex::MpdVertex(const char *name, const char *title, Double_t x, Double_t y, Double_t z, Double_t chi2, Int_t ndf,
...
MpdVertex::MpdVertex(const MpdVertex &vert)
   : TNamed(vert), fX(vert.fX), fY(vert.fY), fZ(vert.fZ), fChi2(vert.fChi2), fNDF(vert.fNDF), fNTracks(vert.fNTracks),
     fTrInd(new TArrayI(1))
{
   /// copy constructor
...
void MpdVertex::SetVertex(Double_t x, Double_t y, Double_t z, Double_t chi2, Int_t ndf, Int_t nTracks,
