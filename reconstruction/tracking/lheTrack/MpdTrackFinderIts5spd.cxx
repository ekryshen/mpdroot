...
MpdTrackFinderIts5spd::MpdTrackFinderIts5spd(Bool_t useVector, const char *name, Int_t iVerbose)
   : FairTask(name, iVerbose), fExact(0), // 1),
     fGeo(0), fUseVector(useVector), fTpcKF(nullptr)
{
...
void MpdTrackFinderIts5spd::FillGeoScheme()
{
   ...
   for (Int_t layer = 1; layer < 999; ++layer) {
      ...
      for (Int_t ladder = 1; ladder < 999; ++ladder) {
         ...
         // cout << "Node: " << layer << " " << vLad->GetNodes()->GetEntriesFast() << " " << vLad->GetNdaughters() <<
         // endl;
         ...
         for (Int_t det = 0; det < nDaught; ++det) {
            ...
            // cout << "det1= " << det1 << " " << iZ << " layer= " << layer << " ladder="  << ladder << " detID= " <<
            // detID << endl;
            ...
            //	  cout << "##### Node: " << node->GetName() <<  " DX= " << box->GetDX() << " DZ=" << box->GetDZ() <<
            // endl;
...
void MpdTrackFinderIts5spd::MakeKalmanHits()
{
   ...
   if (!fUseVector) {
      for (Int_t ih = 0; ih < nHits; ++ih) {
         ...
         xloc             = h->GetLocalX();
         lay              = h->Layer() - 1;
         //    cout <<" n= " << ih << " r= " << r << " z= " << z << " layer= " << h->Layer()  << " xloc= " << xloc <<
         //    endl;
   ...
   for (Int_t ihit = 0; ihit < nKH; ++ihit) {
      ...
      if (TMath::Abs(rphi - rphiMax) < 3.0) {
         ...
         if (rphi < 0)
            fHitMapRphi[lay].insert(pair<Double_t, Int_t>(rphi + 2 * rphiMax, ihit)); // overlap 3 cm near phi=180
         else
            fHitMapRphi[lay].insert(pair<Double_t, Int_t>(rphi - 2 * rphiMax, ihit)); // overlap 3 cm near phi=180
      }
   }
...
Bool_t MpdTrackFinderIts5spd::NavigateToLayer(Int_t lay, MpdItsKalmanTrack *curTr,
                                              std::map<Int_t, MpdItsKalmanTrack> &trackBr,
                                              std::multimap<Int_t, Int_t>        &hitsInWin)
{
   ...
   //  cout << " Nodes: " << nodeNew << " " << curTr->GetNode() << " Positions: " << posNew << " " << curTr->GetPos() <<
   //  endl; curTr->GetParamNew()->Print(); curTr->GetParam()->Print();
   ...
   vector<Int_t>::iterator vit =
      set_intersection(indsRphi.begin(), indsRphi.end(), indsZ.begin(), indsZ.end(), indv.begin());
