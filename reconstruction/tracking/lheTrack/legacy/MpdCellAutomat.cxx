...
MpdCellAutomat::MpdCellAutomat(const char *name, Int_t iVerbose)
   : FairTask(name, iVerbose), fExact(0), fNTracks(0), fNPass(2), fGeo(0)
{
...
InitStatus MpdCellAutomat::Init()
{
   ...
   for (Int_t i = 0; i < nLay; ++i) {
      ...
      cout << " *** STS sensitive volume: " << sensVol->GetName() << " " << params->At(0) << " " << fDz[i] << " "
           << fZmod[i] << endl;
   }
   // fStereoA[0] = -7.5;
   // fStereoA[1] = 7.5;
...
void MpdCellAutomat::MakeKalmanHits()
{
   ...
   for (Int_t ih = 0; ih < nHits; ++ih) {
      ...
      // Double_t dist, Int_t index) MpdKalmanStripHit *hit = new ((*fKHits)[nKH++]) MpdKalmanStripHit(r,
      // fStereoA[h->Side()], 			  xloc+dX*errX, z+dZ*errZ, errX, errZ, lay, ih);
      ...
      if (fGeo && h->GetUniqueID())
         hit = new ((*fKHits1)[nKH++]) MpdKalmanHit(lay * 1000000 + fId2Id[lay][h->GetDetectorID()], 1,
                                                    MpdKalmanHit::kFixedP, meas, err, cossin, 0., r, ih);
      // Mask out sector number - sensor layout
      else if (fGeo)
         hit = new ((*fKHits1)[nKH++]) MpdKalmanHit(lay * 1000000 + (h->GetDetectorID() & ((2 << 12) - 1)), 1,
                                                    MpdKalmanHit::kFixedP, meas, err, cossin, 0., r, ih);
      else
         hit = new ((*fKHits1)[nKH++])
            MpdKalmanHit(lay * 1000000 + nKH - 1, 1, MpdKalmanHit::kFixedR, meas, err, cossin, 0., r, ih);
      hit->SetUniqueID(0);
   ...
   for (Int_t i234 = 0; i234 < 4; ++i234) { // Loop over layers 1-4
      ...
      for (Int_t ih = fLayBeg[i234]; ih < hEnd; ++ih) {
         ...
         for (Int_t ih1 = ih + 1; ih1 < hEnd; ++ih1) {
            ...
            Double_t x1 =
               ((xloc[1] * cossin[1] - xloc[0] * cossinn[1]) / (cossinn[0] * cossin[1] - cossin[0] * cossinn[1]));
            ...
            // printf("sizeZ=%f %d %d %d %d %d %d %d %d %f
            ...
            if (h->Layer() == 1 && TMath::Abs(v31[2]) > 18.0)
               continue; // Eta < 1.99
            else if (h->Layer() == 2 && TMath::Abs(v31[2]) > 40.0)
               continue; // rad 11
            else if (h->Layer() == 3 && TMath::Abs(v31[2]) > 61.0)
               continue; //  rad 17
            else if (h->Layer() == 4 && TMath::Abs(v31[2]) > 83.0)
               continue; //  rad 23
            ...
            /*
               ////printf("angL=%f\n",angL);
               //(Int_t detID, Int_t nDim, HitType hitType, Double_t *meas, Double_t *err, Double_t *cosSin, Double_t
               signal, Double_t dist, Int_t index)

               //MpdKalmanStripHit *hit = new ((*fKHits)[nKH++]) MpdKalmanStripHit(r, fStereoA[h->Side()],
               //			  xloc+dX*errX, z+dZ*errZ, errX, errZ, lay, ih);
               //hit->SetType(MpdKalmanHit::kFixedR);
               //cout << h->GetDetectorID() << " " << fId2Id[lay][h->GetDetectorID()] << endl;
               //if (fGeo && h->GetUniqueID()) hit = new ((*fKHits)[nKH++]) MpdKalmanHit(lay*1000000+h->GetDetectorID(),
               1,
               // if (fGeo && h->GetUniqueID()) hit = new ((*fKHits)[nKH++])
               MpdKalmanHit(lay1*1000000+fId2Id[lay1][hsts->GetDetectorID()], 1, MpdKalmanHit::kFixedP,meas, err,
               cossin, 0., r, ih);//ms
               */

            if (fGeo && h->GetUniqueID())
               hit = new ((*f2DHits[h->Layer() - 1])[nKH++])
                  MpdCellTrack(lay1 * 1000000 + fId2Id[lay1][hsts->GetDetectorID()], 1, MpdCellTrack::kFixedP, point1,
                               err, cossin, 0., r, ih, ih1, -1); // ms 17.06
            // Mask out sector number - sensor layout
            // else if (fGeo) hit = new ((*fKHits)[nKH++]) MpdKalmanHit(lay*1000000+(h->GetDetectorID()&((2<<12)-1)), 1,
            // MpdKalmanHit::kFixedP,meas, err, cossin, 0., r, ih);//ms else hit = new ((*fKHits)[nKH++])
            // MpdKalmanHit(lay*1000000+nKH-1, 1, MpdKalmanHit::kFixedR,meas, err, cossin, 0., r, ih); //ms
            else if (fGeo)
               hit = new ((*f2DHits[h->Layer() - 1])[nKH++])
                  MpdCellTrack(lay * 1000000 + (h->GetDetectorID() & ((2 << 12) - 1)), 1, MpdCellTrack::kFixedP, point1,
                               err, cossin, 0., r, ih, ih1, -1); // ms 17.06
            else
               hit = new ((*f2DHits[h->Layer() - 1])[nKH++])
                  MpdCellTrack(lay * 1000000 + nKH - 1, 1, MpdCellTrack::kFixedR, point1, err, cossin, 0., r, ih, ih1,
                               -1); // ms 17.06
...
void MpdCellAutomat::MakeTrackCandidates(Int_t iPass)
{
   ...
   for (Int_t i = 0; i < nHits; ++i) {
      ...
      if (fGeo && h->GetUniqueID())
         cellTr = new ((*fKHits[0])[nKH++]) MpdCellTrack(*cellTr); /// work!!
      else if (fGeo)
         cellTr = new ((*fKHits[0])[nKH++]) MpdCellTrack(*cellTr); //
      else
         cellTr = new ((*fKHits[0])[nKH++]) MpdCellTrack(*cellTr); //
...
void MpdCellAutomat::ExtendCellTracks(Int_t iPass)
{
   ...
   for (Int_t i234 = 1; i234 < 4; ++i234) { // Loop over layers 1-3
      ...
      for (Int_t i = 0; i < nHits2; ++i) {
         ...
         Int_t nTracks =
            fKHits[i234 - 1]->GetEntriesFast(); // prediduchii sloi fkHits ->massiv ykazateley// work version 6.06.2014
         ...
         for (Int_t itr = 0; itr < nTracks; ++itr) {
            Float_t angT, angL, deltaZ,
               dangL; // ang1->angT(transverse-"poperechnii" ); ang2->angL(longitudinal-"prodolnii");
            ...
            if (track->GetPrevTrack() < 0) {
               ...
               if (iPass == 0 && TMath::Abs(angT) > 0.023)
                  continue; // Pt > 0.4 GeV/c
               else if (iPass == 1 && TMath::Abs(angT) > 0.043)
                  continue; // Pt > 0.2 GeV/c
               else if (iPass == 2 && TMath::Abs(angT) > 0.18)
                  continue; // Pt > 0.05 GeV/c

               if (TMath::Abs(dangL) > nSigm * Interp(TMath::Abs(angT), 0, i234 - 1)) continue;

            } else {

               MpdCellTrack *track1 = (MpdCellTrack *)fKHits[i234 - 2]->UncheckedAt(
                  track->GetPrevTrack()); // trek s previos layer // work version 6.06.2014
               ...
               if (h->Layer() == 3 && TMath::Abs(dangL) > 0.02 * nSigm)
                  continue; // sigma lay == 3 pt 0.05 GeV
               else if (h->Layer() == 4 && TMath::Abs(dangL) > 0.016 * nSigm)
                  continue; // sigma lay == 4 pt 0.05 GeV
               ...
               // if (TMath::Abs(angT - track->GetCosSin(0)) > nSigm * Interp(TMath::Abs(angTmean),1,h->Layer()-3))
               // continue;///dangt
               if (TMath::Abs(angTmean - track->GetCosSin(0)) > nSigm * Interp(TMath::Abs(angTmean), 1, i234 - 2))
                  continue; /// dangt
            ...
            if (fGeo && h->GetUniqueID())
               trNew = new ((*fKHits[h->Layer() - 1])[nKH++]) MpdCellTrack(*hit);
            else if (fGeo)
               trNew = new ((*fKHits[h->Layer() - 1])[nKH++]) MpdCellTrack(*hit);
            else
               trNew = new ((*fKHits[h->Layer() - 1])[nKH++]) MpdCellTrack(*hit);
...
void MpdCellAutomat::GetTrackSeeds(Int_t iPass)
{
   ...
   for (Int_t itr = 0; itr < nTracks; ++itr) {
      ...
      // MpdItsKalmanTrack *track = new ((*fTracks)[nCand++]) MpdItsKalmanTrack(*track1, vert); //sozdatia constructor
      // iz CellTrack vvodit v ITS track constructor
...
void MpdCellAutomat::EvalCovar(Double_t *parOut, Double_t *parIn, MpdItsKalmanTrack *track, const MpdCellTrack *track1)
{
   ...
   // Obtain errors
   // if (lunErr && idEct == tofP->GetTrackID()) fprintf(lunErr,"%10.3e %10.3e %10.3e %10.3e %10.3e %10.3e %10.3e %10.3e
   // %10.3e %10.3e
   ...
   */
   ...
   if (lunErr&&p->GetTrackID()<1) fprintf(lunErr,"%12.3e %12.3e %12.3e %12.3e %12.3e %12.3e %12.3e %12.3e %12.3e\n",
             ...
             TMath::Sqrt(wwTmp(0,0)),TMath::Sqrt(wwTmp(1,1)),TMath::Sqrt(wwTmp(2,2)),TMath::Sqrt(wwTmp(3,3)), phi); //9
   parametr "phi" writing in "file.txt"
   */
   if (lunErr && p->GetTrackID() < 1)
      fprintf(lunErr, "%12.3e %12.3e %12.3e %12.3e %12.3e %12.3e %12.3e %12.3e %12.3e\n",
              ...
              TMath::PiOver2() - pmom.Theta() - track->GetParam(3), TMath::Sqrt(wwTmp(0, 0)), TMath::Sqrt(wwTmp(1, 1)),
              TMath::Sqrt(wwTmp(2, 2)), TMath::Sqrt(wwTmp(3, 3)));
   /*
...
void MpdCellAutomat::DoTracking(Int_t iPass)
{
   ...
   for (Int_t i = 0; i < nCand; ++i) {
      ...
      // cout << " Track seed No. " << i << ", ID: " << track->GetTrackID() << ", Hits: " << track->GetNofTrHits() <<
      // endl;
      ...
      */
      ...
      // ((MpdKalmanHitR*)track->GetHits()->First())->GetLength() << endl; Double_t pos =
      // ((MpdKalmanHit*)track->GetHits()->Last())->GetPos(); MpdKalmanFilter::Instance()->PropagateParamR(track, &hit,
      // kTRUE);
      iok = MpdKalmanFilter::Instance()->PropagateToHit(track, &hit, kTRUE);
...
Int_t MpdCellAutomat::RunKalmanFilterCell(MpdItsKalmanTrack *track)
{
   ...
   for (Int_t lay = 0; lay < 4; ++lay) {
      ...
      if (lay == 0)
         cellTr = (MpdCellTrack *)fKHits[lay]->UncheckedAt(track3->GetPrevTrack()); // 1 layer
      else if (lay == 1)
         cellTr = (MpdCellTrack *)fKHits[lay]->UncheckedAt(track2->GetPrevTrack()); // 2 layer
      if (lay == 2)
         cellTr = (MpdCellTrack *)fKHits[lay]->UncheckedAt(track1->GetPrevTrack()); // 3 layer
      else if (lay == 3)
         cellTr = (MpdCellTrack *)fKHits[lay]->UncheckedAt(track->GetUniqueID() - 1); // 4 layer
                                                                                      /*
      ...
      for (Int_t ihit = 0; ihit < 2; ++ihit) {
         ...
         if (TMath::Abs(hit->GetPos() - track->GetPosNew()) > 1.e-4) {
            ...
            if (lay1 % 2 != 0 && step > 1.e-4) {
               ...
               // cout << " Scat: " << hit->GetLayer() << " " << step << " " << TMath::Sqrt(angle2) << endl; ///old
               // coment
         ...
         // else cout << " lay, c2, id1, id2: " << lay << " " << dChi2 << " " << TrackID(hit) << " " << TrackID(hit,1)
         // << endl;
...
TVector2 MpdCellAutomat::GetDistance(MpdKalmanTrack *track, MpdKalmanHit *hit)
{
   ...
   if (dMod != 0) {
      ...
      if (dMod > 0)
         dZ = zloc - (module + 1) * fZmod[lay2];
      else
         dZ = zloc - module * fZmod[lay2];
...
void MpdCellAutomat::AddHits()
{
   ...
   for (Int_t i = 0; i < nFound; ++i) {
      ...
      cout << nHits << " " << track->GetNofTrHits() << " " << track->GetTrackID() << " " << track->GetChi2Its() << " "
           << track->GetChi2() << endl;
   }
