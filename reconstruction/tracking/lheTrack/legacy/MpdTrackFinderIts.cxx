...
MpdTrackFinderIts::MpdTrackFinderIts(const char *name, Int_t iVerbose) : FairTask(name, iVerbose), fGeo(0)
{
...
void MpdTrackFinderIts::MakeKalmanHits()
{
   ...
   for (Int_t ih = 0; ih < nHits; ++ih) {
      ...
      // Mask out sector number - sensor layout
         ...
         hit = new ((*fKHits)[nKH++]) MpdKalmanHit(lay * 1000000 + (h->GetDetectorID() & ((2 << 12) - 1)), 1,
                                                   MpdKalmanHit::kFixedP, meas, err, cossin, 0., r, ih);
      else
         hit = new ((*fKHits)[nKH++])
...
void MpdTrackFinderIts::GetTrackSeeds(Int_t iPass)
{
   ...
   for (Int_t itr = 0; itr < nSeeds; ++itr) {
      ...
      if (TString(tpc->ClassName()).Contains("Tpc"))
         track = new ((*fTracks)[nCand++]) MpdItsKalmanTrack(*tpc);
      else
         track = new ((*fTracks)[nCand++]) MpdItsKalmanTrack(*((MpdEctKalmanTrack *)tpc));
   ...
   for (Int_t i = 0; i < nCand; ++i) {
      ...
      */
      ...
      if (fGeo)
         ...
         iok = RunKalmanFilterCyl(track, lay0); // cylindrical geometry
      if (iok == -1) {
         fTracks->RemoveAt(i);
         continue;
      }
...
Int_t MpdTrackFinderIts::RunKalmanFilterCyl(MpdItsKalmanTrack *track, Int_t layBeg)
{
   ...
   for (Int_t lay = layBeg; lay != layEnd; lay += dLay) {
      ...
      for (Int_t ibr = 0; ibr < nBr0; ++ibr) {
         ...
         for (Int_t indx = indxBeg; indx != indxEnd; indx += dIndx) {
            ...
            // if (track->GetTrackID() != -1335) cout <<
            // /*((TpcLheKalmanTrack*)fTpcTracks->UncheckedAt(track->GetTpcIndex()))->GetNofTrHits() <<*/ " " <<
            // dist.X() << " " << dist.Y() << " " << hit->GetLayer() << endl;
            ...
            // cout << " Chi2: " << dChi2 << " " << dist.X() << " " << dist.Y() << " " << curTr->GetParamNew(0) << " "
            // << hit->GetLayer() << endl;
   ...
   for (Int_t i = 1; i < nBr; ++i) {
      if (branch[i].GetNofHits() > branch[ibest].GetNofHits())
         ibest = i;
      else if (branch[i].GetNofHits() == branch[ibest].GetNofHits() && branch[i].GetChi2() < branch[ibest].GetChi2())
         ibest = i;
   }
   track->Reset();
   *track = branch[ibest];
                                          ...
                                          std::map<Int_t, Int_t> &trackBrM)
{
   ...
   for (Int_t iover = 0; iover < 2; ++iover) {
      ...
      if (iover == 0) {
         ...
         /*cout << " Node: " << inode << " " << gGeoManager->GetPath() << endl;*/ }
            ...
            break;
                  ...
                  // cout << " ok " << gGeoManager->GetCurrentPoint()[0] << " " <<  gGeoManager->GetCurrentPoint()[1] <<
                  // " " << gGeoManager->GetCurrentPoint()[2] << " " << gGeoManager->GetStep() << endl;
      ...
      // cout << iover << " " << name << " " << detID[iover] << " " << hit->GetDetectorID() << " " << hitDb1.Ladder() <<
      // " " << hitDb1.Sector() << " " << hitDb2.Ladder() << " " << hitDb2.Sector() << " " << hit->GetMeas(0) << endl;
      break;
   ...
   for (Int_t iover = 0; iover < 3; ++iover) {
      if (detID[iover] >= 0) {
         ...
         if (!MpdKalmanFilter::Instance()->PropagateToHit(&trackBr[iover], &hitTmp, kTRUE, kTRUE)) {
            ...
            // cout << iover << " " << detID[iover] << " " << trackBr[iover].GetNode() << " " <<
            // trackBr[iover].GetNodeNew() << " " << trackBr[iover].GetNofHits() << endl;
            detID[iover] = -1;
            continue;
         }
         ...
         if (firstBranch) {
            ...
            if (TMath::Abs(TMath::Abs(xloc) - size.X()) < 0.3) {
               ...
               if (iover < 2 && detID[iover + 1] < 0)
                  detID[iover + 1] = 1000000 * lay + fHitSts.SetLadder(idSTS, ladder);
               else if (iover < 1 && detID[iover + 2] < 0)
                  detID[iover + 2] = 1000000 * lay + fHitSts.SetLadder(idSTS, ladder);
            }
            if (TMath::Abs(TMath::Abs(zloc) - size.Y()) < 0.3) {
               ...
               if (sector > 0 && sector <= fNsectors[lay]) {
                  if (iover < 2 && detID[iover + 1] < 0)
                     detID[iover + 1] = 1000000 * lay + fHitSts.SetSensor(idSTS, sector);
                  else if (iover < 1 && detID[iover + 2] < 0)
                     detID[iover + 2] = 1000000 * lay + fHitSts.SetSensor(idSTS, sector);
               }
            }
            firstBranch = 0;
         }
