...
std::shared_ptr<const Acts::TrackingGeometry> buildTGeoDetector(
    ...
    std::function<bool(TGeoManager*)> geoEditor) {
  ...
  if (!config.fileName.empty()) {
    // Import the geometry from the ROOT file
    geoManager = TGeoManager::Import(config.fileName.c_str());
    assert(geoManager && "Problems when importing the geometry");
  } else {
    // Take the existing geometry.
    geoManager = gGeoManager;
    assert(geoManager && "Missing geometry");
  }

  // Modify the geometry if required
  if (geoEditor != nullptr) {
    bool status = geoEditor(geoManager);
    assert(status && "Problems when editing the geometry");
  }
...
auto TGeoDetector::finalize(
  ...
  TrackingGeometryPtr tgeoTrackingGeometry =
      buildTGeoDetector(cfg, tGeoContext, detectorStore, mdecorator, geoEditor);
