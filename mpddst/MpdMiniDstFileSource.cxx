...
MpdMiniDstFileSource::MpdMiniDstFileSource(TFile *f, const char* Title, UInt_t identifier)
: FairSource(),
fInputTitle(Title)
, fRootFile(f)
, fCurrentEntryNr(0)
...
, fCheckFileLayout(kTRUE) {
    if (fRootFile->IsZombie())
        Fatal("MpdMiniDstFileSource::MpdMiniDstFileSource", "Error opening the Input file");
}

//_____________________________________________________________________________

MpdMiniDstFileSource::MpdMiniDstFileSource(const TString* RootFileName, const char* Title, UInt_t identifier)
: FairSource()
, fInputTitle(Title)
, fRootFile(0)
, fCurrentEntryNr(0)
...
, fCheckFileLayout(kTRUE) {
    ...
    if (fRootFile->IsZombie())
        ...
        // fInTree = (TTree*) fRootFile->Get("MiniDst");
        fInChain = new TChain("MiniDst");
...
MpdMiniDstFileSource::~MpdMiniDstFileSource() {
}

//_____________________________________________________________________________

Bool_t MpdMiniDstFileSource::Init() {
    if (IsInitialized) {
        LOG(INFO) << "MpdMiniDstFileSource already initialized";
        return kTRUE;
    }

    if (!fInChain) {
        fInChain = new TChain(FairRootManager::GetTreeName(), "/cbmroot");
        LOG(DEBUG) << "MpdMiniDstFileSource::Init() chain created";
...
void MpdMiniDstFileSource::SetInTree(TTree* tempTree) {
    fInTree = tempTree;
...
Int_t MpdMiniDstFileSource::ReadEvent(UInt_t i) {
    fCurrentEntryNo = i;
    SetEventTime();
...
void MpdMiniDstFileSource::Close() {
    CloseInFile();
}
//_____________________________________________________________________________

void MpdMiniDstFileSource::Reset() {
}

//_____________________________________________________________________________

void MpdMiniDstFileSource::AddFriend(TString fName) {
    fFriendFileList.push_back(fName);
}

//_____________________________________________________________________________

void MpdMiniDstFileSource::AddFile(TString FileName) {
    fInputChainList.push_back(FileName);
}
//_____________________________________________________________________________

void MpdMiniDstFileSource::AddFriendsToChain() {
...
void MpdMiniDstFileSource::PrintFriendList() {
...
void MpdMiniDstFileSource::CheckFriendChains() {
...
void MpdMiniDstFileSource::CreateNewFriendChain(TString inputFile, TString inputLevel) {
    TFile* temp = gFile;
...
Bool_t MpdMiniDstFileSource::CompareBranchList(TFile* fileHandle, TString inputLevel) {
    ...
    TChain* fileTree = new TChain("MiniDst", "/cbmroot");
...
Bool_t MpdMiniDstFileSource::ActivateObject(TObject** obj, const char* BrName) {
    if (fInTree) {
...
void MpdMiniDstFileSource::SetInputFile(TString name) {
    fRootFile = new TFile(name.Data());

    if (fRootFile->IsZombie())
        LOG(FATAL) << "Error opening the Input file";

    LOG(INFO) << "MpdMiniDstFileSource set------------";
}

//_____________________________________________________________________________

Int_t MpdMiniDstFileSource::CheckMaxEventNo(Int_t EvtEnd) {
    Int_t MaxEventNo = 0;
    if (EvtEnd != 0)
        MaxEventNo = EvtEnd;
...
void MpdMiniDstFileSource::SetEventMeanTime(Double_t mean) {
    fEventMeanTime = mean;
...
void MpdMiniDstFileSource::SetEventTimeInterval(Double_t min, Double_t max) {
    fEventTimeMin = min;
    fEventTimeMax = max;
...
void MpdMiniDstFileSource::SetBeamTime(Double_t beamTime, Double_t gapTime) {
    fBeamTime = beamTime;
    fGapTime = gapTime;
}
...
void MpdMiniDstFileSource::SetEventTime() {
...
Double_t MpdMiniDstFileSource::GetDeltaEventTime() {
...
Double_t MpdMiniDstFileSource::GetEventTime() {
...
void MpdMiniDstFileSource::ReadBranchEvent(const char* BrName) {
...
void MpdMiniDstFileSource::ReadBranchEvent(const char* BrName, Int_t Entry) {
...
void MpdMiniDstFileSource::FillEventHeader(FairEventHeader* feh) {
