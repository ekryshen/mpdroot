...
MpdEmcGeoParams::MpdEmcGeoParams(): 
  fSectorNumber(25), fCrateNumber(6), fModuleTypes(8), fTowerNumber(38400/2),
  fTowerNumberXY(2), fTowerNumberZ(64), fTowerLength(41.55) {
...
// Total barell sizes
    ...
    for (Int_t iChamber = 0; iChamber < 2; iChamber++) {
     ...
     for (Int_t iTower = 0; iTower < fTowerNumber; iTower++) {
      ...
      if (iTowerRange%2 == 0) {iTowerRange = 0;  iTowerNum++;}  
      if (iTowerNum == fTowerNumberZ + 1) iTowerNum = 1; 
      iModule = iTowerNode/(2*fModuleTypes);              
      pathTower.Form("%s/emcSector_%d/emcCrate_%d/emcModule%d_0/emc_box%d_%d/",
		pathCh.Data(),iSector,iCrate,iModule,iTowerNum,iTowerNode);  
      ...
      xTower.push_back(fTransMatrix[0]); yTower.push_back(fTransMatrix[1]);
      zTower.push_back(fTransMatrix[2]); 
      rhoTower.push_back(sqrt(fTransMatrix[0]*fTransMatrix[0] +
			      fTransMatrix[1]*fTransMatrix[1] + fTransMatrix[2]*fTransMatrix[2]));
      iTowerElem++;
